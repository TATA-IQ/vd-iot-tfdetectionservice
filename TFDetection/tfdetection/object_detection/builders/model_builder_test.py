...
from object_detection.protos import hyperparams_pb2, losses_pb2, model_pb2
...
class ModelBuilderTest(test_case.TestCase, parameterized.TestCase):
    ...
    def create_default_faster_rcnn_model_proto(self):
      ...
      faster_rcnn {
        ...
        model_proto = model_pb2.DetectionModel()
        text_format.Merge(model_text_proto, model_proto)
        (model_proto.faster_rcnn.feature_extractor.type) = self.default_faster_rcnn_feature_extractor()
        return model_proto
    ...
    @parameterized.named_parameters(
        {"testcase_name": "mask_rcnn_with_matmul", "use_matmul_crop_and_resize": False, "enable_mask_prediction": True},
        {
    ...
    def test_create_faster_rcnn_models_from_config(self, use_matmul_crop_and_resize, enable_mask_prediction):
        ...
        for extractor_type, extractor_class in self.faster_rcnn_feature_extractors().items():
    ...
    def test_create_faster_rcnn_from_config_with_crop_feature(self, output_final_box_features):
        model_proto = self.create_default_faster_rcnn_model_proto()
    ...
    def test_invalid_model_config_proto(self):
        model_proto = ""
        with self.assertRaisesRegex(ValueError, "model_config not of type model_pb2.DetectionModel."):
    ...
    def test_invalid_faster_rcnn_batchnorm_update(self):
        ...
        with self.assertRaisesRegex(ValueError, "inplace batchnorm updates not supported"):
            model_builder.build(model_proto, is_training=True)
