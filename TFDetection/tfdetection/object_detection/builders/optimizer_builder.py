...
try:
    from tensorflow.contrib import \
        opt as tf_opt  # pylint: disable=g-import-not-at-top
...
def build_optimizers_tf_v1(optimizer_config, global_step=None):
    ...
    if optimizer_config.use_moving_average:
        optimizer = tf_opt.MovingAverageOptimizer(optimizer, average_decay=optimizer_config.moving_average_decay)
...
def _create_learning_rate(learning_rate_config, global_step=None):
    ...
    if learning_rate_type == "manual_step_learning_rate":
        ...
        learning_rate = learning_schedules.manual_stepping(
            global_step, learning_rate_step_boundaries, learning_rate_sequence, config.warmup
        )
