...
class BalancedPositiveNegativeSampler(minibatch_sampler.MinibatchSampler):
    ...
    def _static_subsample(self, indicator, batch_size, labels):
        ...
        indicator_idx = tf.where(indicator, tf.range(1, input_length + 1), tf.zeros(input_length, tf.int32))

        # Replace -1 for negative, +1 for positive labels
        signed_label = tf.where(
            labels, tf.ones(input_length, tf.int32), tf.scalar_mul(-1, tf.ones(input_length, tf.int32))
        )
        ...
        sampled_idx_indicator = tf.cast(tf.reduce_sum(tf.one_hot(sampled_idx, depth=input_length), axis=0), tf.bool)

        # project back the order based on stored permutations
        idx_indicator = tf.scatter_nd(tf.expand_dims(permutation, -1), sampled_idx_indicator, shape=(input_length,))
