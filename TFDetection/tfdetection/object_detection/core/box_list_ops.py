...
def l1(boxlist1, boxlist2, scope=None):
    ...
    with tf.name_scope(scope, "PairwiseL1"):
        ...
        ycenters = tf.abs(tf.expand_dims(ycenter2, axis=0) - tf.expand_dims(tf.transpose(ycenter1), axis=1))
        xcenters = tf.abs(tf.expand_dims(xcenter2, axis=0) - tf.expand_dims(tf.transpose(xcenter1), axis=1))
        heights = tf.abs(tf.expand_dims(h2, axis=0) - tf.expand_dims(tf.transpose(h1), axis=1))
        widths = tf.abs(tf.expand_dims(w2, axis=0) - tf.expand_dims(tf.transpose(w1), axis=1))
    ...
    with tf.name_scope(scope, "BooleanMask"):
        ...
        if use_static_shapes:
            ...
            sampled_indices = tf.cast(
                tf.tensordot(
                    tf.cast(tf.range(tf.shape(indicator)[0]), dtype=tf.float32), one_hot_selector, axes=[0, 0]
                ),
                dtype=tf.int32,
            )
...
def sort_by_field(boxlist, field, order=SortOrder.descend, scope=None):
    ...
    with tf.name_scope(scope, "SortByField"):
        ...
        length_assert = tf.Assert(
            tf.equal(num_boxes, num_entries), ["Incorrect field size: actual vs expected.", num_entries, num_boxes]
        )
...
def select_random_box(boxlist, default_box=None, seed=None, scope=None):
    ...
    return tf.cond(
        tf.greater_equal(number_of_boxes, 1), true_fn=select_box, false_fn=lambda: (default_box, tf.constant(False))
    )
...
def get_minimal_coverage_box(boxlist, default_box=None, scope=None):
    ...
    with tf.name_scope(scope, "CreateCoverageBox"):
        ...
        return tf.cond(
            tf.greater_equal(num_boxes, 1), true_fn=lambda: coverage_box(boxlist.get()), false_fn=lambda: default_box
        )
