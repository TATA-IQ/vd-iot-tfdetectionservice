...
def prune_outside_window(dp_num_points, dp_part_ids, dp_surface_coords, window, scope=None):
    ...
    with tf.name_scope(scope, "DensePosePruneOutsideWindow"):
        ...
        num_instances, num_points = shape_utils.combined_static_and_dynamic_shape(dp_part_ids)
        dp_num_points_tiled = tf.tile(dp_num_points[:, tf.newaxis], multiples=[1, num_points])
        range_tiled = tf.tile(tf.range(num_points)[tf.newaxis, :], multiples=[num_instances, 1])
        valid_initial = range_tiled < dp_num_points_tiled
        valid_in_window = tf.logical_and(
            tf.logical_and(y >= win_y_min, y <= win_y_max), tf.logical_and(x >= win_x_min, x <= win_x_max)
        )
        ...
        def gather_and_reshuffle(elems):
            ...
            valid_part_ids_padded = shape_utils.pad_or_clip_nd(valid_part_ids, output_shape=[max_num_points])
...
def change_coordinate_frame(dp_surface_coords, window, scope=None):
    ...
    with tf.name_scope(scope, "DensePoseChangeCoordinateFrame"):
        ...
        new_dp_surface_coords = scale(
            dp_surface_coords - [window[0], window[1], 0, 0], 1.0 / win_height, 1.0 / win_width
        )
        return new_dp_surface_coords


def to_normalized_coordinates(dp_surface_coords, height, width, check_range=True, scope=None):
...
def to_absolute_coordinates(dp_surface_coords, height, width, check_range=True, scope=None):
    ...
    with tf.name_scope(scope, "DensePoseToAbsoluteCoordinates"):
        ...
        if check_range:
            ...
            max_assert = tf.Assert(
                tf.greater_equal(1.01, max_val), ["maximum coordinate value is larger than 1.01: ", max_val]
            )
            with tf.control_dependencies([max_assert]):
                width = tf.identity(width)
...
class DensePoseHorizontalFlip(object):
    ...
    def __init__(self):
        ...
        uv_symmetry_transforms_path = tf.resource_loader.get_path_to_datafile(
            os.path.join(path, "..", "dataset_tools", "densepose", "UV_symmetry_transforms.mat")
        )
        ...
        for key in ("U_transforms", "V_transforms"):
            ...
            uv_symmetry_map[key] = tf.reshape(tf.stack(uv_symmetry_map_per_part, axis=0), [-1])
        ...
        for i, part_name in enumerate(PART_NAMES):
            if b"left" in part_name:
                part_symmetries.append(PART_NAMES.index(part_name.replace(b"left", b"right")))
            elif b"right" in part_name:
                part_symmetries.append(PART_NAMES.index(part_name.replace(b"right", b"left")))
            else:
    ...
    def flip_parts_and_coords(self, part_ids, vu):
        ...
        symmetry_lookup_inds = part_ids_flattened * 65536 + 256 * v_locs_flattened + u_locs_flattened
