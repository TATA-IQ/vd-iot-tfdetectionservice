...
def _self_suppression(iou, iou_threshold, loop_condition, iou_sum):
    ...
    iou_suppressed = (
        tf.reshape(tf.cast(tf.reduce_max(can_suppress_others * iou, 1) <= iou_threshold, iou.dtype), [1, -1, 1]) * iou
    )
...
def _suppression_loop_body(boxes, iou_threshold, output_size, idx):
    ...
    mask = tf.expand_dims(
        tf.reshape(tf.range(_NMS_TILE_SIZE), [1, -1]) > tf.reshape(tf.range(_NMS_TILE_SIZE), [-1, 1]), 0
    )
    ...
    boxes = tf.tile(tf.expand_dims(box_slice, [1]), [1, num_tiles, 1, 1]) * mask + tf.reshape(
        boxes, [1, num_tiles, _NMS_TILE_SIZE, 4]
    ) * (1 - mask)
...
def partitioned_non_max_suppression_padded(
    ...
    selected_boxes, _, output_size, _ = tf.while_loop(
        _loop_cond, _suppression_loop_body, [boxes, iou_threshold, tf.zeros([1], tf.int32), tf.constant(0)]
    )
...
def _validate_boxes_scores_iou_thresh(boxes, scores, iou_thresh, change_coordinate_frame, clip_window):
    ...
    if not (
        shape_utils.get_dim_as_int(boxes.shape[1]) == shape_utils.get_dim_as_int(scores.shape[1])
...
def _clip_window_prune_boxes(sorted_boxes, clip_window, pad_to_max_output_size, change_coordinate_frame):
    ...
    sorted_boxes_scores = tf.where(
        non_zero_box_area, sorted_boxes.get_field(fields.BoxListFields.scores), -1 * tf.ones(sorted_boxes_size)
    )
...
def multiclass_non_max_suppression(
    ...
    if pad_to_max_output_size and soft_nms_sigma != 0.0:
        raise ValueError(
            "Soft NMS (soft_nms_sigma != 0.0) is currently not supported when pad_to_max_output_size is True."
        )
    ...
    with tf.name_scope(scope, "MultiClassNonMaxSuppression"), tf.device(
        ...
        for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
            ...
            if pad_to_max_output_size:
                ...
                if use_partitioned_nms:
                    ...
                    selected_indices, num_valid_nms_boxes = tf.image.non_max_suppression_padded(
                        boxlist_and_class_scores.get(),
                        boxlist_and_class_scores.get_field(fields.BoxListFields.scores),
                        max_selection_size,
                        iou_threshold=iou_thresh,
                ...
                if (
                    ...
                    selected_indices = tf.concat(
                        [selected_indices, tf.zeros(max_selection_size - num_valid_nms_boxes, tf.int32)], 0
                    )
                    selected_scores = tf.concat(
                        [selected_scores, tf.zeros(max_selection_size - num_valid_nms_boxes, tf.float32)], -1
                    )
                    nms_result = box_list_ops.gather(boxlist_and_class_scores, selected_indices)
                else:
                    if soft_nms_sigma != 0:
        ...
        if max_total_size:
            ...
            num_valid_nms_boxes_cumulative = tf.where(
                max_total_size > num_valid_nms_boxes_cumulative, num_valid_nms_boxes_cumulative, max_total_size
            )
...
def batch_multiclass_non_max_suppression(
    ...
    if use_combined_nms:
        ...
        if clip_window is not None:
            tf.logging.warning(
                "clip_window is not supported by combined_nms unless it is [0. 0. 1. 1.] for each image."
            )
        if additional_fields is not None:
            tf.logging.warning("additional_fields is not supported by combined_nms.")
        if parallel_iterations != 32:
            tf.logging.warning("Number of batch items to be processed in parallel is not configurable by combined_nms.")
        if max_classes_per_detection > 1:
    ...
    with tf.name_scope(scope, "BatchMultiClassNonMaxSuppression"):
        ...
        def _single_image_nms_fn(args):
            ...
            if use_static_shapes:
                ...
                per_image_boxes = tf.reshape(
                    tf.slice(per_image_boxes, 3 * [0], tf.stack([per_image_num_valid_boxes, -1, -1])), [-1, q, 4]
                )
                per_image_scores = tf.reshape(
                    tf.slice(per_image_scores, [0, 0], tf.stack([per_image_num_valid_boxes, -1])), [-1, num_classes]
                )
                per_image_masks = tf.reshape(
                    tf.slice(per_image_masks, 4 * [0], tf.stack([per_image_num_valid_boxes, -1, -1, -1])),
                    [
                        -1,
                        q,
                ...
                if per_image_additional_fields is not None:
                    for key, tensor in per_image_additional_fields.items():
                        ...
                        per_image_additional_fields[key] = tf.reshape(
                            ...
                            [-1] + [shape_utils.get_dim_as_int(dim) for dim in additional_field_shape[1:]],
                        )
            if use_class_agnostic_nms:
        ...
        batch_outputs = map_fn(
            ...
            elems=([boxes, scores, masks, clip_window] + list(ordered_additional_fields.values()) + [num_valid_boxes]),
