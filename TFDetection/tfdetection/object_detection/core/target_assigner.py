...
from object_detection.core import (box_coder, box_list, box_list_ops,
                                   densepose_ops, keypoint_ops)
...
if tf_version.is_tf1():
    from object_detection.matchers import \
        bipartite_matcher  # pylint: disable=g-import-not-at-top
...
class TargetAssigner(object):
    ...
    def assign(
        self, anchors, groundtruth_boxes, groundtruth_labels=None, unmatched_class_label=None, groundtruth_weights=None
    ):
    ...
    def _create_classification_targets(self, groundtruth_labels, unmatched_class_label, match):
        ...
        return match.gather_based_on_match(
            groundtruth_labels, unmatched_value=unmatched_class_label, ignored_value=unmatched_class_label
        )
...
def create_target_assigner(reference, stage=None, negative_class_weight=1.0, use_matmul_gather=False):
    ...
    if reference == "Multibox" and stage == "proposal":
        ...
        box_coder_instance = faster_rcnn_box_coder.FasterRcnnBoxCoder(scale_factors=[10.0, 10.0, 5.0, 5.0])

    elif reference == "FasterRCNN" and stage == "detection":
        similarity_calc = sim_calc.IouSimilarity()
        # Uses all proposals with IOU < 0.5 as candidate negatives.
        matcher = argmax_matcher.ArgMaxMatcher(
            matched_threshold=0.5, negatives_lower_than_unmatched=True, use_matmul_gather=use_matmul_gather
        )
        box_coder_instance = faster_rcnn_box_coder.FasterRcnnBoxCoder(scale_factors=[10.0, 10.0, 5.0, 5.0])
...
def batch_assign(
    ...
    if not (len(anchors_batch) == len(gt_box_batch) == len(gt_class_targets_batch)):
        raise ValueError(
            "batch size incompatible with lengths of anchors_batch, gt_box_batch and gt_class_targets_batch."
        )
    cls_targets_list = []
    cls_weights_list = []
    reg_targets_list = []
...
def batch_get_targets(
    ...
    for match_tensor, groundtruth_tensor, groundtruth_weight in zip(
        ...
        weights = match_object.gather_based_on_match(
            groundtruth_weight, unmatched_value=unmatched_weight, ignored_value=tf.zeros_like(unmatched_weight)
        )
...
def batch_assign_confidences(
    ...
    if not (len(anchors_batch) == len(gt_box_batch) == len(gt_class_confidences_batch)):
        raise ValueError(
            "batch size incompatible with lengths of anchors_batch, gt_box_batch and gt_class_confidences_batch."
        )
    ...
    for anchors, gt_boxes, gt_class_confidences, gt_weights in zip(
        ...
        cls_targets, _, reg_targets, _, match = target_assigner.assign(
            anchors, gt_boxes, gt_class_confidences, unmatched_class_label, groundtruth_weights=gt_weights
        )
        ...
        if include_background_class:
            ...
            classification_weights = tf.concat([cls_weights_background, cls_weights_without_background], axis=-1)
            cls_targets_background = 1 - regression_weights_expanded
            classification_targets = tf.concat([cls_targets_background, cls_targets_without_background], axis=-1)
        else:
...
def max_distance_for_overlap(height, width, min_iou):
    ...
    # in it.
    distance_detection_in_gt = _smallest_positive_root(a=4, b=-2 * (height + width), c=(1 - min_iou) * width * height)
    ...
    distance_gt_in_detection = _smallest_positive_root(
        a=4 * min_iou, b=(2 * min_iou) * (width + height), c=(min_iou - 1) * width * height
    )

    return tf.reduce_min([distance_detection_offset, distance_gt_in_detection, distance_detection_in_gt], axis=0)
...
class CenterNetCenterHeatmapTargetAssigner(object):
    ...
    def assign_center_targets_from_boxes(self, height, width, gt_boxes_list, gt_classes_list, gt_weights_list=None):
        ...
        for boxes, class_targets, weights in zip(gt_boxes_list, gt_classes_list, gt_weights_list):
            boxes = box_list.BoxList(boxes)
            # Convert the box coordinates to absolute output image dimension space.
            boxes = box_list_ops.to_absolute_coordinates(boxes, height // self._stride, width // self._stride)
            # Get the box center coordinates. Each returned tensors have the shape of
            # [num_instances]
            (y_center, x_center, boxes_height, boxes_width) = boxes.get_center_coordinates_and_sizes()

            # Compute the sigma from box size. The tensor shape: [num_instances].
            sigma = _compute_std_dev_from_box_size(boxes_height, boxes_width, self._min_overlap)
...
class CenterNetBoxTargetAssigner(object):
    ...
    def assign_size_and_offset_targets(self, height, width, gt_boxes_list, gt_weights_list=None):
        ...
        for i, (boxes, weights) in enumerate(zip(gt_boxes_list, gt_weights_list)):
            ...
            #   offsets: [num_boxes, 2]
            #   indices: [num_boxes, 2]
            (offsets, indices) = ta_utils.compute_floor_offsets_with_indices(y_source=y_center, x_source=x_center)
...
class CenterNetKeypointTargetAssigner(object):
    ...
    def __init__(
        self, stride, class_id, keypoint_indices, keypoint_std_dev=None, per_keypoint_offset=False, peak_radius=0
    ):
        ...
        if keypoint_std_dev is None:
            self._keypoint_std_dev = [_DEFAULT_KEYPOINT_OFFSET_STD_DEV] * len(keypoint_indices)
        else:
    ...
    def _preprocess_keypoints_and_weights(
        self, out_height, out_width, keypoints, class_onehot, class_weights, keypoint_weights
    ):
        ...
        keypoints_absolute = keypoint_ops.to_absolute_coordinates(keypoints, out_height, out_width)
        # Assign default weights for the keypoints.
        if keypoint_weights is None:
            keypoint_weights = tf.ones_like(keypoints[:, :, 0])
        else:
            keypoint_weights = tf.gather(keypoint_weights, indices=self._keypoint_indices, axis=1)
    ...
    def assign_keypoint_heatmap_targets(
        ...
        for keypoints, classes, kp_weights, weights, boxes in zip(
            gt_keypoints_list, gt_classes_list, gt_keypoints_weights_list, gt_weights_list, gt_boxes_list
        ):
            ...
            num_instances, num_keypoints, _ = shape_utils.combined_static_and_dynamic_shape(keypoints_absolute)
            ...
            keypoint_types = tf.tile(
                input=tf.expand_dims(tf.range(num_keypoints), axis=0), multiples=[num_instances, 1]
            )
            ...
            keypoint_std_dev = tf.tile(
                input=tf.expand_dims(tf.constant(self._keypoint_std_dev), axis=0), multiples=[num_instances, 1]
            )
            ...
            if boxes is not None:
                ...
                keypoint_std_dev = keypoint_std_dev * tf.stack([sigma] * num_keypoints, axis=1)

                # Generate the valid region mask to ignore regions with target class but
                # no corresponding keypoints.
                # Shape: [num_instances].
                blackout = tf.logical_and(classes[:, self._class_id] > 0, tf.reduce_max(kp_weights, axis=1) < 1e-3)
            ...
            heatmap = ta_utils.coordinates_to_heatmap(
                ...
                channel_onehot=tf.one_hot(tf.keras.backend.flatten(keypoint_types), depth=num_keypoints),
                channel_weights=tf.keras.backend.flatten(kp_weights),
            )
            num_instances_list.append(tf.cast(tf.reduce_sum(kp_weights, axis=0), dtype=tf.int32))
            heatmaps.append(heatmap)
        return (tf.stack(heatmaps, axis=0), tf.stack(num_instances_list, axis=0), tf.stack(valid_mask_list, axis=0))
    ...
    def _get_keypoint_types(self, num_instances, num_keypoints, num_neighbors):
        ...
        tiled_keypoint_types = tf.tile(keypoint_types, multiples=[num_instances, 1, num_neighbors])
        return tiled_keypoint_types

    def assign_keypoints_offset_targets(
        self, height, width, gt_keypoints_list, gt_classes_list, gt_keypoints_weights_list=None, gt_weights_list=None
    ):
        ...
        for i, (keypoints, classes, kp_weights, weights) in enumerate(
            zip(gt_keypoints_list, gt_classes_list, gt_keypoints_weights_list, gt_weights_list)
        ):
            keypoints_absolute, kp_weights = self._preprocess_keypoints_and_weights(
            ...
            (y_source_neighbors, x_source_neighbors, valid_sources) = ta_utils.get_surrounding_grids(
                height // self._stride, width // self._stride, y_source, x_source, self._peak_radius
            )
            _, num_neighbors = shape_utils.combined_static_and_dynamic_shape(y_source_neighbors)
            ...
            offsets, indices = ta_utils.compute_floor_offsets_with_indices(
                y_source=y_source_neighbors, x_source=x_source_neighbors, y_target=y_source, x_target=x_source
            )
            # Reshape to:
            #   offsets: [num_instances * num_keypoints * num_neighbors, 2]
            #   indices: [num_instances * num_keypoints * num_neighbors, 2]
            ...
            if self._per_keypoint_offset:
                tiled_keypoint_types = self._get_keypoint_types(num_instances, num_keypoints, num_neighbors)
                batch_indices.append(
                    tf.concat([batch_index, indices, tf.reshape(tiled_keypoint_types, [-1, 1])], axis=1)
                )
            else:
                batch_indices.append(tf.concat([batch_index, indices], axis=1))
            batch_offsets.append(offsets)
    ...
    def assign_joint_regression_targets(
        ...
        for i, (keypoints, classes, boxes, kp_weights, weights) in enumerate(
            zip(gt_keypoints_list, gt_classes_list, gt_boxes_list, gt_keypoints_weights_list, gt_weights_list)
        ):
            keypoints_absolute, kp_weights = self._preprocess_keypoints_and_weights(
                out_height=height // self._stride,
                out_width=width // self._stride,
            ...
            y_center_tiled = tf.tile(tf.reshape(y_center, shape=[num_instances, 1]), multiples=[1, num_keypoints])
            x_center_tiled = tf.tile(tf.reshape(x_center, shape=[num_instances, 1]), multiples=[1, num_keypoints])
            # [num_instance * num_keypoints, num_neighbors]
            (y_source_neighbors, x_source_neighbors, valid_sources) = ta_utils.get_surrounding_grids(
                height // self._stride,
                width // self._stride,
            ...
            batch_index = tf.fill([num_instances * num_keypoints * num_neighbors, 1], i)
            batch_indices.append(tf.concat([batch_index, indices, tf.reshape(tiled_keypoint_types, [-1, 1])], axis=1))
            batch_offsets.append(offsets)
            batch_weights.append(tf.keras.backend.flatten(valid_keypoints))
...
def _resize_masks(masks, height, width, method):
    ...
    masks = tf2.image.resize(masks[:, :, :, tf.newaxis], size=(height, width), method=method)
    return masks[:, :, :, 0]
...
class CenterNetMaskTargetAssigner(object):
    ...
    def assign_segmentation_targets(self, gt_masks_list, gt_classes_list, mask_resize_method=ResizeMethod.BILINEAR):
        ...
        _, num_classes = shape_utils.combined_static_and_dynamic_shape(gt_classes_list[0])

        _, input_height, input_width = shape_utils.combined_static_and_dynamic_shape(gt_masks_list[0])
        ...
        for gt_masks, gt_classes in zip(gt_masks_list, gt_classes_list):
            gt_masks = _resize_masks(gt_masks, output_height, output_width, mask_resize_method)
            gt_masks = gt_masks[:, :, :, tf.newaxis]
            gt_classes_reshaped = tf.reshape(gt_classes, [-1, 1, 1, num_classes])
            # Shape: [h, w, num_classes].
            segmentations_for_image = tf.reduce_max(gt_masks * gt_classes_reshaped, axis=0)
...
class CenterNetDensePoseTargetAssigner(object):
    ...
    def assign_part_and_coordinate_targets(
        self, height, width, gt_dp_num_points_list, gt_dp_part_ids_list, gt_dp_surface_coords_list, gt_weights_list=None
    ):
        ...
        for i, (num_points, part_ids, surface_coords, weights) in enumerate(
            zip(gt_dp_num_points_list, gt_dp_part_ids_list, gt_dp_surface_coords_list, gt_weights_list)
        ):
            num_boxes, max_sampled_points = shape_utils.combined_static_and_dynamic_shape(part_ids)
            ...
            weights_per_point = tf.reshape(
                tf.tile(weights[:, tf.newaxis], multiples=[1, max_sampled_points]), shape=[-1]
            )
            # Mask out invalid (i.e. padded) DensePose points.
            num_points_tiled = tf.tile(num_points[:, tf.newaxis], multiples=[1, max_sampled_points])
            range_tiled = tf.tile(tf.range(max_sampled_points)[tf.newaxis, :], multiples=[num_boxes, 1])
            ...
            batch_indices.append(tf.stack([batch_index, indices_y, indices_x, part_ids_flattened], axis=1))
...
class CenterNetTrackTargetAssigner(object):
    ...
    def assign_track_targets(self, height, width, gt_track_ids_list, gt_boxes_list, gt_weights_list=None):
        ...
        """
        track_id_targets = tf.one_hot(gt_track_ids_list, depth=self._num_track_ids, axis=-1)
        ...
        for i, (boxes, weights) in enumerate(zip(gt_boxes_list, gt_weights_list)):
            ...
            #   indices: [num_boxes, 2]
            (_, indices) = ta_utils.compute_floor_offsets_with_indices(y_source=y_center, x_source=x_center)
...
def filter_mask_overlap_min_area(masks):
    ...
    def _filter_min_area():
        ...
        filtered_masks = tf.range(num_instances)[:, tf.newaxis, tf.newaxis] == min_index[tf.newaxis, :, :]
...
class CenterNetCornerOffsetTargetAssigner(object):
    ...
    def assign_corner_offset_targets(self, gt_boxes_list, gt_masks_list):
        ...
        output_height = input_height // self._stride
        output_width = input_width // self._stride
        y_grid, x_grid = tf.meshgrid(tf.range(output_height), tf.range(output_width), indexing="ij")
        ...
        for gt_masks, gt_boxes in zip(gt_masks_list, gt_boxes_list):
            gt_masks = _resize_masks(gt_masks, output_height, output_width, method=ResizeMethod.NEAREST_NEIGHBOR)
            ...
            foreground_target = tf.cast(tf.reduce_sum(gt_masks, axis=0) > 0.5, tf.float32)
...
class CenterNetTemporalOffsetTargetAssigner(object):
    ...
    def assign_temporal_offset_targets(
        self, height, width, gt_boxes_list, gt_offsets_list, gt_match_list, gt_weights_list=None
    ):
