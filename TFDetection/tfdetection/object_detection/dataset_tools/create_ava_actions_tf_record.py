...
class Ava(object):
    ...
    def generate_and_write_records(
        ...
        for key in splits_to_process.split(","):
            ...
            shard_names = [
                os.path.join(self.path_to_output_dir, FILEPATTERN % key + "-%05d-of-%05d" % (i, shards))
        ...
        with open(annotation_file, "r") as annotations:
            ...
            for media_id in ids:
                ...
                while middle_frame_time < POSSIBLE_TIMESTAMPS[-1]:
                    start_time = (
                        middle_frame_time - seconds_per_sequence // 2 - (0 if seconds_per_sequence % 2 == 0 else 1)
                    )
                    ...
                    while windowed_timestamp < end_time:
                        ...
                        cur_vid.set(cv2.CAP_PROP_POS_MSEC, (windowed_timestamp) * SECONDS_TO_MILLI)
    ...
    def _generate_examples(
        ...
        with open(annotation_file, "r") as annotations:
            ...
            for media_id in ids:
                ...
                cur_frame_num = 0
                while middle_frame_time < POSSIBLE_TIMESTAMPS[-1]:
                    cur_vid.set(cv2.CAP_PROP_POS_MSEC, middle_frame_time * SECONDS_TO_MILLI)
                    _, image = cur_vid.read()
                    _, buffer = cv2.imencode(".jpg", image)
                    ...
                    for row in frame_annotations[(media_id, middle_frame_time)]:
                        if len(row) > 2 and int(row["action_label"]) in label_map:
                            ...
                            areas.append(float((xmaxs[-1] - xmins[-1]) * (ymaxs[-1] - ymins[-1])) / 2)
                            labels.append(int(row["action_label"]))
                            label_strings.append(label_map[int(row["action_label"])])
                            confidences.append(1)
                        else:
                    ...
                    context_feature_dict = {
                        ...
                        "image/filename": dataset_util.bytes_feature(source_id.encode("utf8")),
                        ...
                        "image/object/class/text": dataset_util.bytes_list_feature(label_strings),
                        "image/location": dataset_util.bytes_feature(media_id.encode("utf8")),
                        "image/date_captured": dataset_util.bytes_feature(date_captured_feature.encode("utf8")),
                        "image/seq_num_frames": dataset_util.int64_feature(total_non_excluded),
                        "image/seq_frame_num": dataset_util.int64_feature(cur_frame_num),
                        "image/seq_id": dataset_util.bytes_feature(media_id.encode("utf8")),
                    }

                    yield tf.train.Example(features=tf.train.Features(feature=context_feature_dict))
    ...
    def _download_data(self):
        ...
        label_map = self.get_label_map(
            os.path.join(self.path_to_data_download, "ava_action_list_v2.2_for_activitynet_2019.pbtxt")
        )
...
def main(argv):
    ...
    Ava(flags.FLAGS.path_to_output_dir, flags.FLAGS.path_to_download_data).generate_and_write_records(
...
if __name__ == "__main__":
    flags.DEFINE_string("path_to_download_data", "", "Path to directory to download data to.")
    flags.DEFINE_string("path_to_output_dir", "", "Path to directory to write data to.")
    flags.DEFINE_string("splits_to_process", "train,val", "Process these splits. Useful for custom data splits.")
    flags.DEFINE_string(
        "video_path_format_string",
        None,
    ...
    flags.DEFINE_integer(
        "seconds_per_sequence",
        10,
        "The number of seconds per example in each example.Always 1 when examples_for_context is True.",
    )
    flags.DEFINE_integer(
        "hop_between_sequences",
        10,
