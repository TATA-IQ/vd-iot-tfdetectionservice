...
from object_detection.builders import model_builder, post_processing_builder
...
class SSDModule(tf.Module):
    ...
    def __init__(self, pipeline_config, detection_model, max_detections, use_regular_nms):
        """Initialization.
    ...
    def _process_config(self, pipeline_config):
        ...
        self._scale_values = {}
        self._scale_values["y_scale"] = pipeline_config.model.ssd.box_coder.faster_rcnn_box_coder.y_scale
        self._scale_values["x_scale"] = pipeline_config.model.ssd.box_coder.faster_rcnn_box_coder.x_scale
        self._scale_values["h_scale"] = pipeline_config.model.ssd.box_coder.faster_rcnn_box_coder.height_scale
        self._scale_values["w_scale"] = pipeline_config.model.ssd.box_coder.faster_rcnn_box_coder.width_scale
        ...
        if image_resizer == "fixed_shape_resizer":
            ...
            raise ValueError(
                "Only fixed_shape_resizeris supported with tflite. Found {}".format(
    ...
    def postprocess_implements_signature(self):
        ...
        implements_signature = [
            ...
            'attr { key: "max_classes_per_detection" value { i: %d } }' % _MAX_CLASSES_PER_DETECTION,
            'attr { key: "use_regular_nms" value { b: %s } }' % str(self._use_regular_nms).lower(),
            'attr { key: "nms_score_threshold" value { f: %f } }' % self._nms_score_threshold,
            'attr { key: "nms_iou_threshold" value { f: %f } }' % self._nms_iou_threshold,
    ...
    def inference_fn(self, image):
        ...
        _, score_conversion_fn = post_processing_builder.build(self._pipeline_config.model.ssd.post_processing)
        ...
        with tf.name_scope("raw_outputs"):
            ...
            box_encodings = tf.identity(predicted_tensors["box_encodings"], name="box_encodings")
        ...
        num_anchors, anchors = get_const_center_size_encoded_anchors(predicted_tensors["anchors"])
        anchors = tf.identity(anchors, name="anchors")

        # tf.function@ seems to reverse order of inputs, so reverse them here.
        return self._get_postprocess_fn(num_anchors, self._num_classes)(box_encodings, class_predictions, anchors)[::-1]
...
def export_tflite_model(pipeline_config, trained_checkpoint_dir, output_directory, max_detections, use_regular_nms):
    ...
    detection_module = SSDModule(pipeline_config, detection_model, max_detections, use_regular_nms)
    ...
    concrete_function = detection_module.inference_fn.get_concrete_function(
        tf.TensorSpec(shape=detection_module.input_shape(), dtype=tf.float32, name="input")
    )
    status.assert_existing_objects_matched()
