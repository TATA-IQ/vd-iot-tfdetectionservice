...
if tf_version.is_tf1():
    from tensorflow.tools.graph_transforms import \
        TransformGraph  # pylint: disable=g-import-not-at-top
...
def export_tflite_graph(
    ...
    if pipeline_config.eval_config.use_moving_averages:
        ...
        exporter.replace_variable_values_with_moving_averages(
            tf.get_default_graph(), trained_checkpoint_prefix, moving_average_checkpoint.name
        )
    ...
    frozen_graph_def = exporter.freeze_graph_with_def_protos(
        ...
        output_node_names=",".join(
            ["raw_outputs/box_encodings", "raw_outputs/class_predictions", "anchors"] + list(additional_output_tensors)
        ),
