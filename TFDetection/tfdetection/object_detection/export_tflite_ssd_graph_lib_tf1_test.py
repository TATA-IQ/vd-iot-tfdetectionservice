...
class FakeModel(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        with tf.control_dependencies([features]):
            prediction_tensors = {
                "box_encodings": tf.constant([[[0.0, 0.0, 0.5, 0.5], [0.5, 0.5, 0.8, 0.8]]], tf.float32),
                "class_predictions_with_background": tf.constant([[[0.7, 0.6], [0.9, 0.0]]], tf.float32),
            }
        ...
        with graph.as_default():
            ...
            for node in graph_def.node:
                if node.name == "TFLite_Detection_PostProcess":
                    ...
                    self.assertTrue(node.attr["_support_output_type_float_in_quantized_op"].b)
