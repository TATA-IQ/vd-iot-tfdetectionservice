...
def _multiclass_scores_or_one_hot_labels(multiclass_scores, groundtruth_boxes, groundtruth_classes, num_classes):
    ...
    # tensor.
    def true_fn():
        return tf.reshape(multiclass_scores, [tf.shape(groundtruth_boxes)[0], num_classes])
...
def transform_input_data(
    ...
    if labeled_classes_field in out_tensor_dict and image_classes_field in out_tensor_dict:
        raise KeyError(
    ...
    if fields.InputDataFields.multiclass_scores in out_tensor_dict:
        out_tensor_dict[fields.InputDataFields.multiclass_scores] = _multiclass_scores_or_one_hot_labels(
    ...
    if fields.InputDataFields.groundtruth_boxes in tensor_dict:
        ...
        out_tensor_dict[fields.InputDataFields.groundtruth_boxes] = valid_boxes_tensor
    ...
    if fields.InputDataFields.groundtruth_keypoints in tensor_dict:
        ...
        if flds_gt_kpt_vis not in out_tensor_dict:
            out_tensor_dict[flds_gt_kpt_vis] = tf.ones_like(out_tensor_dict[flds_gt_kpt][:, :, 0], dtype=tf.bool)
        out_tensor_dict[flds_gt_kpt_weights] = keypoint_ops.keypoint_weights_from_visibilities(
    ...
    if dp_surface_coords_fld in tensor_dict:
        ...
        realigned_dp_surface_coords = densepose_ops.change_coordinate_frame(dp_surface_coords, im_box)
    ...
    out_tensor_dict[fields.InputDataFields.image] = tf.squeeze(preprocessed_resized_image, axis=0)
    out_tensor_dict[fields.InputDataFields.true_image_shape] = tf.squeeze(true_image_shape, axis=0)
    ...
    zero_indexed_groundtruth_classes = out_tensor_dict[fields.InputDataFields.groundtruth_classes] - _LABEL_OFFSET
    if use_multiclass_scores:
        out_tensor_dict[fields.InputDataFields.groundtruth_classes] = out_tensor_dict[
    ...
    if fields.InputDataFields.groundtruth_confidences in out_tensor_dict:
        ...
        out_tensor_dict[fields.InputDataFields.groundtruth_confidences] = (
            tf.reshape(groundtruth_confidences, [-1, 1]) * out_tensor_dict[fields.InputDataFields.groundtruth_classes]
        )
    else:
        groundtruth_confidences = tf.ones_like(zero_indexed_groundtruth_classes, dtype=tf.float32)
        out_tensor_dict[fields.InputDataFields.groundtruth_confidences] = out_tensor_dict[
            fields.InputDataFields.groundtruth_classes
        ]

    if merge_multiple_boxes:
        merged_boxes, merged_classes, merged_confidences, _ = util_ops.merge_boxes_with_multiple_labels(
            out_tensor_dict[fields.InputDataFields.groundtruth_boxes],
...
def pad_input_data_to_static_shapes(
    ...
    if fields.InputDataFields.image_additional_channels in tensor_dict:
        num_additional_channels = shape_utils.get_dim_as_int(
            tensor_dict[fields.InputDataFields.image_additional_channels].shape[2]
        )
    ...
    if num_additional_channels:
        ...
        if (
            fields.InputDataFields.original_image in tensor_dict
            and shape_utils.get_dim_as_int(tensor_dict[fields.InputDataFields.original_image].shape[2]) == num_channels
        ):
            raise ValueError("Image must be already concatenated with additional channels.")

    if fields.InputDataFields.context_features in tensor_dict and (max_num_context_features is None):
        raise ValueError(
    ...
    padding_shapes = {
        ...
        fields.InputDataFields.image_additional_channels: [height, width, num_additional_channels],
    ...
    if fields.InputDataFields.groundtruth_keypoint_weights in tensor_dict:
        tensor_shape = tensor_dict[fields.InputDataFields.groundtruth_keypoint_weights].shape
        padding_shape = [max_num_boxes, shape_utils.get_dim_as_int(tensor_shape[1])]
    ...
    if fields.InputDataFields.groundtruth_dp_num_points in tensor_dict:
        padding_shapes[fields.InputDataFields.groundtruth_dp_num_points] = [max_num_boxes]
        padding_shapes[fields.InputDataFields.groundtruth_dp_part_ids] = [max_num_boxes, max_dp_points]
        padding_shapes[fields.InputDataFields.groundtruth_dp_surface_coords] = [max_num_boxes, max_dp_points, 4]
    if fields.InputDataFields.groundtruth_track_ids in tensor_dict:
        padding_shapes[fields.InputDataFields.groundtruth_track_ids] = [max_num_boxes]
    ...
    if fields.InputDataFields.num_groundtruth_boxes in padded_tensor_dict:
        padded_tensor_dict[fields.InputDataFields.num_groundtruth_boxes] = tf.minimum(
            padded_tensor_dict[fields.InputDataFields.num_groundtruth_boxes], max_num_boxes
        )
...
def augment_input_data(tensor_dict, data_augmentation_options):
    ...
    include_keypoint_visibilities = fields.InputDataFields.groundtruth_keypoint_visibilities in tensor_dict
    ...
    tensor_dict[fields.InputDataFields.image] = tf.squeeze(tensor_dict[fields.InputDataFields.image], axis=0)
    return tensor_dict
...
def _replace_empty_string_with_random_number(string_tensor):
    ...
    random_source_id = tf.as_string(tf.random_uniform(shape=[], maxval=2**63 - 1, dtype=tf.int64))

    out_string = tf.cond(
        tf.equal(string_tensor, empty_string), true_fn=lambda: random_source_id, false_fn=lambda: string_tensor
    )
...
def _get_features_dict(input_dict, include_source_id=False):
    ...
    if fields.InputDataFields.context_features in input_dict:
        features[fields.InputDataFields.context_features] = input_dict[fields.InputDataFields.context_features]
    if fields.InputDataFields.valid_context_size in input_dict:
        features[fields.InputDataFields.valid_context_size] = input_dict[fields.InputDataFields.valid_context_size]
    return features
...
def train_input(train_config, train_input_config, model_config, model=None, params=None, input_context=None):
    ...
    def transform_and_pad_input_data_fn(tensor_dict):
        ...
        data_augmentation_options = [
            preprocessor_builder.build(step) for step in train_config.data_augmentation_options
        ]
...
def eval_input(eval_config, eval_input_config, model_config, model=None, params=None):
    ...
    if model is None:
        model_preprocess_fn = INPUT_BUILDER_UTIL_MAP["model_build"](model_config, is_training=False).preprocess
    else:
        model_preprocess_fn = model.preprocess
...
def create_predict_input_fn(model_config, predict_input_config):
    ...
    def _predict_input_fn(params=None):
        ...
        decoder = tf_example_decoder.TfExampleDecoder(
            load_instance_masks=False, num_additional_channels=predict_input_config.num_additional_channels
        )
        input_dict = transform_fn(decoder.decode(example))
        images = tf.cast(input_dict[fields.InputDataFields.image], dtype=tf.float32)
        images = tf.expand_dims(images, axis=0)
        true_image_shape = tf.expand_dims(input_dict[fields.InputDataFields.true_image_shape], axis=0)

        return tf.estimator.export.ServingInputReceiver(
            features={fields.InputDataFields.image: images, fields.InputDataFields.true_image_shape: true_image_shape},
...
def get_reduce_to_frame_fn(input_reader_config, is_training):
    ...
    """
    if input_reader_config.input_type != (input_reader_pb2.InputType.Value("TF_SEQUENCE_EXAMPLE")):
        ...
        def reduce_to_frame(dataset, dataset_map_fn, batch_size, input_reader_config):
            ...
            if is_training:
                ...
                def get_single_frame(tensor_dict):
                    ...
                    """
                    num_frames = tf.cast(tf.shape(tensor_dict[fields.InputDataFields.source_id])[0], dtype=tf.int32)
                    if input_reader_config.frame_index == -1:
                        frame_index = tf.random.uniform((), minval=0, maxval=num_frames, dtype=tf.int32)
                    else:
                        frame_index = tf.constant(input_reader_config.frame_index, dtype=tf.int32)
                    out_tensor_dict = {}
                ...
                dataset = dataset_map_fn(dataset, get_single_frame, batch_size, input_reader_config)
            else:
                dataset = dataset_map_fn(dataset, util_ops.tile_context_tensors, batch_size, input_reader_config)
                dataset = dataset.unbatch()
            # Decode frame here as SequenceExample tensors contain encoded images.
            dataset = dataset_map_fn(dataset, util_ops.decode_image, batch_size, input_reader_config)
            return dataset
