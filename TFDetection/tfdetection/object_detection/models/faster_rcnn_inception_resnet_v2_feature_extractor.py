...
class FasterRCNNInceptionResnetV2FeatureExtractor(faster_rcnn_meta_arch.FasterRCNNFeatureExtractor):
    ...
    def _extract_proposal_features(self, preprocessed_inputs, scope):
        ...
        if len(preprocessed_inputs.get_shape().as_list()) != 4:
            raise ValueError(
                "`preprocessed_inputs` must be 4 dimensional, got a tensor of shape %s"
                        ...
                        with tf.variable_scope("Mixed_7a"):
                            ...
                            with tf.variable_scope("Branch_1"):
                                ...
                                tower_conv1_1 = slim.conv2d(
                                    tower_conv1, 288, 3, stride=2, padding="VALID", scope="Conv2d_1a_3x3"
                                )
                            with tf.variable_scope("Branch_2"):
                                ...
                                tower_conv2_2 = slim.conv2d(
                                    tower_conv2_1, 320, 3, stride=2, padding="VALID", scope="Conv2d_1a_3x3"
                                )
                            with tf.variable_scope("Branch_3"):
                                tower_pool = slim.max_pool2d(
                                    proposal_feature_maps, 3, stride=2, padding="VALID", scope="MaxPool_1a_3x3"
                                )
        ...
        for variable in variables_helper.get_global_variables_safely():
            ...
            if variable.op.name.startswith(second_stage_feature_extractor_scope):
                var_name = variable.op.name.replace(
                    second_stage_feature_extractor_scope + "/InceptionResnetV2/Repeat", "InceptionResnetV2/Repeat_2"
                )
