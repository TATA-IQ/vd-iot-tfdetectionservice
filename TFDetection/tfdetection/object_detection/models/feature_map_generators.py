...
def create_conv_block(
    use_depthwise, kernel_size, padding, stride, layer_name, conv_hyperparams, is_training, freeze_batchnorm, depth
):
    ...
    layers.append(
        conv_hyperparams.build_batch_norm(
            training=(is_training and not freeze_batchnorm), name=layer_name + "_batchnorm"
        )
    )
        ...
        for index, from_layer in enumerate(feature_map_layout["from_layer"]):
            ...
            if from_layer:
                ...
                layer_name = "{}_2_Conv2d_{}_{}x{}_s2_{}".format(
                    base_from_layer, index, conv_kernel_size, conv_kernel_size, depth_fn(layer_depth)
                )
                stride = 2
                padding = "SAME"
                ...
                if use_depthwise:
                    ...
                    net.append(
                        conv_hyperparams.build_batch_norm(
                            training=(is_training and not freeze_batchnorm), name=layer_name + "_depthwise_batchnorm"
                        )
                    )
...
def multi_resolution_feature_maps(
    feature_map_layout, depth_multiplier, min_depth, insert_1x1_conv, image_features, pool_residual=False
):
    """Generates multi resolution feature maps from input image features.
        ...
        if from_layer:
            ...
            if insert_1x1_conv:
                ...
                intermediate_layer = slim.conv2d(
                    pre_layer, depth_fn(layer_depth // 2), [1, 1], padding="SAME", stride=1, scope=layer_name
                )
            layer_name = "{}_2_Conv2d_{}_{}x{}_s2_{}".format(
                base_from_layer, index, conv_kernel_size, conv_kernel_size, depth_fn(layer_depth)
            )
            ...
            if use_depthwise:
                ...
                feature_map = slim.conv2d(
                    feature_map, depth_fn(layer_depth), [1, 1], padding="SAME", stride=1, scope=layer_name
                )
                if pool_residual and pre_layer_depth == depth_fn(layer_depth):
                    feature_map += slim.avg_pool2d(
                        pre_layer, [3, 3], padding="SAME", stride=2, scope=layer_name + "_pool"
                    )
            else:
                feature_map = slim.conv2d(
                    intermediate_layer,
    ...
    def __init__(
        ...
        for level in reversed(list(range(num_levels - 1))):
            ...
            if use_native_resize_op:
                ...
                top_down_net.append(
                    tf.keras.layers.Lambda(nearest_neighbor_upsampling, name="nearest_neighbor_upsampling")
                )
        ...
        with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding=padding, stride=1):
            ...
            for level in reversed(list(range(num_levels - 1))):
                ...
                output_feature_maps_list.append(
                    conv_op(pre_output, depth, [kernel_size, kernel_size], scope="smoothing_%d" % (level + 1))
                )
                output_feature_map_keys.append("top_down_%s" % image_features[level][0])
            return collections.OrderedDict(reversed(list(zip(output_feature_map_keys, output_feature_maps_list))))
