...
def _obtain_input_shape(input_shape, default_size, min_size, data_format, require_flatten):
    ...
    if input_shape and len(input_shape) == 3:
        if data_format == "channels_first":
            if input_shape[0] not in {1, 3}:
                warnings.warn(
                    "This model usually expects 1 or 3 input channels. However, it was passed an input_shape with "
                    + str(input_shape[0])
                    + " input channels."
                )
    ...
    if require_flatten:
        if None in input_shape:
            raise ValueError(
                "If `include_top` is True, you should specify a static `input_shape`. Got `input_shape="
                + str(input_shape)
                + "`"
            )
    return input_shape
...
def mobilenet_v2(input_shape=None, alpha=1.0, include_top=True, classes=1000):
    ...
    x = Conv2D(first_block_filters, kernel_size=3, strides=(2, 2), padding="same", use_bias=False, name="Conv1")(
        img_input
    )
...
def _first_inverted_res_block(inputs, stride, alpha, filters, block_id):
    ...
    x = BatchNormalization(epsilon=1e-3, momentum=0.999, name="bn%d_conv_project" % block_id)(x)
