...
class _LayersOverride(object):
    ...
    def Conv2D(self, filters, kernel_size, **kwargs):
        ...
        if self._use_explicit_padding and kernel_size[0] > 1:
            ...
            def padded_conv(features):  # pylint: disable=invalid-name
                ...
                return tf.keras.layers.Conv2D(filters, kernel_size, **kwargs)(padded_features)
    ...
    def DepthwiseConv2D(self, kernel_size, **kwargs):
        ...
        if self._use_explicit_padding:
            ...
            def padded_depthwise_conv(features):  # pylint: disable=invalid-name
                padded_features = self._FixedPaddingLayer(kernel_size)(features)
                return tf.keras.layers.DepthwiseConv2D(kernel_size, **kwargs)(padded_features)
    ...
    def BatchNormalization(self, **kwargs):
        ...
        if self._conv_hyperparams:
            return self._conv_hyperparams.build_batch_norm(training=self._batchnorm_training, name=name)
        else:
            return freezable_batch_norm.FreezableBatchNorm(
                training=self._batchnorm_training, epsilon=1e-3, momentum=self._default_batchnorm_momentum, name=name
            )
    ...
    def Input(self, shape):
        ...
        placeholder_with_default = tf.placeholder_with_default(input=input_tensor, shape=[None] + shape)
...
def mobilenet_v1(
    ...
    **kwargs,
    ...
    return tf.keras.applications.MobileNet(alpha=alpha, layers=layers_override, **kwargs)
