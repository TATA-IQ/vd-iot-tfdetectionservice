...
class MaskRCNNClassHead(head.Head):
    ...
    def __init__(
        self, is_training, num_class_slots, fc_hyperparams_fn, use_dropout, dropout_keep_prob, scope="ClassPredictor"
    ):
        """Constructor.
    ...
    def predict(self, features, num_predictions_per_location=1):
        ...
        if self._use_dropout:
            flattened_roi_pooled_features = slim.dropout(
                flattened_roi_pooled_features, keep_prob=self._dropout_keep_prob, is_training=self._is_training
            )
