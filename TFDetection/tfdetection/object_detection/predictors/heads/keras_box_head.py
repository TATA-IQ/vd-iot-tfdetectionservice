        ...
        if self._use_depthwise:
            ...
            self._box_encoder_layers.append(
                conv_hyperparams.build_batch_norm(
                    training=(is_training and not freeze_batchnorm), name="BoxEncodingPredictor_depthwise_batchnorm"
                )
            )
            self._box_encoder_layers.append(
                conv_hyperparams.build_activation_layer(name="BoxEncodingPredictor_depthwise_activation")
            )
            self._box_encoder_layers.append(
    ...
    def _predict(self, features):
        ...
        if self._box_encodings_clip_range is not None:
            box_encodings = tf.clip_by_value(
                box_encodings, self._box_encodings_clip_range.min, self._box_encodings_clip_range.max
            )
...
class MaskRCNNBoxHead(head.KerasHead):
    ...
    def __init__(
        ...
        self._box_encoder_layers.append(
            tf.keras.layers.Dense(self._number_of_boxes * self._box_code_size, name="BoxEncodingPredictor_dense")
        )
        self._box_encoder_layers.append(
            fc_hyperparams.build_batch_norm(
                training=(is_training and not freeze_batchnorm), name="BoxEncodingPredictor_batchnorm"
            )
        )
