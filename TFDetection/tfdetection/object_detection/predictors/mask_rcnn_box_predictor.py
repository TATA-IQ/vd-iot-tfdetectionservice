...
class MaskRCNNBoxPredictor(box_predictor.BoxPredictor):
    ...
    def _predict(self, image_features, num_predictions_per_location, prediction_stage=2):
        ...
        if len(num_predictions_per_location) != 1 or num_predictions_per_location[0] != 1:
            raise ValueError(
                "Currently FullyConnectedBoxPredictor only supports predicting a single box per class per location."
            )
        ...
        if prediction_stage == 2:
            predictions_dict[BOX_ENCODINGS] = self._box_prediction_head.predict(
                features=image_feature, num_predictions_per_location=num_predictions_per_location[0]
            )
            predictions_dict[CLASS_PREDICTIONS_WITH_BACKGROUND] = self._class_prediction_head.predict(
                features=image_feature, num_predictions_per_location=num_predictions_per_location[0]
            )
