...
from object_detection.tpu_exporters import faster_rcnn, ssd
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        tf.logging.info(
            "Inputs:\n{}\nOutputs:{}\nPredict method name:{}".format(
                tensor_info_inputs, tensor_info_outputs, tf.saved_model.signature_constants.PREDICT_METHOD_NAME
            )
        )
        # Graph for TPU.
...
def run_inference(
    inputs, pipeline_config_file, ckpt_path, input_type="encoded_image_string_tensor", use_bfloat16=False, repeat=1
):
