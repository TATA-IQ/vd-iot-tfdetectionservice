...
def pad_to_multiple(tensor, multiple):
    ...
    if tensor_height is None:
        ...
        padded_tensor_height = (
            tf.cast(
                tf.ceil(tf.cast(tensor_height, dtype=tf.float32) / tf.cast(multiple, dtype=tf.float32)), dtype=tf.int32
            )
            * multiple
        )
    else:
    ...
    if tensor_width is None:
        ...
        padded_tensor_width = (
            tf.cast(
                tf.ceil(tf.cast(tensor_width, dtype=tf.float32) / tf.cast(multiple, dtype=tf.float32)), dtype=tf.int32
            )
            * multiple
        )
    else:
    ...
    if padded_tensor_height != tensor_height:
        height_pad = tf.zeros(
            [batch_size, padded_tensor_height - tensor_height, tensor_width, tensor_depth], dtype=tensor.dtype
        )
        tensor = tf.concat([tensor, height_pad], 1)
    if padded_tensor_width != tensor_width:
        width_pad = tf.zeros(
            [batch_size, padded_tensor_height, padded_tensor_width - tensor_width, tensor_depth], dtype=tensor.dtype
        )
...
def padded_one_hot_encoding(indices, depth, left_pad):
    ...
    result = tf.cond(
        tf.greater(tf.size(indices), 0), one_hot_and_pad, lambda: tf.zeros((tf.size(indices), depth + left_pad))
    )
...
def batch_position_sensitive_crop_regions(
    ...
    def _position_sensitive_crop_fn(inputs):
        ...
        return position_sensitive_crop_regions(
            images, boxes, crop_size=crop_size, num_spatial_bins=num_spatial_bins, global_pool=global_pool
        )

    return shape_utils.static_or_dynamic_map_fn(
        _position_sensitive_crop_fn, elems=[images, boxes], dtype=tf.float32, parallel_iterations=parallel_iterations
    )
...
def position_sensitive_crop_regions(image, boxes, crop_size, num_spatial_bins, global_pool):
    ...
    for split, box in zip(image_splits, position_sensitive_boxes):
        if split.shape.is_fully_defined() and box.shape.is_fully_defined():
            crop = tf.squeeze(
                matmul_crop_and_resize(tf.expand_dims(split, axis=0), tf.expand_dims(box, axis=0), bin_crop_size),
                axis=0,
            )
        else:
            crop = tf.image.crop_and_resize(
                tf.expand_dims(split, 0), box, tf.zeros(tf.shape(boxes)[0], dtype=tf.int32), bin_crop_size
            )
    ...
    if global_pool:
        ...
        position_sensitive_features = tf.squeeze(
            tf.batch_to_space_nd(
                position_sensitive_features, block_shape=[1] + num_spatial_bins, crops=tf.zeros((3, 2), dtype=tf.int32)
            ),
            axis=[0],
        )
...
def merge_boxes_with_multiple_labels(boxes, classes, confidences, num_classes, quantization_bins=10000):
    ...
    with tf.control_dependencies(
        ...
        class_encodings, confidence_encodings = tf.map_fn(
            map_box_encodings, tf.range(tf.to_int64(num_unique_boxes)), back_prop=False, dtype=(tf.int64, tf.float32)
        )
...
def fpn_feature_levels(num_levels, unit_scale_index, image_ratio, boxes):
    ...
    levels = tf.cast(tf.floordiv(tf.log(areas_sqrt * image_ratio), log_2) + unit_scale_index, dtype=tf.int32)
...
EqualizationLossConfig = collections.namedtuple("EqualizationLossConfig", ["weight", "exclude_prefixes"])
    ...
    for key in tensor_dict:
        if key not in fields.SEQUENCE_FIELDS:
            ...
            tensor_dict[key] = tf.tile(
                tf.expand_dims(original_tensor, 0), tf.stack([num_frames] + [1] * len(tensor_shape), axis=0)
            )
    return tensor_dict
