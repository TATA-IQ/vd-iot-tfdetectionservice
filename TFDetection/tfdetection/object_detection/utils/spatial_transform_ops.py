...
def ravel_indices(feature_grid_y, feature_grid_x, num_levels, height, width, box_levels):
    ...
    batch_dim_indices = tf.reshape(tf.range(batch_size) * batch_dim_offset, [batch_size, 1, 1, 1]) * tf.ones(
        [1, num_boxes, size_y, size_x], dtype=tf.int32
    )
    box_level_indices = tf.reshape(box_levels * level_dim_offset, [batch_size, num_boxes, 1, 1]) * tf.ones(
        [1, 1, size_y, size_x], dtype=tf.int32
    )
    height_indices = tf.reshape(feature_grid_y * height_dim_offset, [batch_size, num_boxes, size_y, 1]) * tf.ones(
        [1, 1, 1, size_x], dtype=tf.int32
    )
    width_indices = tf.reshape(feature_grid_x, [batch_size, num_boxes, 1, size_x]) * tf.ones(
        [1, 1, size_y, 1], dtype=tf.int32
    )
...
def multilevel_roi_align(
    ...
    with tf.name_scope(scope, "MultiLevelRoIAlign"):
        ...
        (feature_grid_y0, feature_grid_x0, feature_grid_y1, feature_grid_x1) = feature_grid_coordinate_vectors(
            box_grid_y, box_grid_x
        )
        feature_grid_y = tf.reshape(tf.stack([feature_grid_y0, feature_grid_y1], axis=3), [batch_size, num_boxes, -1])
        feature_grid_x = tf.reshape(tf.stack([feature_grid_x0, feature_grid_x1], axis=3), [batch_size, num_boxes, -1])
        feature_coordinates = ravel_indices(
            feature_grid_y, feature_grid_x, num_levels, max_feature_height, max_feature_width, box_levels
        )
        ...
        features_per_box = tf.reshape(features_per_box, [batch_size * num_boxes, size_y * 2, size_x * 2, num_filters])
...
def multilevel_matmul_crop_and_resize(images, boxes, box_levels, crop_size, extrapolation_value=0.0, scope=None):
    ...
    with tf.name_scope(scope, "MultiLevelMatMulCropAndResize"):
        ...
        return multilevel_roi_align(
            images, boxes, box_levels, crop_size, align_corners=True, extrapolation_value=extrapolation_value
        )
...
def matmul_crop_and_resize(image, boxes, crop_size, extrapolation_value=0.0, scope=None):
    ...
    with tf.name_scope(scope, "MatMulCropAndResize"):
        box_levels = tf.zeros(tf.shape(boxes)[:2], dtype=tf.int32)
        return multilevel_roi_align(
            [image], boxes, box_levels, crop_size, align_corners=True, extrapolation_value=extrapolation_value
        )
