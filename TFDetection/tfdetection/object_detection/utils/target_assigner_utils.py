...
def coordinates_to_heatmap(y_grid, x_grid, y_coordinates, x_coordinates, sigma, channel_onehot, channel_weights=None):
...
def compute_floor_offsets_with_indices(y_source, x_source, y_target=None, x_target=None):
    ...
    if y_target is None and x_target is None:
        ...
        if len(source_shape) == 2 and len(target_shape) == 1:
            ...
            y_target = tf.tile(tf.expand_dims(y_target, -1), multiples=[1, num_neighbors])
            x_target = tf.tile(tf.expand_dims(x_target, -1), multiples=[1, num_neighbors])
...
def get_valid_keypoint_mask_for_class(
    ...
    keypoints_nan_to_zeros = tf.where(not_nan, keypoint_coordinates, tf.zeros_like(keypoint_coordinates))
    if class_weights is not None:
        reshaped_class_weight = tf.tile(tf.expand_dims(class_weights, axis=-1), multiples=[1, num_keypoints])
...
def blackout_pixel_weights_by_box_regions(height, width, boxes, blackout):
    ...
    in_boxes = tf.cast(
        tf.logical_and(
            tf.logical_and(y_grid >= y_min, y_grid <= y_max), tf.logical_and(x_grid >= x_min, x_grid <= x_max)
        ),
        dtype=tf.float32,
    )

    # Shape: [num_instances, height, width]
    blackout = tf.tile(tf.expand_dims(tf.expand_dims(blackout, axis=-1), axis=-1), [1, height, width])
...
def _get_yx_indices_offset_by_radius(radius):
    ...
    return (tf.constant(y_offsets, dtype=tf.float32), tf.constant(x_offsets, dtype=tf.float32))
...
def get_surrounding_grids(height, width, y_coordinates, x_coordinates, radius):
    ...
    valid = tf.logical_and(
        tf.logical_and(x_output >= 0, x_output < width), tf.logical_and(y_output >= 0, y_output < height)
    )
