...
class MockMaskHead(object):
    ...
    def predict(self, features):
        ...
        return tf.zeros((batch_size, 1, self._num_classes, DEFAULT_MASK_SIZE, DEFAULT_MASK_SIZE), dtype=tf.float32)
...
def first_rows_close_as_set(a, b, k=None, rtol=1e-6, atol=1e-6):
    ...
    a_sorted = sorted(a[:k])
    b_sorted = sorted(b[:k])
    return all([np.allclose(entry_a, entry_b, rtol, atol) for (entry_a, entry_b) in zip(a_sorted, b_sorted)])
...
def keypoints_with_dynamic_shape(num_instances, num_keypoints, num_coordinates):
    ...
    n = tf.random.uniform([], minval=num_instances, maxval=num_instances + 1, dtype=tf.int32)
