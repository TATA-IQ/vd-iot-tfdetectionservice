...
sf.downloadtf(model_path,"model/test.zip")
...
modelmaster=requests.get(apis["model_master_config"],json={"model_id":model_id})
gpu=False
if modelmaster["data"][0]["device"]=="gpu":
    gpu=True
im=InferenceModel("model/temp/"+model_nm+"/saved_model",gpu)
files=glob2.glob("model/temp/"+model_nm"/*.pbtxt")
