...
class MultiscaleGridAnchorGeneratorTest(test_case.TestCase):
    def test_construct_single_anchor(self):
        def graph_fn():
            ...
            anchor_generator = mg.MultiscaleGridAnchorGenerator(
                min_level, max_level, anchor_scale, aspect_ratios, scales_per_octave, normalize_coordinates=False
            )
    ...
    def test_construct_single_anchor_in_normalized_coordinates(self):
        def graph_fn():
            ...
            feature_map_shape_list = [(2, 2)]
            anchor_generator = mg.MultiscaleGridAnchorGenerator(
                min_level, max_level, anchor_scale, aspect_ratios, scales_per_octave, normalize_coordinates=True
            )
            anchors_list = anchor_generator.generate(feature_map_shape_list, im_height=im_height, im_width=im_width)
            anchor_corners = anchors_list[0].get()
            return anchor_corners
    ...
    def test_construct_single_anchor_dynamic_size(self):
        ...
        exp_anchor_corners = [[b + anchor_offset for b in a] for a in exp_anchor_corners]
        anchor_corners_out = self.execute(graph_fn, [])
        self.assertAllClose(anchor_corners_out, exp_anchor_corners)
