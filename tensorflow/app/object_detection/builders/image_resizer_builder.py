...
def build(image_resizer_config):
    ...
    if image_resizer_oneof == "keep_aspect_ratio_resizer":
        ...
        if conditional_shape_resize_config.condition == (image_resizer_pb2.ConditionalShapeResizer.GREATER):
            ...
            raise ValueError(
                "Invalid image resizer condition option for ConditionalShapeResizer: '%s'."
        ...
        if pad_to_multiple_resizer_config.multiple < 0:
            ...
            image_resizer_fn = functools.partial(
                preprocessor.resize_pad_to_multiple, multiple=pad_to_multiple_resizer_config.multiple
            )
