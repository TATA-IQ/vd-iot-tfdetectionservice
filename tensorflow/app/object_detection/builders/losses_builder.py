...
def _build_classification_loss(loss_config):
    ...
    if loss_type == "bootstrapped_sigmoid":
        ...
        return losses.BootstrappedSigmoidClassificationLoss(
            alpha=config.alpha, bootstrap_type=("hard" if config.hard_bootstrap else "soft")
        )

    if loss_type == "penalty_reduced_logistic_focal_loss":
        config = loss_config.penalty_reduced_logistic_focal_loss
        return losses.PenaltyReducedLogisticFocalLoss(alpha=config.alpha, beta=config.beta)
