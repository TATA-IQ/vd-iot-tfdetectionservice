...
from object_detection.protos import losses_pb2, model_pb2
...
if tf_version.is_tf2():
    ...
    from object_detection.models import \
        faster_rcnn_inception_resnet_v2_keras_feature_extractor as \
        frcnn_inc_res_keras
    from object_detection.models import \
...
if tf_version.is_tf1():
    ...
    from object_detection.models.ssd_mobiledet_feature_extractor import (
        SSDMobileDetCPUFeatureExtractor, SSDMobileDetDSPFeatureExtractor,
        SSDMobileDetEdgeTPUFeatureExtractor, SSDMobileDetGPUFeatureExtractor)
...
if tf_version.is_tf1():
    ...
    FEATURE_EXTRACTOR_MAPS = [SSD_FEATURE_EXTRACTOR_CLASS_MAP, FASTER_RCNN_FEATURE_EXTRACTOR_CLASS_MAP]
...
def _build_ssd_model(ssd_config, is_training, add_summaries):
    ...
    target_assigner_instance = target_assigner.TargetAssigner(
        region_similarity_calculator, matcher, box_coder, negative_class_weight=negative_class_weight
    )
...
def _build_faster_rcnn_feature_extractor(
    feature_extractor_config, is_training, reuse_weights=True, inplace_batchnorm_update=False
):
...
def _build_faster_rcnn_keras_feature_extractor(feature_extractor_config, is_training, inplace_batchnorm_update=False):
    ...
    return feature_extractor_class(is_training, first_stage_features_stride, batch_norm_trainable, **kwargs)
...
def _build_faster_rcnn_model(frcnn_config, is_training, add_summaries):
    ...
    if is_keras:
        feature_extractor = _build_faster_rcnn_keras_feature_extractor(
            frcnn_config.feature_extractor, is_training, inplace_batchnorm_update=frcnn_config.inplace_batchnorm_update
        )
    else:
    ...
    first_stage_target_assigner = target_assigner.create_target_assigner(
        "FasterRCNN", "proposal", use_matmul_gather=frcnn_config.use_matmul_gather_in_matcher
    )
    first_stage_atrous_rate = frcnn_config.first_stage_atrous_rate
    if is_keras:
        first_stage_box_predictor_arg_scope_fn = hyperparams_builder.KerasLayerHyperparams(
    ...
    second_stage_target_assigner = target_assigner.create_target_assigner(
        "FasterRCNN", "detection", use_matmul_gather=frcnn_config.use_matmul_gather_in_matcher
    )
    if is_keras:
    ...
    if (not is_keras and isinstance(second_stage_box_predictor, rfcn_box_predictor.RfcnBoxPredictor)) or (
        is_keras and isinstance(second_stage_box_predictor, rfcn_keras_box_predictor.RfcnKerasBoxPredictor)
    ):
...
def keypoint_proto_to_params(kp_config, keypoint_map_dict):
    ...
    classification_loss, localization_loss, _, _, _, _, _ = losses_builder.build(kp_config.loss)
    ...
    keypoint_std_dev_dict = {label: KEYPOINT_STD_DEV_DEFAULT for label in keypoint_labels}
...
def object_detection_proto_to_params(od_config):
    ...
    # directly.
    loss.classification_loss.weighted_sigmoid.CopyFrom(losses_pb2.WeightedSigmoidClassificationLoss())
...
def densepose_proto_to_params(densepose_config):
    ...
    classification_loss, localization_loss, _, _, _, _, _ = losses_builder.build(densepose_config.loss)
...
def temporal_offset_proto_to_params(temporal_offset_config):
    ...
    return center_net_meta_arch.TemporalOffsetParams(
        localization_loss=localization_loss, task_loss_weight=temporal_offset_config.task_loss_weight
    )
...
def _build_center_net_model(center_net_config, is_training, add_summaries):
    ...
    feature_extractor = _build_center_net_feature_extractor(center_net_config.feature_extractor)
    object_center_params = object_center_proto_to_params(center_net_config.object_center_params)

    object_detection_params = None
    if center_net_config.HasField("object_detection_task"):
        object_detection_params = object_detection_proto_to_params(center_net_config.object_detection_task)

    keypoint_params_dict = None
    if center_net_config.keypoint_estimation_task:
        label_map_proto = label_map_util.load_labelmap(center_net_config.keypoint_label_map_path)
        keypoint_map_dict = {item.name: item for item in label_map_proto.item if item.keypoints}
        ...
        for task in center_net_config.keypoint_estimation_task:
            ...
            if kp_params.class_id in keypoint_class_id_set:
                raise ValueError(
                    ("Multiple keypoint tasks map to the same class id is not allowed: %d" % kp_params.class_id)
                )
            else:
    ...
    if center_net_config.HasField("densepose_estimation_task"):
        densepose_params = densepose_proto_to_params(center_net_config.densepose_estimation_task)
    ...
    if center_net_config.HasField("temporal_offset_task"):
        temporal_offset_params = temporal_offset_proto_to_params(center_net_config.temporal_offset_task)
...
def _build_center_net_feature_extractor(feature_extractor_config):
    ...
    if feature_extractor_config.type not in CENTER_NET_EXTRACTOR_FUNCTION_MAP:
        raise ValueError("'{}' is not a known CenterNet feature extractor type".format(feature_extractor_config.type))
