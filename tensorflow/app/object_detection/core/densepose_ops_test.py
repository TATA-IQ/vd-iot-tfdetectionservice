...
class DensePoseOpsTest(test_case.TestCase):
    ...
    def test_scale(self):
        def graph_fn():
            dp_surface_coords = tf.constant(
                [[[0.0, 0.0, 0.1, 0.2], [100.0, 200.0, 0.3, 0.4]], [[50.0, 120.0, 0.5, 0.6], [100.0, 140.0, 0.7, 0.8]]]
            )
        ...
        expected_dp_surface_coords = np.array(
            [[[0.0, 0.0, 0.1, 0.2], [1.0, 1.0, 0.3, 0.4]], [[0.5, 0.6, 0.5, 0.6], [1.0, 0.7, 0.7, 0.8]]]
        )
    ...
    def test_clip_to_window(self):
        def graph_fn():
            dp_surface_coords = tf.constant(
                [[[0.25, 0.5, 0.1, 0.2], [0.75, 0.75, 0.3, 0.4]], [[0.5, 0.0, 0.5, 0.6], [1.0, 1.0, 0.7, 0.8]]]
            )
        ...
        expected_dp_surface_coords = np.array(
            [[[0.25, 0.5, 0.1, 0.2], [0.75, 0.75, 0.3, 0.4]], [[0.5, 0.25, 0.5, 0.6], [0.75, 0.75, 0.7, 0.8]]]
        )
        self.assertAllClose(output, expected_dp_surface_coords)
    ...
    def test_prune_outside_window(self):
        ...
        new_dp_num_points, new_dp_part_ids, new_dp_surface_coords = self.execute_cpu(graph_fn, [])
    ...
    def test_change_coordinate_frame(self):
        ...
        expected_dp_surface_coords = np.array(
            [[[0, 0.5, 0.1, 0.2], [1.0, 1.0, 0.3, 0.4]], [[0.5, -0.5, 0.5, 0.6], [1.5, 1.5, 0.7, 0.8]]]
        )
        self.assertAllClose(output, expected_dp_surface_coords)
    ...
    def test_to_normalized_coordinates(self):
        def graph_fn():
            dp_surface_coords = tf.constant(
                [[[10.0, 30.0, 0.1, 0.2], [30.0, 45.0, 0.3, 0.4]], [[20.0, 0.0, 0.5, 0.6], [40.0, 60.0, 0.7, 0.8]]]
            )
    ...
    def test_horizontal_flip(self):
        ...
        def graph_fn():
            ...
            flipped_part_ids, flipped_surf_coords = densepose_ops.flip_horizontal(part_ids, surf_coords)
            flipped_twice_part_ids, flipped_twice_surf_coords = densepose_ops.flip_horizontal(
                flipped_part_ids, flipped_surf_coords
            )
            return (flipped_part_ids, flipped_surf_coords, flipped_twice_part_ids, flipped_twice_surf_coords)

        (flipped_part_ids, flipped_surf_coords, flipped_twice_part_ids, flipped_twice_surf_coords) = self.execute(
            graph_fn, []
        )
        ...
        self.assertAllClose(expected_flipped_surf_coords_yx, flipped_surf_coords[:, :, 0:2])
        self.assertAllEqual(part_ids_np, flipped_twice_part_ids)
        self.assertAllClose(surf_coords_np, flipped_twice_surf_coords, rtol=1e-2, atol=1e-2)
