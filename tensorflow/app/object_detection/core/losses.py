...
class Loss(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def __call__(
        self, prediction_tensor, target_tensor, ignore_nan_targets=False, losses_mask=None, scope=None, **params
    ):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        Returns:
          loss: a float tensor of shape [batch_size, num_anchors] tensor
            representing the value of the loss function.
        """
        batch_size, num_anchors, _ = shape_utils.combined_static_and_dynamic_shape(prediction_tensor)
        ...
        return tf.reshape(tf.reshape(weights, [-1]) * per_anchor_iou_loss, [batch_size, num_anchors])
...
class WeightedSoftmaxClassificationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        per_row_cross_ent = tf.nn.softmax_cross_entropy_with_logits(
            labels=tf.reshape(target_tensor, [-1, num_classes]), logits=tf.reshape(prediction_tensor, [-1, num_classes])
        )
...
class BootstrappedSigmoidClassificationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        if self._bootstrap_type == "soft":
            ...
            bootstrap_target_tensor = self._alpha * target_tensor + (1.0 - self._alpha) * tf.cast(
...
class HardExampleMiner(object):
        ...
        for ind, detection_boxlist in enumerate(decoded_boxlist_list):
            ...
            if self._max_negatives_per_positive is not None and match:
                (selected_indices, num_positives, num_negatives) = self._subsample_selection_to_desired_neg_pos_ratio(
                    selected_indices, match, self._max_negatives_per_positive, self._min_negatives_per_image
                )
    ...
    def _subsample_selection_to_desired_neg_pos_ratio(
        ...
        max_negatives = tf.maximum(
            ...
            tf.cast(max_negatives_per_positive * tf.cast(num_positives, dtype=tf.float32), dtype=tf.int32),
        )
...
class PenaltyReducedLogisticFocalLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        prediction_tensor = tf.clip_by_value(
            tf.sigmoid(prediction_tensor), self._sigmoid_clip_value, 1 - self._sigmoid_clip_value
        )

        positive_loss = tf.math.pow((1 - prediction_tensor), self._alpha) * tf.math.log(prediction_tensor)
        negative_loss = (
...
class L1LocalizationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        return tf.losses.absolute_difference(
            target_tensor, prediction_tensor, weights=weights, loss_collection=None, reduction=tf.losses.Reduction.NONE
        )
