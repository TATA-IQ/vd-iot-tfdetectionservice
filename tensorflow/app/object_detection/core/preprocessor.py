...
from object_detection.core import (box_list, box_list_ops, densepose_ops,
                                   keypoint_ops, preprocessor_cache)
from object_detection.core import standard_fields as fields
from object_detection.utils import (autoaugment_utils, ops, patch_ops,
                                    shape_utils)
...
def _apply_with_random_selector(x, func, num_cases, preprocess_vars_cache=None, key=""):
    ...
    return control_flow_ops.merge(
        [func(control_flow_ops.switch(x, tf.equal(rand_sel, case))[1], case) for case in range(num_cases)]
    )[0]
...
def _apply_with_random_selector_tuples(x, func, num_cases, preprocess_vars_cache=None, key=""):
    ...
    generator_func = functools.partial(tf.random_uniform, [], maxval=num_cases, dtype=tf.int32)
    rand_sel = _get_or_create_preprocess_rand_vars(
        generator_func, preprocessor_cache.PreprocessorCache.SELECTOR_TUPLES, preprocess_vars_cache, key
    )
...
def drop_label_probabilistically(
    ...
    with tf.name_scope("DropLabelProbabilistically", values=[boxes, labels]):
        indices = tf.where(
            tf.logical_or(
                tf.random_uniform(tf.shape(labels), seed=seed) > drop_probability, tf.not_equal(labels, dropped_label)
            )
        )
        indices = tf.squeeze(indices, axis=1)
...
def random_horizontal_flip(
    ...
    if (densepose_part_ids is not None and densepose_surface_coords is None) or (
        ...
        raise ValueError("Must provide both `densepose_part_ids` and `densepose_surface_coords`")
    ...
    with tf.name_scope("RandomHorizontalFlip", values=[image, boxes]):
        ...
        do_a_flip_random = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.HORIZONTAL_FLIP, preprocess_vars_cache
        )
        ...
        if keypoints is not None and keypoint_flip_permutation is not None:
            ...
            keypoints = tf.cond(
                do_a_flip_random, lambda: keypoint_ops.flip_horizontal(keypoints, 0.5, permutation), lambda: keypoints
            )
        ...
        if densepose_part_ids is not None:
            flip_densepose_fn = functools.partial(
                densepose_ops.flip_horizontal, densepose_part_ids, densepose_surface_coords
            )
            densepose_tensors = tf.cond(
                do_a_flip_random, flip_densepose_fn, lambda: (densepose_part_ids, densepose_surface_coords)
            )
...
def random_vertical_flip(
    ...
    with tf.name_scope("RandomVerticalFlip", values=[image, boxes]):
        ...
        if keypoints is not None and keypoint_flip_permutation is not None:
            permutation = keypoint_flip_permutation
            keypoints = tf.cond(
                do_a_flip_random, lambda: keypoint_ops.flip_vertical(keypoints, 0.5, permutation), lambda: keypoints
            )
            result.append(keypoints)
...
def random_rotation90(
    ...
    with tf.name_scope("RandomRotation90", values=[image, boxes]):
        ...
        if keypoints is not None:
            keypoints = tf.cond(
                do_a_rot90_random, lambda: keypoint_ops.rot90(keypoints, keypoint_rot_permutation), lambda: keypoints
            )
            result.append(keypoints)
...
def random_pixel_value_scale(image, minval=0.9, maxval=1.1, seed=None, preprocess_vars_cache=None):
    ...
    with tf.name_scope("RandomPixelValueScale", values=[image]):
        ...
        color_coef = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.PIXEL_VALUE_SCALE, preprocess_vars_cache
        )
...
def random_image_scale(
    image, masks=None, min_scale_ratio=0.5, max_scale_ratio=2.0, seed=None, preprocess_vars_cache=None
):
    ...
    with tf.name_scope("RandomImageScale", values=[image]):
        ...
        image_newysize = tf.cast(tf.multiply(tf.cast(image_height, dtype=tf.float32), size_coef), dtype=tf.int32)
        image_newxsize = tf.cast(tf.multiply(tf.cast(image_width, dtype=tf.float32), size_coef), dtype=tf.int32)
...
def random_adjust_brightness(image, max_delta=0.2, seed=None, preprocess_vars_cache=None):
    ...
    with tf.name_scope("RandomAdjustBrightness", values=[image]):
        ...
        delta = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.ADJUST_BRIGHTNESS, preprocess_vars_cache
        )
...
def random_adjust_contrast(image, min_delta=0.8, max_delta=1.25, seed=None, preprocess_vars_cache=None):
    ...
    with tf.name_scope("RandomAdjustContrast", values=[image]):
        ...
        contrast_factor = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.ADJUST_CONTRAST, preprocess_vars_cache
        )
...
def random_adjust_saturation(image, min_delta=0.8, max_delta=1.25, seed=None, preprocess_vars_cache=None):
    ...
    with tf.name_scope("RandomAdjustSaturation", values=[image]):
        ...
        saturation_factor = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.ADJUST_SATURATION, preprocess_vars_cache
        )
...
def random_distort_color(image, color_ordering=0, preprocess_vars_cache=None):
    ...
    with tf.name_scope("RandomDistortColor", values=[image]):
        if color_ordering == 0:
            image = random_adjust_brightness(image, max_delta=32.0 / 255.0, preprocess_vars_cache=preprocess_vars_cache)
            image = random_adjust_saturation(
                image, min_delta=0.5, max_delta=1.5, preprocess_vars_cache=preprocess_vars_cache
            )
            image = random_adjust_hue(image, max_delta=0.2, preprocess_vars_cache=preprocess_vars_cache)
...
def _strict_random_crop_image(
    ...
    with tf.name_scope("RandomCropImage", values=[image, boxes]):
        densepose_tensors = [densepose_num_points, densepose_part_ids, densepose_surface_coords]
        if any(t is not None for t in densepose_tensors) and not all(t is not None for t in densepose_tensors):
            raise ValueError(
        ...
        if keypoints is not None:
            ...
            if clip_boxes:
                new_keypoints = keypoint_ops.prune_outside_window(new_keypoints, [0.0, 0.0, 1.0, 1.0])
        ...
        if densepose_num_points is not None:
            ...
            for dp_tensor in densepose_tensors:
                ...
                dp_tensor_completely_inside_window = tf.gather(dp_tensor_inside_window, keep_ids)
            ...
            new_dp_surf_coords = densepose_ops.change_coordinate_frame(filtered_dp_tensors[2], im_box_rank1)
            if clip_boxes:
                new_dp_num_points, new_dp_point_ids, new_dp_surf_coords = densepose_ops.prune_outside_window(
                    new_dp_num_points, new_dp_point_ids, new_dp_surf_coords, window=[0.0, 0.0, 1.0, 1.0]
                )
...
def random_crop_image(
    ...
    if random_coef < sys.float_info.min:
        ...
        if densepose_num_points is not None:
            outputs.extend([densepose_num_points, densepose_part_ids, densepose_surface_coords])
...
def random_pad_image(
    ...
    # setting boxes
    new_window = tf.cast(
        tf.stack([-offset_height, -offset_width, target_height - offset_height, target_width - offset_width]),
        dtype=tf.float32,
    )
    ...
    if densepose_surface_coords is not None:
        new_densepose_surface_coords = densepose_ops.change_coordinate_frame(densepose_surface_coords, new_window)
...
def random_absolute_pad_image(
    ...
    max_image_size = min_image_size + tf.cast([max_height_padding, max_width_padding], dtype=tf.int32)
...
def random_crop_pad_image(
    ...
    min_image_size = tf.cast(
        tf.cast(tf.stack([image_height, image_width]), dtype=tf.float32) * min_padded_size_ratio, dtype=tf.int32
    )
    max_image_size = tf.cast(
        tf.cast(tf.stack([image_height, image_width]), dtype=tf.float32) * max_padded_size_ratio, dtype=tf.int32
    )
...
def random_crop_to_aspect_ratio(
    ...
    with tf.name_scope("RandomCropToAspectRatio", values=[image]):
        ...
        offset_height, offset_width = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.CROP_TO_ASPECT_RATIO, preprocess_vars_cache
        )
...
def random_pad_to_aspect_ratio(
    ...
    with tf.name_scope("RandomPadToAspectRatio", values=[image]):
        ...
        new_aspect_ratio = tf.constant(aspect_ratio, dtype=tf.float32)
        target_height = tf.cond(
            image_aspect_ratio <= new_aspect_ratio, lambda: image_height, lambda: image_width / new_aspect_ratio
        )
        target_width = tf.cond(
            image_aspect_ratio >= new_aspect_ratio, lambda: image_width, lambda: image_height * new_aspect_ratio
        )
        ...
        scale = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.PAD_TO_ASPECT_RATIO, preprocess_vars_cache
        )
        ...
        if masks is not None:
            ...
            new_masks = tf.image.pad_to_bounding_box(
                new_masks, 0, 0, tf.cast(target_height, dtype=tf.int32), tf.cast(target_width, dtype=tf.int32)
            )
...
def random_black_patches(
    image, max_black_patches=10, probability=0.5, size_to_image_ratio=0.1, random_seed=None, preprocess_vars_cache=None
):
    ...
    with tf.name_scope("RandomBlackPatchInImage", values=[image]):
        for idx in range(max_black_patches):
            ...
            random_prob = _get_or_create_preprocess_rand_vars(
                generator_func, preprocessor_cache.PreprocessorCache.BLACK_PATCHES, preprocess_vars_cache, key=idx
            )
            image = tf.cond(
...
def random_jpeg_quality(
    image, min_jpeg_quality=0, max_jpeg_quality=100, random_coef=0.0, seed=None, preprocess_vars_cache=None
):
    ...
    def _adjust_jpeg_quality():
        ...
        generator_func = functools.partial(
            tf.random_uniform, [], minval=min_jpeg_quality, maxval=max_jpeg_quality, dtype=tf.int32, seed=seed
        )
        quality = _get_or_create_preprocess_rand_vars(
            generator_func, preprocessor_cache.PreprocessorCache.JPEG_QUALITY, preprocess_vars_cache, key="quality"
        )
...
def random_downscale_to_target_pixels(
    ...
    with tf.name_scope("RandomDownscaleToTargetPixels", values=[image]):
        ...
        do_downscale_random = _get_or_create_preprocess_rand_vars(
            generator_fn, preprocessor_cache.PreprocessorCache.DOWNSCALE_TO_TARGET_PIXELS, preprocess_vars_cache
        )
        do_downscale_random = tf.greater_equal(do_downscale_random, random_coef)

        generator_fn = functools.partial(
            tf.random_uniform, [], minval=min_target_pixels, maxval=max_target_pixels, dtype=tf.int32, seed=seed
        )
        ...
        target_height = tf.cast(scale_factor * tf.cast(image_height, dtype=tf.float32), dtype=tf.int32)
        target_width = tf.cast(scale_factor * tf.cast(image_width, dtype=tf.float32), dtype=tf.int32)
        ...
        if masks is not None:
            ...
            return tf.cond(should_apply_resize, resize_fn, lambda: (tf.cast(image, dtype=tf.float32), masks))
        else:
            resize_fn = lambda: _resize_image_to_target(target_height, target_width)
            return tf.cond(should_apply_resize, resize_fn, lambda: (tf.cast(image, dtype=tf.float32),))
...
def random_patch_gaussian(
    ...
    def _apply_patch_gaussian(image):
        ...
        patch_size = get_or_create_rand_vars_fn(
            functools.partial(
                tf.random_uniform, [], minval=min_patch_size, maxval=max_patch_size, dtype=tf.int32, seed=seed
            ),
            key="patch_size",
        )
        ...
        y = get_or_create_rand_vars_fn(
            functools.partial(tf.random_uniform, [], minval=0, maxval=image_shape[0], dtype=tf.int32, seed=seed),
            key="y",
        )
        x = get_or_create_rand_vars_fn(
            functools.partial(tf.random_uniform, [], minval=0, maxval=image_shape[1], dtype=tf.int32, seed=seed),
            key="x",
        )
        gaussian = get_or_create_rand_vars_fn(
            functools.partial(tf.random.normal, image_shape, stddev=gaussian_stddev, dtype=tf.float32, seed=seed),
            key="gaussian",
        )
...
def resize_to_min_dimension(image, masks=None, min_dimension=600, method=tf.image.ResizeMethod.BILINEAR):
    ...
    with tf.name_scope("ResizeGivenMinDimension", values=[image, min_dimension]):
        ...
        target_ratio = tf.cast(min_target_dimension, dtype=tf.float32) / tf.cast(min_image_dimension, dtype=tf.float32)
        ...
        image = tf.image.resize_images(
            tf.expand_dims(image, axis=0), size=[target_height, target_width], method=method, align_corners=True
        )
...
def resize_to_max_dimension(image, masks=None, max_dimension=600, method=tf.image.ResizeMethod.BILINEAR):
    ...
    with tf.name_scope("ResizeGivenMaxDimension", values=[image, max_dimension]):
        ...
        target_ratio = tf.cast(max_target_dimension, dtype=tf.float32) / tf.cast(max_image_dimension, dtype=tf.float32)
        target_height = tf.cast(tf.cast(image_height, dtype=tf.float32) * target_ratio, dtype=tf.int32)
        target_width = tf.cast(tf.cast(image_width, dtype=tf.float32) * target_ratio, dtype=tf.int32)
        image = tf.image.resize_images(
            tf.expand_dims(image, axis=0), size=[target_height, target_width], method=method, align_corners=True
...
def resize_image(
    image, masks=None, new_height=600, new_width=1024, method=tf.image.ResizeMethod.BILINEAR, align_corners=False
):
...
def random_self_concat_image(
    ...
    if multiclass_scores is not None:
        ...
        outputs.append(
            tf.cond(concat_horizontal, lambda: tf.tile(multiclass_scores, [2, 1]), lambda: multiclass_scores)
        )
...
def ssd_random_crop(
    ...
    result = _apply_with_random_selector_tuples(
        tuple(
            t
            for t in (image, boxes, labels, label_weights, label_confidences, multiclass_scores, masks, keypoints)
            if t is not None
        ),
...
def ssd_random_crop_pad(
    ...
    return _apply_with_random_selector_tuples(
        tuple(t for t in (image, boxes, labels, label_weights, label_confidences, multiclass_scores) if t is not None),
...
def convert_class_logits_to_softmax(multiclass_scores, temperature=1.0):
    ...
    multiclass_scores_scaled = tf.multiply(multiclass_scores, 1.0 / temperature, name="scale_logits")
...
def random_square_crop_by_scale(
    ...
    y_center = _get_or_create_preprocess_rand_vars(
        y_function, preprocessor_cache.PreprocessorCache.SQUARE_CROP_BY_SCALE, preprocess_vars_cache, "y_center"
    )

    x_center = _get_or_create_preprocess_rand_vars(
        x_function, preprocessor_cache.PreprocessorCache.SQUARE_CROP_BY_SCALE, preprocess_vars_cache, "x_center"
    )
    ...
    output_image = tf.image.pad_to_bounding_box(
        cropped_image, offset_height=oy_i, offset_width=ox_i, target_height=image_size, target_width=image_size
    )
    ...
    boxlist = box_list_ops.clip_to_window(boxlist, [0.0, 0.0, 1.0, 1.0], filter_nonoverlapping=False)

    return_values = [output_image, boxlist.get(), tf.gather(labels, indices), tf.gather(label_weights, indices)]
...
def random_scale_crop_and_pad_to_square(
    ...
    scaled_image = tf.image.resize_images(image, [scaled_height, scaled_width], method=resize_method)
    scaled_image = scaled_image[offset_y : offset_y + output_size, offset_x : offset_x + output_size, :]
    output_image = tf.image.pad_to_bounding_box(scaled_image, 0, 0, output_size, output_size)

    # Update the boxes.
    new_window = tf.cast(
        tf.stack([offset_y, offset_x, offset_y + output_size, offset_x + output_size]), dtype=tf.float32
    )
    new_window /= tf.cast(tf.stack([scaled_height, scaled_width, scaled_height, scaled_width]), dtype=tf.float32)
    boxlist = box_list.BoxList(boxes)
    ...
    if masks is not None:
        new_masks = tf.expand_dims(masks, -1)
        new_masks = tf.image.resize_images(new_masks, [scaled_height, scaled_width], method=resize_method)
        new_masks = new_masks[:, offset_y : offset_y + output_size, offset_x : offset_x + output_size, :]
        new_masks = tf.image.pad_to_bounding_box(new_masks, 0, 0, output_size, output_size)
        new_masks = tf.squeeze(new_masks, [-1])
        return_values.append(tf.gather(new_masks, indices))
...
def get_default_func_arg_map(
    ...
    if include_keypoint_visibilities:
        groundtruth_keypoint_visibilities = fields.InputDataFields.groundtruth_keypoint_visibilities
    ...
    if include_dense_pose:
        ...
        groundtruth_dp_surface_coords = fields.InputDataFields.groundtruth_dp_surface_coords
