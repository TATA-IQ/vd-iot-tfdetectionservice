...
from object_detection.utils import label_map_util, shape_utils
...
class _ClassTensorHandler(slim_example_decoder.Tensor):
    ...
    def __init__(self, tensor_key, label_map_proto_file, shape_keys=None, shape=None, default_value=""):
        ...
        """
        name_to_id = label_map_util.get_label_map_dict(label_map_proto_file, use_display_name=False)
        ...
        name_to_id_table = hash_table_class(
            initializer=lookup.KeyValueTensorInitializer(
                keys=tf.constant(list(name_to_id.keys())), values=tf.constant(list(name_to_id.values()), dtype=tf.int64)
            ),
            default_value=-1,
        )
        display_name_to_id = label_map_util.get_label_map_dict(label_map_proto_file, use_display_name=True)
        # We use a default_value of -1, but we expect all labels to be contained
        # in the label map.
    ...
    def tensors_to_item(self, keys_to_tensors):
        ...
        return tf.maximum(
            self._name_to_id_table.lookup(unmapped_tensor), self._display_name_to_id_table.lookup(unmapped_tensor)
        )
...
class TfExampleDecoder(data_decoder.DataDecoder):
    ...
    def __init__(
        ...
        if dct_method:
            image = slim_example_decoder.Image(
                image_key="image/encoded", format_key="image/format", channels=3, dct_method=dct_method
            )
            ...
            additional_channel_image = slim_example_decoder.Image(
                image_key="image/additional_channels/encoded", format_key="image/format", channels=1, repeated=True
            )
        ...
        if load_multiclass_scores:
            ...
            self.items_to_handlers[fields.InputDataFields.multiclass_scores] = slim_example_decoder.Tensor(
                "image/object/class/multiclass_scores"
            )

        if load_context_features:
            self.keys_to_features["image/context_features"] = tf.VarLenFeature(tf.float32)
            self.items_to_handlers[fields.InputDataFields.context_features] = slim_example_decoder.ItemHandlerCallback(
                ["image/context_features", "image/context_feature_length"], self._reshape_context_features
            )

            self.keys_to_features["image/context_feature_length"] = tf.FixedLenFeature((), tf.int64)
            self.items_to_handlers[fields.InputDataFields.context_feature_length] = slim_example_decoder.Tensor(
        ...
        if num_keypoints > 0:
            ...
            self.keys_to_features["image/object/keypoint/visibility"] = tf.VarLenFeature(tf.int64)
            self.items_to_handlers[
            ...
            self.items_to_handlers[kpt_vis_field] = slim_example_decoder.ItemHandlerCallback(
                ["image/object/keypoint/x", "image/object/keypoint/visibility"], self._reshape_keypoint_visibilities
            )
        ...
        if load_dense_pose:
            self.keys_to_features["image/object/densepose/num"] = tf.VarLenFeature(tf.int64)
            self.keys_to_features["image/object/densepose/part_index"] = tf.VarLenFeature(tf.int64)
            self.keys_to_features["image/object/densepose/x"] = tf.VarLenFeature(tf.float32)
            self.keys_to_features["image/object/densepose/y"] = tf.VarLenFeature(tf.float32)
            self.keys_to_features["image/object/densepose/u"] = tf.VarLenFeature(tf.float32)
            self.keys_to_features["image/object/densepose/v"] = tf.VarLenFeature(tf.float32)
            self.items_to_handlers[fields.InputDataFields.groundtruth_dp_num_points] = slim_example_decoder.Tensor(
                "image/object/densepose/num"
            )
            self.items_to_handlers[
                fields.InputDataFields.groundtruth_dp_part_ids
            ] = slim_example_decoder.ItemHandlerCallback(
                ["image/object/densepose/part_index", "image/object/densepose/num"], self._dense_pose_part_indices
            )
            self.items_to_handlers[
        ...
        if load_track_id:
            self.keys_to_features["image/object/track/label"] = tf.VarLenFeature(tf.int64)
            self.items_to_handlers[fields.InputDataFields.groundtruth_track_ids] = slim_example_decoder.Tensor(
        ...
        if expand_hierarchy_labels:
            if label_map_proto_file:
                ancestors_lut, descendants_lut = label_map_util.get_label_map_hierarchy_lut(label_map_proto_file, True)
                ...
                raise ValueError("In order to expand labels, the label_map_proto_file has to be provided.")
    ...
    def decode(self, tf_example_string_tensor):
        ...
        if fields.InputDataFields.groundtruth_keypoints in tensor_dict:
            ...
            visibilities_tiled = tf.tile(tf.expand_dims(tensor_dict[gt_kpt_vis_fld], -1), [1, 1, 2])
            tensor_dict[gt_kpt_fld] = tf.where(
                visibilities_tiled, tensor_dict[gt_kpt_fld], np.nan * tf.ones_like(tensor_dict[gt_kpt_fld])
            )
        ...
        if self._expand_hierarchy_labels:
            ...
            def expand_field(field_name):
                return self._expansion_box_field_labels(
                    tensor_dict[input_fields.groundtruth_classes], tensor_dict[field_name]
                )
            ...
            for field in box_fields:
                if field in tensor_dict:
                    tensor_dict[field] = tf.cond(
                        tf.size(tensor_dict[field]) > 0, lambda: expand_field(field), lambda: tensor_dict[field]
                    )
            # pylint: enable=cell-var-from-loop

            tensor_dict[input_fields.groundtruth_classes] = self._expansion_box_field_labels(
                tensor_dict[input_fields.groundtruth_classes], tensor_dict[input_fields.groundtruth_classes], True
            )
        ...
        if fields.InputDataFields.groundtruth_dp_num_points in tensor_dict:
            tensor_dict[fields.InputDataFields.groundtruth_dp_num_points] = tf.cast(
                tensor_dict[fields.InputDataFields.groundtruth_dp_num_points], dtype=tf.int32
            )
            tensor_dict[fields.InputDataFields.groundtruth_dp_part_ids] = tf.cast(
                tensor_dict[fields.InputDataFields.groundtruth_dp_part_ids], dtype=tf.int32
            )
        ...
        if fields.InputDataFields.groundtruth_track_ids in tensor_dict:
            tensor_dict[fields.InputDataFields.groundtruth_track_ids] = tf.cast(
                tensor_dict[fields.InputDataFields.groundtruth_track_ids], dtype=tf.int32
            )
    ...
    def _reshape_keypoint_visibilities(self, keys_to_tensors):
        ...
        vis = tf.cond(tf.equal(tf.size(x), tf.size(vis)), true_fn=lambda: vis, false_fn=lambda: default_vis)
        vis = tf.math.logical_or(
            tf.math.equal(vis, Visibility.NOT_VISIBLE.value), tf.math.equal(vis, Visibility.VISIBLE.value)
        )
    ...
    def _dense_pose_part_indices(self, keys_to_tensors):
        ...
        if isinstance(num_points_per_instances, tf.SparseTensor):
            num_points_per_instances = tf.sparse_tensor_to_dense(num_points_per_instances)
        ...
        max_points_per_instance = tf.cast(tf.math.reduce_max(num_points_per_instances), dtype=tf.int32)
        num_points_cumulative = tf.concat([[0], tf.math.cumsum(num_points_per_instances)], axis=0)
        ...
        def pad_parts_tensor(instance_ind):
            ...
            return shape_utils.pad_or_clip_nd(part_inds, output_shape=[max_points_per_instance])

        return tf.map_fn(pad_parts_tensor, tf.range(tf.size(num_points_per_instances)), dtype=tf.int32)
    ...
    def _dense_pose_surface_coordinates(self, keys_to_tensors):
        ...
        def pad_surface_coordinates_tensor(instance_ind):
            ...
            return shape_utils.pad_or_clip_nd(unpadded_tensor, output_shape=[max_points_per_instance, 4])

        return tf.map_fn(pad_surface_coordinates_tensor, tf.range(tf.size(num_points_per_instances)), dtype=tf.float32)
    ...
    def _expand_image_label_hierarchy(self, image_classes, image_confidences):
        ...
        def expand_labels(relation_tensor, confidence_value):
            ...
            expanded_indices = tf.reduce_any(
                (tf.gather(relation_tensor, target_image_classes - _LABEL_OFFSET, axis=0) > 0), axis=0
            )
            ...
            new_groundtruth_image_confidences = tf.concat(
                [
                    ...
                    tf.ones([tf.shape(expanded_indices)[0]], dtype=image_confidences.dtype) * confidence_value,
                ],
                axis=0,
            )
        ...
        new_image_classes, new_image_confidences = expand_labels(self._descendants_lut, 0.0)
        return new_image_classes, new_image_confidences

    def _expansion_box_field_labels(self, object_classes, object_field, copy_class_id=False):
        ...
        """
        expanded_indices = tf.gather(self._ancestors_lut, object_classes - _LABEL_OFFSET, axis=0)
        if copy_class_id:
            new_object_field = tf.where(expanded_indices > 0)[:, 1] + _LABEL_OFFSET
        else:
            new_object_field = tf.repeat(object_field, tf.reduce_sum(expanded_indices, axis=1), axis=0)
