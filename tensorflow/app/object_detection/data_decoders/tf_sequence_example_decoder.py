...
class TfSequenceExampleDecoder(data_decoder.DataDecoder):
    ...
    def __init__(
        self, label_map_proto_file, load_context_features=False, use_display_name=False, fully_annotated=False
    ):
        ...
        self._items_to_handlers = {
            # Context.
            fields.InputDataFields.image_height: slim_example_decoder.Tensor("image/height"),
            fields.InputDataFields.image_width: slim_example_decoder.Tensor("image/width"),
            # Sequence.
            fields.InputDataFields.num_groundtruth_boxes: slim_example_decoder.NumBoxesSequence("region/bbox/xmin"),
            fields.InputDataFields.groundtruth_boxes: slim_example_decoder.BoundingBoxSequence(
                prefix="region/bbox/", default_value=0.0
            ),
            fields.InputDataFields.groundtruth_weights: slim_example_decoder.Tensor("region/label/confidence"),
        }
        ...
        if not fully_annotated:
            self._sequence_keys_to_feature_lists["region/is_annotated"] = tf.FixedLenSequenceFeature([], dtype=tf.int64)
            self._items_to_handlers[fields.InputDataFields.is_annotated] = slim_example_decoder.Tensor(
                "region/is_annotated"
            )

        self._items_to_handlers[fields.InputDataFields.image] = slim_example_decoder.Tensor("image/encoded")
        self._items_to_handlers[fields.InputDataFields.source_id] = slim_example_decoder.Tensor("image/source_id")

        label_handler = _ClassTensorHandler("region/label/string", label_map_proto_file, default_value="")

        self._items_to_handlers[fields.InputDataFields.groundtruth_classes] = label_handler

        if load_context_features:
            self._context_keys_to_features["image/context_features"] = tf.VarLenFeature(dtype=tf.float32)
            self._items_to_handlers[fields.InputDataFields.context_features] = slim_example_decoder.ItemHandlerCallback(
                ["image/context_features", "image/context_feature_length"], self._reshape_context_features
            )

            self._context_keys_to_features["image/context_feature_length"] = tf.FixedLenFeature((), tf.int64)
            self._items_to_handlers[fields.InputDataFields.context_feature_length] = slim_example_decoder.Tensor(
                "image/context_feature_length"
            )
    ...
    def decode(self, tf_seq_example_string_tensor):
        ...
        decoder = slim_example_decoder.TFSequenceExampleDecoder(
            self._context_keys_to_features, self._sequence_keys_to_feature_lists, self._items_to_handlers
        )
        ...
        tensor_dict[fields.InputDataFields.original_image_spatial_shape] = tf.cast(
            tf.stack(
                [tensor_dict[fields.InputDataFields.image_height], tensor_dict[fields.InputDataFields.image_width]]
            ),
            dtype=tf.int32,
        )
        ...
        def default_groundtruth_weights():
            ...
            num_boxes_per_frame = tensor_dict[fields.InputDataFields.num_groundtruth_boxes]
            max_num_boxes = tf.reduce_max(num_boxes_per_frame)
            num_boxes_per_frame_tiled = tf.tile(
                tf.expand_dims(num_boxes_per_frame, axis=-1), multiples=tf.stack([1, max_num_boxes])
            )
            range_tiled = tf.tile(
                tf.expand_dims(tf.range(max_num_boxes), axis=0),
                multiples=tf.stack([tf.shape(num_boxes_per_frame)[0], 1]),
            )
            return tf.cast(tf.greater(num_boxes_per_frame_tiled, range_tiled), tf.float32)

        tensor_dict[fields.InputDataFields.groundtruth_weights] = tf.cond(
            tf.greater(tf.size(tensor_dict[fields.InputDataFields.groundtruth_weights]), 0),
