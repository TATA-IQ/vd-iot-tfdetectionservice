...
class GenerateEmbeddingDataFn(beam.DoFn):
    ...
    def __init__(self, model_dir, top_k_embedding_count, bottom_k_embedding_count):
    ...
    def _run_inference_and_generate_embedding(self, tfexample_key_value):
        ...
        try:
            ...
            unix_time = (date_captured - datetime.datetime.fromtimestamp(0)).total_seconds()
            example.features.feature["image/unix_time"].float_list.value.extend([unix_time])
            temporal_embedding = embed_date_captured(date_captured)
        except Exception:  # pylint: disable=broad-except
            temporal_embedding = None

        detections = self._detect_fn.signatures["serving_default"]((tf.expand_dims(tf.convert_to_tensor(tfexample), 0)))
        ...
        for index in range(min(num_detections, self._top_k_embedding_count)):
            bb_embedding, score = get_bb_embedding(detection_features, detection_boxes, detection_scores, index)
        ...
        for index in range(
            max(0, num_detections - 1), max(-1, num_detections - 1 - self._bottom_k_embedding_count), -1
        ):
            bb_embedding, score = get_bb_embedding(detection_features, detection_boxes, detection_scores, index)
            embed_all.extend(bb_embedding)
            if temporal_embedding is not None:
        ...
        if embedding_count == 0:
            bb_embedding, score = get_bb_embedding(detection_features, detection_boxes, detection_scores, 0)
            embed_all.extend(bb_embedding)
            if temporal_embedding is not None:
                embed_all.extend(temporal_embedding)
            score_all.append(score)
        ...
        example.features.feature["image/embedding_score"].float_list.value.extend(score_all)
        example.features.feature["image/embedding_length"].int64_list.value.append(embedding_length)
        example.features.feature["image/embedding_count"].int64_list.value.append(embedding_count)
...
def construct_pipeline(
    pipeline, input_tfrecord, output_tfrecord, model_dir, top_k_embedding_count, bottom_k_embedding_count, num_shards
):
    ...
    input_collection = (
        pipeline
        | "ReadInputTFRecord" >> beam.io.tfrecordio.ReadFromTFRecord(input_tfrecord, coder=beam.coders.BytesCoder())
