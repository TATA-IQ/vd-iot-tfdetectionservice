...
tf.flags.DEFINE_string(
    ...
    None,
    "Directory containing the image pixels downloaded from the OpenImages GitHub repository.",
)
tf.flags.DEFINE_string(
    "input_image_label_annotations_csv", None, "Path to CSV containing image-level labels annotations"
)
tf.flags.DEFINE_string("input_label_map", None, "Path to the label map proto")
tf.flags.DEFINE_string(
    "output_tf_record_path_prefix",
    None,
    "Path to the output TFRecord. The shard index and the number of shards will be appended for each output shard.",
)
