...
def sequence_strings_feature(strings):
    ...
    for single_str in strings:
        new_str_arr.append(tf.train.Feature(bytes_list=tf.train.BytesList(value=[single_str.encode("utf8")])))
...
def make_sequence_example(
    ...
    if detection_bboxes is not None:
        det_bbox_ymin, det_bbox_xmin, det_bbox_ymax, det_bbox_xmax = boxes_to_box_components(detection_bboxes)
        feature_list["predicted/region/bbox/xmin"] = sequence_float_feature(det_bbox_xmin)
        feature_list["predicted/region/bbox/xmax"] = sequence_float_feature(det_bbox_xmax)
        feature_list["predicted/region/bbox/ymin"] = sequence_float_feature(det_bbox_ymin)
        feature_list["predicted/region/bbox/ymax"] = sequence_float_feature(det_bbox_ymax)
    if detection_classes is not None:
        feature_list["predicted/region/label/index"] = sequence_int64_feature(detection_classes)
    if detection_scores is not None:
        feature_list["predicted/region/label/confidence"] = sequence_float_feature(detection_scores)
