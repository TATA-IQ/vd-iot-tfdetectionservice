...
class EvalUtilTest(test_case.TestCase, parameterized.TestCase):
    ...
    def _make_evaluation_dict(
        self, resized_groundtruth_masks=False, batch_size=1, max_gt_boxes=None, scale_to_absolute=False
    ):
        ...
        if batch_size > 1:
            ...
            groundtruth_keypoints = tf.tile(tf.expand_dims(groundtruth_keypoints, 0), multiples=[batch_size, 1, 1])
        ...
        if batch_size > 1:
            return eval_util.result_dict_for_batched_example(
                image, key, detections, groundtruth, scale_to_absolute=scale_to_absolute, max_gt_boxes=max_gt_boxes
            )
        else:
    ...
    def test_get_eval_metric_ops_for_evaluators(self):
        ...
        eval_config.metrics_set.extend(
            ["coco_detection_metrics", "coco_mask_metrics", "precision_at_recall_detection_metrics"]
        )
        ...
        self.assertFalse(evaluator_options["coco_detection_metrics"]["skip_predictions_for_unlabeled_class"])
        self.assertTrue(evaluator_options["coco_mask_metrics"]["include_metrics_per_category"])
        self.assertAlmostEqual(
            evaluator_options["precision_at_recall_detection_metrics"]["recall_lower_bound"],
            eval_config.recall_lower_bound,
        )
        self.assertAlmostEqual(
            evaluator_options["precision_at_recall_detection_metrics"]["recall_upper_bound"],
    ...
    def test_get_evaluator_with_keypoint_metrics(self):
        ...
        person_keypoints_metric.coco_keypoint_metrics.keypoint_label_to_sigmas["left_eye"] = 0.1
        person_keypoints_metric.coco_keypoint_metrics.keypoint_label_to_sigmas["right_eye"] = 0.2
        dog_keypoints_metric = eval_config.parameterized_metric.add()
        dog_keypoints_metric.coco_keypoint_metrics.class_label = "dog"
        dog_keypoints_metric.coco_keypoint_metrics.keypoint_label_to_sigmas["tail_start"] = 0.3
        dog_keypoints_metric.coco_keypoint_metrics.keypoint_label_to_sigmas["mouth"] = 0.4
    ...
    def test_get_evaluator_with_unmatched_label(self):
        ...
        person_keypoints_metric.coco_keypoint_metrics.keypoint_label_to_sigmas["kpt"] = 0.1
        categories = self._get_categories_list_with_keypoints()
    ...
    def test_padded_image_result_dict(self):
        ...
        detection_keypoints = np.array([[0.0, 0.0], [0.5, 0.5], [1.0, 1.0]], dtype=np.float32)
        ...
        def graph_fn():
            detections = {
                ...
                detection_fields.detection_keypoints: tf.tile(
                    tf.reshape(tf.constant(detection_keypoints), shape=[1, 1, 3, 2]), multiples=[2, 1, 1, 1]
                ),
            }
        ...
        (gt_boxes, gt_keypoints, detection_boxes, detection_keypoints) = self.execute_cpu(graph_fn, [])
        self.assertAllEqual([[[0.0, 0.0, 200.0, 200.0]], [[0.0, 0.0, 150.0, 150.0]]], gt_boxes)
        self.assertAllClose(
            [[[[0.0, 0.0], [100.0, 100.0], [200.0, 200.0]]], [[[0.0, 0.0], [150.0, 150.0], [300.0, 300.0]]]],
            gt_keypoints,
        )

        # Predictions from the model are not scaled.
        self.assertAllEqual([[[0.0, 0.0, 200.0, 200.0]], [[0.0, 0.0, 75.0, 150.0]]], detection_boxes)
        self.assertAllClose(
            [[[[0.0, 0.0], [100.0, 100.0], [200.0, 200.0]]], [[[0.0, 0.0], [75.0, 150.0], [150.0, 300.0]]]],
            detection_keypoints,
        )
