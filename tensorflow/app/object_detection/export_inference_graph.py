...
flags.DEFINE_string(
    "trained_checkpoint_prefix", None, "Path to trained checkpoint, typically of the form path/to/model.ckpt"
)
...
flags.DEFINE_string(
    ...
    None,
    "Additional Tensors to output, to be specified as a comma separated list of tensor names.",
)
...
def main(_):
    ...
    if FLAGS.use_side_inputs:
        side_input_shapes, side_input_names, side_input_types = exporter.parse_side_inputs(
    ...
    if FLAGS.additional_output_tensor_names:
        additional_output_tensor_names = list(FLAGS.additional_output_tensor_names.split(","))
    else:
