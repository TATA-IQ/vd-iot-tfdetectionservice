...
class FakeModel(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        with tf.control_dependencies([tf.convert_to_tensor(prediction_tensors["image"].get_shape().as_list()[1:3])]):
...
class ExportTfLiteGraphTest(tf.test.TestCase):
    ...
    def test_postprocess_implements_signature(self):
        ...
        with mock.patch.object(model_builder, "build", autospec=True) as mock_builder:
            ...
            self.assertEqual(expected_signature, detection_module.postprocess_implements_signature())
