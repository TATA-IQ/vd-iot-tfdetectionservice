...
from tensorflow.python.tools import \
    freeze_graph  # pylint: disable=g-direct-tensorflow-import
...
def parse_side_inputs(side_input_shapes_string, side_input_names_string, side_input_types_string):
    ...
    if side_input_shapes_string:
        ...
        for side_input_shape_list in side_input_shapes_string.split("/"):
            side_input_shape = [int(dim) if dim != "-1" else None for dim in side_input_shape_list.split(",")]
            side_input_shapes.append(side_input_shape)
    else:
        raise ValueError("When using side_inputs, side_input_shapes must be specified in the input flags.")
    if side_input_names_string:
        side_input_names = list(side_input_names_string.split(","))
    else:
        raise ValueError("When using side_inputs, side_input_names must be specified in the input flags.")
    if side_input_types_string:
        typelookup = {"float": tf.float32, "int": tf.int32, "string": tf.string}
        side_input_types = [typelookup[side_input_type] for side_input_type in side_input_types_string.split(",")]
    else:
        raise ValueError("When using side_inputs, side_input_types must be specified in the input flags.")
...
def rewrite_nn_resize_op(is_quantized=False):
    ...
    def remove_nn():
        ...
        consumer_pattern1 = graph_matcher.OpTypePattern(
            "Add|AddV2|Max|Mul", inputs=[reshape_pattern, "*"], ordered_inputs=False
        )
        consumer_pattern2 = graph_matcher.OpTypePattern(
            "StridedSlice", inputs=[reshape_pattern, "*", "*", "*"], ordered_inputs=False
        )
...
def _side_input_tensor_placeholder(side_input_shape, side_input_name, side_input_type):
    ...
    side_input_tensor = tf.placeholder(dtype=side_input_type, shape=side_input_shape, name=side_input_name)
...
def _tf_example_input_placeholder(input_shape=None):
    ...
    return (
        ...
        shape_utils.static_or_dynamic_map_fn(
            decode, elems=batch_tf_example_placeholder, dtype=tf.uint8, parallel_iterations=32, back_prop=False
        ),
    )
...
def profile_inference_graph(graph):
    ...
    """
    tfprof_vars_option = contrib_tfprof.model_analyzer.TRAINABLE_VARS_PARAMS_STAT_OPTIONS
    ...
    contrib_tfprof.model_analyzer.print_model_analysis(graph, tfprof_options=tfprof_vars_option)

    contrib_tfprof.model_analyzer.print_model_analysis(graph, tfprof_options=tfprof_flops_option)
