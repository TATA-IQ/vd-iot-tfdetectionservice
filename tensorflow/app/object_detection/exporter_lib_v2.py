...
class DetectionFromEncodedImageModule(DetectionInferenceModule):
    ...
    def __call__(self, input_tensor):
        with tf.device("cpu:0"):
            image = tf.map_fn(
                _decode_image, elems=input_tensor, dtype=tf.uint8, parallel_iterations=32, back_prop=False
            )
...
class DetectionFromTFExampleModule(DetectionInferenceModule):
    ...
    def __call__(self, input_tensor):
        with tf.device("cpu:0"):
            image = tf.map_fn(
                _decode_tf_example, elems=input_tensor, dtype=tf.uint8, parallel_iterations=32, back_prop=False
            )
        return self._run_inference_on_images(image)
