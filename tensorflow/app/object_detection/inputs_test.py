...
from object_detection.utils import (config_util, test_case, test_utils,
                                    tf_version)
...
def _get_configs_for_model_sequence_example(model_name, frame_index=-1):
    """Returns configurations for model."""
    fname = os.path.join(tf.resource_loader.get_data_files_path(), "test_data/" + model_name + ".config")
    label_map_path = os.path.join(tf.resource_loader.get_data_files_path(), "data/snapshot_serengeti_label_map.pbtxt")
    data_path = os.path.join(
        tf.resource_loader.get_data_files_path(), "test_data/snapshot_serengeti_sequence_examples.record"
    )
    configs = config_util.get_configs_from_pipeline_file(fname)
    override_dict = {
...
class InputFnTest(test_case.TestCase, parameterized.TestCase):
    ...
    def test_faster_rcnn_resnet50_eval_input(self, eval_batch_size=1):
        ...
        self.assertEqual(tf.float32, features[fields.InputDataFields.image].dtype)
        self.assertAllEqual(
            [eval_batch_size, None, None, 3], features[fields.InputDataFields.original_image].shape.as_list()
        )
        ...
        self.assertAllEqual(
            [eval_batch_size, 100], labels[fields.InputDataFields.groundtruth_difficult].shape.as_list()
        )
        self.assertEqual(tf.int32, labels[fields.InputDataFields.groundtruth_difficult].dtype)

    def test_context_rcnn_resnet50_train_input_with_sequence_example(self, train_batch_size=8):
        """Tests the training input function for FasterRcnnResnet50."""
        configs = _get_configs_for_model_sequence_example("context_rcnn_camera_trap")
        model_config = configs["model"]
        train_config = configs["train_config"]
        train_config.batch_size = train_batch_size
        train_input_fn = inputs.create_train_input_fn(train_config, configs["train_input_config"], model_config)
        features, labels = _make_initializable_iterator(train_input_fn()).get_next()

        self.assertAllEqual([train_batch_size, 640, 640, 3], features[fields.InputDataFields.image].shape.as_list())
        self.assertEqual(tf.float32, features[fields.InputDataFields.image].dtype)
        self.assertAllEqual([train_batch_size], features[inputs.HASH_KEY].shape.as_list())
        self.assertEqual(tf.int32, features[inputs.HASH_KEY].dtype)
        self.assertAllEqual(
            [train_batch_size, 100, 4], labels[fields.InputDataFields.groundtruth_boxes].shape.as_list()
        )
        self.assertEqual(tf.float32, labels[fields.InputDataFields.groundtruth_boxes].dtype)
        self.assertAllEqual(
        ...
        self.assertEqual(tf.float32, labels[fields.InputDataFields.groundtruth_classes].dtype)
        self.assertAllEqual([train_batch_size, 100], labels[fields.InputDataFields.groundtruth_weights].shape.as_list())
        self.assertEqual(tf.float32, labels[fields.InputDataFields.groundtruth_weights].dtype)
        self.assertAllEqual(
            [train_batch_size, 100, model_config.faster_rcnn.num_classes],
            labels[fields.InputDataFields.groundtruth_confidences].shape.as_list(),
    ...
    def test_context_rcnn_resnet50_eval_input_with_sequence_example(self, eval_batch_size=8):
        """Tests the eval input function for FasterRcnnResnet50."""
        configs = _get_configs_for_model_sequence_example("context_rcnn_camera_trap")
        model_config = configs["model"]
        eval_config = configs["eval_config"]
        ...
        eval_input_fn = inputs.create_eval_input_fn(eval_config, configs["eval_input_configs"][0], model_config)
        features, labels = _make_initializable_iterator(eval_input_fn()).get_next()
        self.assertAllEqual([eval_batch_size, 640, 640, 3], features[fields.InputDataFields.image].shape.as_list())
        self.assertEqual(tf.float32, features[fields.InputDataFields.image].dtype)
        self.assertAllEqual(
            [eval_batch_size, 640, 640, 3], features[fields.InputDataFields.original_image].shape.as_list()
        )
        self.assertEqual(tf.uint8, features[fields.InputDataFields.original_image].dtype)
        self.assertAllEqual([eval_batch_size], features[inputs.HASH_KEY].shape.as_list())
        self.assertEqual(tf.int32, features[inputs.HASH_KEY].dtype)
    ...
    def test_context_rcnn_resnet50_train_input_with_sequence_example_frame_index(self, train_batch_size=8):
        """Tests the training input function for FasterRcnnResnet50."""
        configs = _get_configs_for_model_sequence_example("context_rcnn_camera_trap", frame_index=2)
        model_config = configs["model"]
        train_config = configs["train_config"]
        train_config.batch_size = train_batch_size
        train_input_fn = inputs.create_train_input_fn(train_config, configs["train_input_config"], model_config)
    ...
    def test_ssd_inceptionV2_eval_input(self, eval_batch_size=1):
        ...
        self.assertEqual(tf.float32, features[fields.InputDataFields.image].dtype)
        self.assertAllEqual(
            [eval_batch_size, 300, 300, 3], features[fields.InputDataFields.original_image].shape.as_list()
        )
        self.assertEqual(tf.uint8, features[fields.InputDataFields.original_image].dtype)
        self.assertAllEqual([eval_batch_size], features[inputs.HASH_KEY].shape.as_list())
        self.assertEqual(tf.int32, features[inputs.HASH_KEY].dtype)
    ...
    def test_ssd_inceptionV2_eval_input_with_additional_channels(self, eval_batch_size=1):
        ...
        self.assertEqual(tf.uint8, features[fields.InputDataFields.original_image].dtype)
        self.assertAllEqual(
            [eval_batch_size, 300, 300, 1], features[fields.InputDataFields.image_additional_channels].shape.as_list()
        )
    ...
    def test_predict_input(self):
        ...
        predict_input_fn = inputs.create_predict_input_fn(
            model_config=configs["model"], predict_input_config=configs["eval_input_configs"][0]
        )
...
class DataAugmentationFnTest(test_case.TestCase):
    def test_apply_image_and_box_augmentation(self):
        data_augmentation_options = [
            (
                ...
                {"new_height": 20, "new_width": 20, "method": tf.image.ResizeMethod.NEAREST_NEIGHBOR},
            ),
            (preprocessor.scale_boxes_to_pixel_coordinates, {}),
        ]
        data_augmentation_fn = functools.partial(
            inputs.augment_input_data, data_augmentation_options=data_augmentation_options
        )

        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(np.random.rand(10, 10, 3).astype(np.float32)),
                fields.InputDataFields.groundtruth_boxes: tf.constant(np.array([[0.5, 0.5, 1.0, 1.0]], np.float32)),
            }
    ...
    def test_apply_image_and_box_augmentation_with_scores(self):
        ...
        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(np.random.rand(10, 10, 3).astype(np.float32)),
                fields.InputDataFields.groundtruth_boxes: tf.constant(np.array([[0.5, 0.5, 1.0, 1.0]], np.float32)),
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([1.0], np.float32)),
                fields.InputDataFields.groundtruth_weights: tf.constant(np.array([0.8], np.float32)),
            }
            augmented_tensor_dict = data_augmentation_fn(tensor_dict=tensor_dict)
            return (
                augmented_tensor_dict[fields.InputDataFields.image],
        ...
        (image, groundtruth_boxes, groundtruth_classes, groundtruth_weights) = self.execute_cpu(graph_fn, [])
        self.assertAllEqual(image.shape, [20, 20, 3])
        self.assertAllClose(groundtruth_boxes, [[10, 10, 20, 20]])
    ...
    def test_include_masks_in_data_augmentation(self):
        ...
        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(np.random.rand(10, 10, 3).astype(np.float32)),
                fields.InputDataFields.groundtruth_instance_masks: tf.constant(np.zeros([2, 10, 10], np.uint8)),
            }
            augmented_tensor_dict = data_augmentation_fn(tensor_dict=tensor_dict)
            return (
                augmented_tensor_dict[fields.InputDataFields.image],
                augmented_tensor_dict[fields.InputDataFields.groundtruth_instance_masks],
            )
...
class DataTransformationFnTest(test_case.TestCase, parameterized.TestCase):
    def test_combine_additional_channels_if_present(self):
        ...
        self.assertAllClose(out_image, np.concatenate((image, additional_channels), axis=2))

    def test_use_multiclass_scores_when_present(self):
        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(np.random.rand(4, 4, 3).astype(np.float32)),
                ...
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([1, 2], np.int32)),
            }
        ...
        self.assertAllClose(np.array([[0.2, 0.3, 0.5], [0.1, 0.6, 0.3]], np.float32), groundtruth_classes)

    @unittest.skipIf(tf_version.is_tf2(), "Skipping due to different behaviour in TF 2.X")
    def test_use_multiclass_scores_when_not_present(self):
        def graph_fn():
            zero_num_elements = tf.random.uniform([], minval=0, maxval=1, dtype=tf.int32)
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(np.random.rand(4, 4, 3).astype(np.float32)),
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    np.array([[0.5, 0.5, 1, 1], [0.5, 0.5, 1, 1]], np.float32)
        ...
        groundtruth_classes = self.execute_cpu(graph_fn, [])
        self.assertAllClose(np.array([[0, 1, 0], [0, 0, 1]], np.float32), groundtruth_classes)
    ...
    def test_use_labeled_classes(self, labeled_classes):
        def compute_fn(image, groundtruth_boxes, groundtruth_classes, groundtruth_labeled_classes):
            tensor_dict = {
                fields.InputDataFields.image: image,
        ...
        transformed_inputs = self.execute_cpu(
            compute_fn, [image, groundtruth_boxes, groundtruth_classes, groundtruth_labeled_classes]
        )
    ...
    def test_returns_correct_class_label_encodings(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([3, 1], np.int32)),
            }
            num_classes = 3
            input_transformation_fn = functools.partial(
                inputs.transform_input_data,
    ...
    def test_returns_correct_labels_with_unrecognized_class(self):
        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(np.random.rand(4, 4, 3).astype(np.float32)),
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    np.array([[0, 0, 1, 1], [0.2, 0.2, 4, 4], [0.5, 0.5, 1, 1]], np.float32)
                ),
                fields.InputDataFields.groundtruth_area: tf.constant(np.array([0.5, 0.4, 0.3])),
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([3, -1, 1], np.int32)),
                fields.InputDataFields.groundtruth_keypoints: tf.constant(
                ...
                fields.InputDataFields.groundtruth_is_crowd: tf.constant([False, True, False]),
                fields.InputDataFields.groundtruth_difficult: tf.constant(np.array([0, 0, 1], np.int32)),
            }
            ...
            return (
                ...
                transformed_inputs[fields.InputDataFields.groundtruth_keypoint_visibilities],
        ...
        self.assertAllEqual(groundtruth_keypoint_visibilities, [[True, True], [True, True]])
    ...
    def test_returns_correct_merged_boxes(self):
        ...
        (groundtruth_boxes, groundtruth_classes, groundtruth_confidences, num_groundtruth_boxes) = self.execute_cpu(
            graph_fn, []
        )
    ...
    def test_returns_correct_groundtruth_confidences_when_input_present(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([3, 1], np.int32)),
                fields.InputDataFields.groundtruth_confidences: tf.constant(np.array([1.0, -1.0], np.float32)),
            }
            num_classes = 3
            input_transformation_fn = functools.partial(
                inputs.transform_input_data,
    ...
    def test_returns_resized_masks(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([3, 1], np.int32)),
                fields.InputDataFields.original_image_spatial_shape: tf.constant(np.array([4, 4], np.int32)),
            }
            ...
            def fake_image_resizer_fn(image, masks=None):
                ...
                if masks is not None:
                    resized_masks = tf.transpose(
                        tf.image.resize_images(tf.transpose(masks, [1, 2, 0]), [8, 8]), [2, 0, 1]
                    )
        ...
        (original_image, original_image_shape, groundtruth_instance_masks) = self.execute_cpu(graph_fn, [])
    ...
    def test_resize_with_padding(self):
        def graph_fn():
            tensor_dict = {
                fields.InputDataFields.image: tf.constant(np.random.rand(100, 50, 3).astype(np.float32)),
                fields.InputDataFields.groundtruth_boxes: tf.constant(
                    np.array([[0.5, 0.5, 1, 1], [0.0, 0.0, 0.5, 0.5]], np.float32)
                ),
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([1, 2], np.int32)),
                fields.InputDataFields.groundtruth_keypoints: tf.constant([[[0.1, 0.2]], [[0.3, 0.4]]]),
            }
        ...
        self.assertAllClose(groundtruth_boxes, [[0.5, 0.25, 1.0, 0.5], [0.0, 0.0, 0.5, 0.25]])
    ...
    def test_groundtruth_keypoint_weights(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_keypoint_visibilities: tf.constant([[True, False], [True, True]]),
            }
        ...
        groundtruth_keypoints, groundtruth_keypoint_weights = self.execute_cpu(graph_fn, [])
        self.assertAllClose(groundtruth_keypoints, [[[0.1, 0.1], [0.3, 0.2]], [[0.5, 0.3], [0.7, 0.4]]])
    ...
    def test_groundtruth_dense_pose(self):
        def graph_fn():
            tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_classes: tf.constant(np.array([1, 2], np.int32)),
                fields.InputDataFields.groundtruth_dp_num_points: tf.constant([0, 2], dtype=tf.int32),
                fields.InputDataFields.groundtruth_dp_part_ids: tf.constant([[0, 0], [4, 23]], dtype=tf.int32),
            ...
            transformed_inputs = input_transformation_fn(tensor_dict=tensor_dict)
            transformed_dp_num_points = transformed_inputs[fields.InputDataFields.groundtruth_dp_num_points]
            transformed_dp_part_ids = transformed_inputs[fields.InputDataFields.groundtruth_dp_part_ids]
            transformed_dp_surface_coords = transformed_inputs[fields.InputDataFields.groundtruth_dp_surface_coords]
            return (transformed_dp_num_points, transformed_dp_part_ids, transformed_dp_surface_coords)
...
class PadInputDataToStaticShapesFnTest(test_case.TestCase):
    def test_pad_images_boxes_and_classes(self):
        input_tensor_dict = {
            ...
            fields.InputDataFields.groundtruth_classes: tf.random.uniform([2, 3], minval=0, maxval=2, dtype=tf.int32),
        ...
        padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
            tensor_dict=input_tensor_dict, max_num_boxes=3, num_classes=3, spatial_image_shape=[5, 6]
        )
        ...
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.original_image_spatial_shape].shape.as_list(), [2]
        )
    ...
    def test_clip_boxes_and_classes(self):
        def graph_fn():
            input_tensor_dict = {
                ...
                fields.InputDataFields.groundtruth_classes: tf.random.uniform([2, 3], maxval=10, dtype=tf.int32),
        ...
        (groundtruth_boxes, groundtruth_classes, num_groundtruth_boxes) = self.execute_cpu(graph_fn, [])
    ...
    def test_images_and_additional_channels(self):
        input_tensor_dict = {
            ...
            fields.InputDataFields.image_additional_channels: test_utils.image_with_dynamic_shape(4, 3, 2),
        }
        padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
            tensor_dict=input_tensor_dict, max_num_boxes=3, num_classes=3, spatial_image_shape=[5, 6]
        )
        ...
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.image_additional_channels].shape.as_list(), [5, 6, 2]
        )

    def test_images_and_additional_channels_errors(self):
        input_tensor_dict = {
            fields.InputDataFields.image: test_utils.image_with_dynamic_shape(10, 10, 3),
            fields.InputDataFields.image_additional_channels: test_utils.image_with_dynamic_shape(10, 10, 2),
            fields.InputDataFields.original_image: test_utils.image_with_dynamic_shape(10, 10, 3),
        }
    ...
    def test_gray_images_and_additional_channels(self):
        input_tensor_dict = {
            ...
            fields.InputDataFields.image_additional_channels: test_utils.image_with_dynamic_shape(4, 4, 2),
        }
        # pad_input_data_to_static_shape assumes that image is already concatenated
        # with additional channels.
        padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
    ...
    def test_keypoints(self):
        ...
        visibilities = tf.cast(tf.random.uniform(tf.shape(keypoints)[:-1], minval=0, maxval=2, dtype=tf.int32), tf.bool)
        input_tensor_dict = {
            fields.InputDataFields.groundtruth_keypoints: test_utils.keypoints_with_dynamic_shape(10, 16, 4),
        ...
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.groundtruth_keypoints].shape.as_list(), [3, 16, 4]
        )
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.groundtruth_keypoint_visibilities].shape.as_list(), [3, 16]
        )
    ...
    def test_dense_pose(self):
        ...
        self.assertAllEqual(padded_tensor_dict[fields.InputDataFields.groundtruth_dp_num_points].shape.as_list(), [3])
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.groundtruth_dp_part_ids].shape.as_list(), [3, 200]
        )
        self.assertAllEqual(
            padded_tensor_dict[fields.InputDataFields.groundtruth_dp_surface_coords].shape.as_list(), [3, 200, 4]
        )

    def test_pad_input_data_to_static_shapes_for_trackid(self):
        input_tensor_dict = {
            fields.InputDataFields.groundtruth_track_ids: tf.constant([0, 1], dtype=tf.int32),
        }

        padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
            tensor_dict=input_tensor_dict, max_num_boxes=3, num_classes=1, spatial_image_shape=[128, 128]
        )

        self.assertAllEqual(padded_tensor_dict[fields.InputDataFields.groundtruth_track_ids].shape.as_list(), [3])
    ...
    def test_context_features(self):
        ...
        def graph_fn():
            input_tensor_dict = {
                fields.InputDataFields.context_features: tf.ones([context_memory_size, context_feature_length]),
                fields.InputDataFields.context_feature_length: tf.constant(context_feature_length),
            }
            padded_tensor_dict = inputs.pad_input_data_to_static_shapes(
                tensor_dict=input_tensor_dict,
                max_num_boxes=3,
            ...
            self.assertAllEqual(
                padded_tensor_dict[fields.InputDataFields.context_features].shape.as_list(),
...
class NegativeSizeTest(test_case.TestCase):
    ...
    def test_negative_size_error(self):
        ...
        def graph_fn():
            tensors = {
                ...
                fields.InputDataFields.groundtruth_classes: tf.constant([1, 1], tf.int32),
                fields.InputDataFields.groundtruth_boxes: tf.constant([[0.5, 0.5, 0.4, 0.5]], tf.float32),
            }
            tensors = inputs.transform_input_data(
                tensors, _fake_model_preprocessor_fn, _fake_image_resizer_fn, num_classes=10
            )
    ...
    def test_negative_size_no_assert(self):
        ...
        with mock.patch.object(tf, "Assert") as tf_assert:
            ...
            self.assertAllClose(tensors[fields.InputDataFields.groundtruth_boxes], [[0.5, 0.5, 0.6, 0.6]])
