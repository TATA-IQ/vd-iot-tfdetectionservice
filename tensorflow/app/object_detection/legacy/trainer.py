...
def train(
    ...
    with tf.Graph().as_default():
        ...
        if num_clones != 1 and train_config.sync_replicas:
            raise ValueError(
                "In Synchronous SGD mode num_clones must ", "be 1. Found num_clones: {}".format(num_clones)
            )
        ...
        with tf.device(deploy_config.optimizer_device()):
            ...
            if train_config.bias_grad_multiplier:
                ...
                grads_and_vars = variables_helper.multiply_gradients_matching_regex(
                    grads_and_vars, biases_regex_list, multiplier=train_config.bias_grad_multiplier
                )
