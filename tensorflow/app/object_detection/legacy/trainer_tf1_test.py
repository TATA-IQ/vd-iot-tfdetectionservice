...
def get_input_function():
    ...
    multiclass_scores = tf.random_uniform([1, NUMBER_OF_CLASSES], minval=0.4, maxval=0.6, dtype=tf.float32)
...
class FakeDetectionModel(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        return {
            "class_predictions_with_background": tf.reshape(class_prediction, [-1, 1, self._num_classes]),
    ...
    def loss(self, prediction_dict, true_image_shapes):
        ...
        cls_losses = self._classification_loss(
            prediction_dict["class_predictions_with_background"], batch_cls_targets, weights=weights
        )
