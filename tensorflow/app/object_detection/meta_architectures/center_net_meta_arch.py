...
from object_detection.core import box_list, box_list_ops, keypoint_ops, model
...
def make_prediction_net(num_out_channels, kernel_size=3, num_filters=256, bias_fill=None):
    ...
    net = tf.keras.Sequential(
        [tf.keras.layers.Conv2D(num_filters, kernel_size=kernel_size, padding="same"), tf.keras.layers.ReLU(), out_conv]
    )
...
def top_k_feature_map_locations(feature_map, max_pool_kernel_size=3, k=100, per_channel=False):
    ...
    # array.
    y_indices, x_indices, channel_indices = row_col_channel_indices_from_flattened_indices(
...
def prediction_tensors_to_boxes(
    detection_scores, y_indices, x_indices, channel_indices, height_width_predictions, offset_predictions
):
...
def prediction_tensors_to_keypoint_candidates(
    ...
    # the keypoint type.
    keypoint_scores, y_indices, x_indices, channel_indices = top_k_feature_map_locations(
        keypoint_heatmap_predictions, max_pool_kernel_size=max_pool_kernel_size, k=max_candidates, per_channel=True
    )
...
def regressed_keypoints_at_object_centers(regressed_keypoint_predictions, y_indices, x_indices):
    ...
    absolute_regressed_keypoints = tf.stack(
        [y_indices + relative_regressed_keypoints_y, x_indices + relative_regressed_keypoints_x], axis=3
    )
...
def refine_keypoints(
    ...
    sqrd_distances = tf.math.reduce_sum(
        tf.math.squared_difference(regressed_keypoint_expanded, keypoint_candidates_expanded), axis=-1
    )
    ...
    nearby_candidate_coords, nearby_candidate_scores = _gather_candidates_at_indices(
        keypoint_candidates, keypoint_scores, nearby_candidate_inds
    )
    ...
    mask = (
        ...
        # keypoint score.
        tf.cast(nearby_candidate_scores < unmatched_keypoint_score, tf.int32)
    ...
    refined_keypoints = tf.where(
        tf.tile(tf.expand_dims(mask, -1), [1, 1, 1, 2]), regressed_keypoints, nearby_candidate_coords
    )
    ...
    refined_scores = tf.where(
        mask, unmatched_keypoint_score * tf.ones_like(nearby_candidate_scores), nearby_candidate_scores
    )
...
def _pad_to_full_keypoint_dim(keypoint_coords, keypoint_scores, keypoint_inds, num_total_keypoints):
    ...
    kpt_scores_scattered = tf.scatter_nd(
        indices=kpt_inds_tensor, updates=kpt_scores_transposed, shape=[num_total_keypoints, batch_size, num_instances]
    )
...
def _pad_to_full_instance_dim(keypoint_coords, keypoint_scores, instance_inds, max_instances):
    ...
    kpt_coords_scattered = tf.scatter_nd(
        indices=instance_inds, updates=kpt_coords_transposed, shape=[max_instances, batch_size, num_keypoints, 2]
    )
    kpt_scores_scattered = tf.scatter_nd(
        indices=instance_inds, updates=kpt_scores_transposed, shape=[max_instances, batch_size, num_keypoints]
    )
...
def _gather_candidates_at_indices(keypoint_candidates, keypoint_scores, indices):
    ...
    nearby_candidate_coords_tranposed = tf.gather(
        keypoint_candidates_transposed, nearby_candidate_inds_transposed, batch_dims=2
    )
    nearby_candidate_scores_transposed = tf.gather(
        keypoint_scores_transposed, nearby_candidate_inds_transposed, batch_dims=2
    )
...
def convert_strided_predictions_to_normalized_keypoints(
    keypoint_coords, keypoint_scores, stride, true_image_shapes, clip_out_of_frame_keypoints=False
):
    ...
    if clip_out_of_frame_keypoints:
        ...
        valid_indices = tf.logical_and(
            tf.logical_and(
                keypoint_coords_normalized[:, :, :, 0] >= 0.0, keypoint_coords_normalized[:, :, :, 0] <= 1.0
            ),
            tf.logical_and(
                keypoint_coords_normalized[:, :, :, 1] >= 0.0, keypoint_coords_normalized[:, :, :, 1] <= 1.0
            ),
        )
        ...
        keypoint_coords_normalized = tf.map_fn(
            clip_to_window, (keypoint_coords_normalized, batch_window), dtype=tf.float32, back_prop=False
        )
...
def convert_strided_predictions_to_instance_masks(
    ...
    if (densepose_part_heatmap is not None) ^ (densepose_surface_coords is not None):
        raise ValueError(
            "To use DensePose, both `densepose_part_heatmap` and `densepose_surface_coords` must be provided"
        )
    ...
    instance_masks, surface_coords = shape_utils.static_or_dynamic_map_fn(
        ...
        elems=[boxes, classes, masks, densepose_part_heatmap, densepose_surface_coords, true_heights, true_widths],
...
def crop_and_threshold_masks(
    elems, input_height, input_width, mask_height=256, mask_width=256, score_threshold=0.5, densepose_class_index=-1
):
    ...
    cropped_masks = tf2.image.crop_and_resize(
        feature_maps_concat, boxes=boxes, box_indices=classes, crop_size=[mask_height, mask_width], method="bilinear"
    )
    ...
    part_mask_int_zero_indexed = tf.math.argmax(part_heatmap_cropped, axis=-1, output_type=tf.int32)[
        :, :, :, tf.newaxis
    ]
...
class ObjectDetectionParams(
    collections.namedtuple(
        "ObjectDetectionParams", ["localization_loss", "scale_loss_weight", "offset_loss_weight", "task_loss_weight"]
    )
):
    ...
    def __new__(cls, localization_loss, scale_loss_weight, offset_loss_weight, task_loss_weight=1.0):
        ...
        return super(ObjectDetectionParams, cls).__new__(
            cls, localization_loss, scale_loss_weight, offset_loss_weight, task_loss_weight
        )
...
class MaskParams(
    ...
    def __new__(
        ...
        return super(MaskParams, cls).__new__(
            cls, classification_loss, task_loss_weight, mask_height, mask_width, score_threshold, heatmap_bias_init
        )
...
class TrackParams(
    collections.namedtuple(
        "TrackParams", ["num_track_ids", "reid_embed_size", "num_fc_layers", "classification_loss", "task_loss_weight"]
    )
):
    ...
    def __new__(cls, num_track_ids, reid_embed_size, num_fc_layers, classification_loss, task_loss_weight=1.0):
        ...
        return super(TrackParams, cls).__new__(
            cls, num_track_ids, reid_embed_size, num_fc_layers, classification_loss, task_loss_weight
        )


class TemporalOffsetParams(collections.namedtuple("TemporalOffsetParams", ["localization_loss", "task_loss_weight"])):
...
class CenterNetMetaArch(model.DetectionModel):
    ...
    def __init__(
        ...
        self._prediction_head_dict = self._construct_prediction_heads(
            num_classes, self._num_feature_outputs, class_prediction_bias_init=self._center_params.heatmap_bias_init
        )
        # Initialize the target assigners.
        self._target_assigner_dict = self._initialize_target_assigners(
            stride=self._stride, min_box_overlap_iou=self._center_params.min_box_overlap_iou
        )
    ...
    def _construct_prediction_heads(self, num_classes, num_feature_outputs, class_prediction_bias_init):
        ...
        prediction_heads[OBJECT_CENTER] = [
            make_prediction_net(num_classes, bias_fill=class_prediction_bias_init) for _ in range(num_feature_outputs)
        ]
        if self._od_params is not None:
            ...
            prediction_heads[BOX_OFFSET] = [
                make_prediction_net(NUM_OFFSET_CHANNELS) for _ in range(num_feature_outputs)
            ]
        if self._kp_params_dict is not None:
            for task_name, kp_params in self._kp_params_dict.items():
                ...
                prediction_heads[get_keypoint_name(task_name, KEYPOINT_HEATMAP)] = [
                    make_prediction_net(num_keypoints, bias_fill=kp_params.heatmap_bias_init)
                    for _ in range(num_feature_outputs)
                ]
                prediction_heads[get_keypoint_name(task_name, KEYPOINT_REGRESSION)] = [
                    make_prediction_net(NUM_OFFSET_CHANNELS * num_keypoints) for _ in range(num_feature_outputs)
                ]
        ...
        if self._mask_params is not None:
            prediction_heads[SEGMENTATION_HEATMAP] = [
                make_prediction_net(num_classes, bias_fill=self._mask_params.heatmap_bias_init)
                for _ in range(num_feature_outputs)
            ]
        if self._densepose_params is not None:
            prediction_heads[DENSEPOSE_HEATMAP] = [
                make_prediction_net(  # pylint: disable=g-complex-comprehension
                    self._densepose_params.num_parts, bias_fill=self._densepose_params.heatmap_bias_init
                )
                for _ in range(num_feature_outputs)
            ]
            prediction_heads[DENSEPOSE_REGRESSION] = [
                make_prediction_net(2 * self._densepose_params.num_parts) for _ in range(num_feature_outputs)
            ]
        if self._track_params is not None:
            prediction_heads[TRACK_REID] = [
                make_prediction_net(self._track_params.reid_embed_size) for _ in range(num_feature_outputs)
            ]
            ...
            for _ in range(self._track_params.num_fc_layers - 1):
                self.track_reid_classification_net.add(
                    tf.keras.layers.Dense(
                        self._track_params.reid_embed_size, input_shape=(self._track_params.reid_embed_size,)
                    )
                )
            ...
            self.track_reid_classification_net.add(
                tf.keras.layers.Dense(
                    self._track_params.num_track_ids, input_shape=(self._track_params.reid_embed_size,)
                )
            )
    ...
    def _initialize_target_assigners(self, stride, min_box_overlap_iou):
        ...
        if self._kp_params_dict is not None:
            for task_name, kp_params in self._kp_params_dict.items():
                target_assigners[task_name] = cn_assigner.CenterNetKeypointTargetAssigner(
                    stride=stride,
    ...
    def _compute_object_center_loss(self, input_height, input_width, object_center_predictions, per_pixel_weights):
        ...
        if self._center_params.use_only_known_classes:
            ...
            batch_labeled_classes = tf.reshape(
                batch_labeled_classes, [batch_labeled_classes_shape[0], 1, batch_labeled_classes_shape[-1]]
            )
    ...
    def _compute_box_scale_and_offset_loss(self, input_height, input_width, scale_predictions, offset_predictions):
        ...
        (
            ...
            height=input_height, width=input_width, gt_boxes_list=gt_boxes_list, gt_weights_list=gt_weights_list
        )
    ...
    def _compute_kp_heatmap_loss(
        self, input_height, input_width, task_name, heatmap_predictions, classification_loss_fn, per_pixel_weights
    ):
        ...
        (keypoint_heatmap, num_instances_per_kp_type, valid_mask_batch) = assigner.assign_keypoint_heatmap_targets(
            height=input_height,
            width=input_width,
    ...
    def _compute_kp_offset_loss(self, input_height, input_width, task_name, offset_predictions, localization_loss_fn):
        ...
        assigner = self._target_assigner_dict[task_name]
        (batch_indices, batch_offsets, batch_weights) = assigner.assign_keypoints_offset_targets(
            height=input_height,
            width=input_width,
            gt_keypoints_list=gt_keypoints_list,
            gt_weights_list=gt_weights_list,
        ...
        for prediction in offset_predictions:
            ...
            unweighted_loss = localization_loss_fn(
                prediction, batch_offsets, weights=tf.expand_dims(tf.ones_like(batch_weights), -1)
            )
            # Apply the weights after the loss function to have full control over it.
    ...
    def _compute_kp_regression_loss(
        self, input_height, input_width, task_name, regression_predictions, localization_loss_fn
    ):
        ...
        assigner = self._target_assigner_dict[task_name]
        (batch_indices, batch_regression_offsets, batch_weights) = assigner.assign_joint_regression_targets(
            height=input_height,
            width=input_width,
            gt_keypoints_list=gt_keypoints_list,
            gt_classes_list=gt_classes_list,
        ...
        for prediction in regression_predictions:
            ...
            unweighted_loss = localization_loss_fn(
                reg_prediction, batch_regression_offsets, weights=tf.expand_dims(tf.ones_like(batch_weights), -1)
            )
            # Apply the weights after the loss function to have full control over it.
            loss += batch_weights * tf.reduce_sum(unweighted_loss, axis=1)

        loss = tf.reduce_sum(loss) / (
            float(len(regression_predictions)) * tf.maximum(tf.reduce_sum(batch_weights), 1.0)
        )
        return loss
    ...
    def _compute_mask_loss(self, segmentation_predictions, per_pixel_weights):
        ...
        heatmap_targets = assigner.assign_segmentation_targets(
            gt_masks_list=gt_masks_list, gt_classes_list=gt_classes_list
        )
    ...
    def _compute_densepose_losses(self, input_height, input_width, prediction_dict):
        ...
        """
        dp_heatmap_loss, dp_regression_loss = self._compute_densepose_part_and_coordinate_losses(
            input_height=input_height,
            input_width=input_width,
    ...
    def _compute_densepose_part_and_coordinate_losses(
        ...
        for part_pred, surface_coord_pred in zip(part_predictions, surface_coord_predictions):
            ...
            surface_coord_loss += localization_loss_fn(
                surface_coord_pred, batch_surface_coords, weights=batch_weights[:, tf.newaxis]
            )
    ...
    def _compute_track_losses(self, input_height, input_width, prediction_dict):
        ...
        embedding_loss = self._compute_track_embedding_loss(
            input_height=input_height, input_width=input_width, object_reid_predictions=object_reid_predictions
        )
    ...
    def _compute_temporal_offset_loss(self, input_height, input_width, prediction_dict):
        ...
        (batch_indices, batch_offset_targets, batch_weights) = assigner.assign_temporal_offset_targets(
            height=input_height,
            width=input_width,
            gt_boxes_list=gt_boxes_list,
    ...
    def preprocess(self, inputs):
        outputs = shape_utils.resize_images_and_return_shapes(inputs, self._image_resizer_fn)
    ...
    def loss(self, prediction_dict, true_image_shapes, scope=None):
        ...
        if self._densepose_params is not None:
            densepose_losses = self._compute_densepose_losses(
                input_height=input_height, input_width=input_width, prediction_dict=prediction_dict
            )
    ...
    def postprocess(self, prediction_dict, true_image_shapes, **params):
        ...
        # center predictions.
        detection_scores, y_indices, x_indices, channel_indices = top_k_feature_map_locations(
            object_center_prob, max_pool_kernel_size=3, k=self._center_params.max_box_predictions
        )

        boxes_strided, classes, scores, num_detections = prediction_tensors_to_boxes(
            detection_scores,
            y_indices,
            x_indices,
            channel_indices,
        ...
        if self._kp_params_dict:
            keypoints, keypoint_scores = self._postprocess_keypoints(
                prediction_dict, classes, y_indices, x_indices, boxes_strided, num_detections
            )
            keypoints, keypoint_scores = convert_strided_predictions_to_normalized_keypoints(
                keypoints, keypoint_scores, self._stride, true_image_shapes, clip_out_of_frame_keypoints=True
            )
        ...
        for ex_ind in range(batch_size):
            ...
            for task_name, kp_params in self._kp_params_dict.items():
                ...
                def true_fn(
                    ...
                    kpts_coords_for_class_padded, kpt_scores_for_class_padded = _pad_to_full_keypoint_dim(
                        kpt_coords_for_class, kpt_scores_for_class, kp_params.keypoint_indices, total_num_keypoints
                    )
            ...
            if tf.size(instance_inds_for_example) > 0:
                ...
                kpt_coords_for_example_all_det, kpt_scores_for_example_all_det = _pad_to_full_instance_dim(
    ...
    def _get_instance_indices(self, classes, num_detections, batch_index, class_id):
        ...
        valid_detections_with_kpt_class = tf.math.logical_and(
            tf.range(max_detections) < num_detections[batch_index], classes[0] == class_id
        )
    ...
    def restore_from_objects(self, fine_tune_checkpoint_type="detection"):
        ...
        if fine_tune_checkpoint_type not in supported_types:
            ...
            raise ValueError(
                message.format(fine_tune_checkpoint_type, self._feature_extractor.__class__.__name__, supported_types)
            )
    ...
    def updates(self):
        raise RuntimeError("This model is intended to be used with model_lib_v2 which does not support updates()")
