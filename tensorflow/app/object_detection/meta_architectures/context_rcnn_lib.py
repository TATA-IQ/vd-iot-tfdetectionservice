...
def filter_weight_value(weights, values, valid_mask):
    ...
    if w_batch_size != v_batch_size or v_batch_size != m_batch_size:
        raise ValueError("Please make sure the first dimension of the input tensors are the same.")

    if w_context_size != v_context_size:
        raise ValueError("Please make sure the third dimension of weights matches the second dimension of values.")

    if w_context_size != m_context_size:
        raise ValueError(
            "Please make sure the third dimension of the weights matches the second dimension of the valid_mask."
        )
    ...
    weights += tf.transpose(
        tf.cast(tf.math.logical_not(valid_mask), weights.dtype) * _NEGATIVE_PADDING_VALUE, perm=[0, 2, 1]
    )
...
def project_features(features, projection_dimension, is_training, normalize):
    ...
    projected_features = tf.reshape(projected_features, [batch_size, -1, projection_dimension])
...
def attention_block(
    ...
    with tf.variable_scope("AttentionBlock"):
        queries = project_features(input_features, bottleneck_dimension, is_training, normalize=True)
        keys = project_features(context_features, bottleneck_dimension, is_training, normalize=True)
        values = project_features(context_features, bottleneck_dimension, is_training, normalize=True)
    ...
    output_features = project_features(features, output_dimension, is_training, normalize=False)
    return output_features


def compute_box_context_attention(
    box_features, context_features, valid_context_size, bottleneck_dimension, attention_temperature, is_training
):
