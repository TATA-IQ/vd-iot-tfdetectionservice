...
from object_detection.utils import ops, shape_utils, variables_helper
...
class FasterRCNNFeatureExtractor(object):
    ...
    def __init__(
        self, is_training, first_stage_features_stride, batch_norm_trainable=False, reuse_weights=None, weight_decay=0.0
    ):
        """Constructor.
...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    def __init__(
        ...
        if isinstance(first_stage_box_predictor_arg_scope_fn, hyperparams_builder.KerasLayerHyperparams):
            ...
            self._first_stage_box_predictor_first_conv = tf.keras.Sequential(
                [
                    ...
                    conv_hyperparams.build_batch_norm(
                        (self._is_training and not freeze_batchnorm), name="RPNBatchNorm"
                    ),
                    tf.keras.layers.Lambda(tf.nn.relu6, name="RPNActivation"),
                ],
                name="FirstStageRPNFeatures",
            )
            self._first_stage_box_predictor = box_predictor_builder.build_convolutional_keras_box_predictor(
            ...
            self._first_stage_box_predictor = box_predictor_builder.build_convolutional_box_predictor(
                is_training=self._is_training,
                num_classes=1,
        ...
        if maxpool_kernel_size:
            self._maxpool_layer = tf.keras.layers.MaxPooling2D(
                [self._maxpool_kernel_size, self._maxpool_kernel_size], strides=self._maxpool_stride, name="MaxPool2D"
            )
    ...
    def preprocess(self, inputs):
        ...
        with tf.name_scope("Preprocessor"):
            (resized_inputs, true_image_shapes) = shape_utils.resize_images_and_return_shapes(
    ...
    def _compute_clip_window(self, image_shapes):
        ...
        clip_window = tf.cast(
            tf.stack([tf.zeros_like(clip_heights), tf.zeros_like(clip_heights), clip_heights, clip_widths], axis=1),
            dtype=tf.float32,
        )
        return clip_window

    def _proposal_postprocess(
        self, rpn_box_encodings, rpn_objectness_predictions_with_background, anchors, image_shape, true_image_shapes
    ):
        ...
        proposal_boxes_normalized, _, _, num_proposals, _, _ = self._postprocess_rpn(
            rpn_box_encodings, rpn_objectness_predictions_with_background, anchors, image_shape_2d, true_image_shapes
        )
    ...
    def _predict_first_stage(self, preprocessed_inputs):
        ...
        (rpn_box_encodings, rpn_objectness_predictions_with_background) = self._predict_rpn_proposals(
            rpn_box_predictor_features
        )
        ...
        if self._is_training:
            if self.clip_anchors_to_image:
                ...
                (
                    ...
                    rpn_box_encodings, rpn_objectness_predictions_with_background, anchors_boxlist, clip_window
                )
        else:
            anchors_boxlist = box_list_ops.clip_to_window(
                anchors_boxlist, clip_window, filter_nonoverlapping=not self._use_static_shapes
            )
    ...
    def _predict_second_stage(
        ...
        **side_inputs,
        ...
        proposal_boxes_normalized, num_proposals = self._proposal_postprocess(
            rpn_box_encodings, rpn_objectness_predictions_with_background, anchors, image_shape, true_image_shapes
        )
        prediction_dict = self._box_prediction(
            rpn_features_to_crop, proposal_boxes_normalized, image_shape, true_image_shapes, **side_inputs
        )
        prediction_dict["num_proposals"] = num_proposals
        return prediction_dict

    def _box_prediction(
        self, rpn_features_to_crop, proposal_boxes_normalized, image_shape, true_image_shapes, **side_inputs
    ):
        """Predicts the output tensors from second stage of Faster R-CNN.
        ...
        refined_box_encodings = tf.squeeze(
            box_predictions[box_predictor.BOX_ENCODINGS], axis=1, name="all_refined_box_encodings"
        )
    ...
    def _raw_detections_and_feature_map_inds(self, refined_box_encodings, absolute_proposal_boxes, true_image_shapes):
        ...
        box_encodings_batch = tf.reshape(
            ...
            [-1, self.max_num_proposals, refined_box_encodings.shape[1], self._box_coder.code_size],
        )
        ...
        raw_detection_boxes_normalized = shape_utils.static_or_dynamic_map_fn(
            self._normalize_and_clip_boxes, elems=[raw_detection_boxes_absolute, true_image_shapes], dtype=tf.float32
        )
    ...
    def _extract_box_classifier_features(self, flattened_feature_maps):
        ...
        if self._feature_extractor_for_box_classifier_features:
            ...
            box_classifier_features = self._feature_extractor.extract_box_classifier_features(
                flattened_feature_maps, scope=self.second_stage_feature_extractor_scope
            )
    ...
    def _predict_third_stage(self, prediction_dict, image_shapes):
        ...
        if self._is_training:
            ...
            flattened_detected_feature_maps = self._compute_second_stage_input_feature_maps(
            ...
            prediction_dict[fields.DetectionResultFields.detection_masks] = tf.reshape(
                tf.sigmoid(detection_masks), [batch_size, max_detection, mask_height, mask_width]
            )
    ...
    def _extract_rpn_feature_maps(self, preprocessed_inputs):
        ...
        for single_rpn_features_to_crop in rpn_features_to_crop:
            ...
            single_rpn_box_predictor_features = self._first_stage_box_predictor_first_conv(single_rpn_features_to_crop)
            rpn_box_predictor_features.append(single_rpn_box_predictor_features)
        anchors = box_list_ops.concatenate(self._first_stage_anchor_generator.generate(feature_map_shapes))
    ...
    def _predict_rpn_proposals(self, rpn_box_predictor_features):
        ...
        if self._first_stage_box_predictor.is_keras_model:
            ...
            box_predictions = self._first_stage_box_predictor.predict(
                rpn_box_predictor_features, num_anchors_per_location, scope=self.first_stage_box_predictor_scope
            )
    ...
    def _remove_invalid_anchors_and_predictions(
        self, box_encodings, objectness_predictions_with_background, anchors_boxlist, clip_window
    ):
    ...
    def postprocess(self, prediction_dict, true_image_shapes):
        ...
        with tf.name_scope("FirstStagePostprocessor"):
            if self._number_of_stages == 1:
                image_shapes = self._image_batch_shape_2d(prediction_dict["image_shape"])
                (
                    proposal_boxes,
                    proposal_scores,
        ...
        if self._number_of_stages == 2 or (self._number_of_stages == 3 and self._is_training):
            ...
            if self._output_final_box_features:
                if "rpn_features_to_crop" not in prediction_dict:
                    raise ValueError("Please make sure rpn_features_to_crop is in the prediction_dict.")
                detections_dict["detection_features"] = self._add_detection_features_output_node(
    ...
    def _add_detection_features_output_node(self, detection_boxes, rpn_features_to_crop, image_shape):
        ...
        with tf.name_scope("SecondStageDetectionFeaturesExtract"):
            ...
            reshaped_detection_features_pool = tf.reshape(
                detection_features_pool, [batch_size, max_detections, tf.shape(detection_features_pool)[-1]]
            )
    ...
    def _postprocess_rpn(
        ...
        (proposal_boxes, proposal_scores, _, _, nmsed_additional_fields, num_proposals) = self._first_stage_nms_fn(
        ...
        if self._is_training:
            ...
            if not self._hard_example_miner:
                ...
                (proposal_boxes, proposal_scores, num_proposals) = self._sample_box_classifier_batch(
                    proposal_boxes,
                    proposal_scores,
                    num_proposals,
                    groundtruth_boxlists,
        ...
        def normalize_boxes(args):
            ...
            normalized_boxes_per_image = box_list_ops.to_normalized_coordinates(
                box_list.BoxList(proposal_boxes_per_image), image_shape[0], image_shape[1], check_range=False
            ).get()
        ...
        proposal_multiclass_scores = (
            nmsed_additional_fields.get("multiclass_scores") if nmsed_additional_fields else None
        )
        return (
    ...
    def _format_groundtruth_data(self, image_shapes):
        ...
        groundtruth_boxlists = [
            box_list_ops.to_absolute_coordinates(box_list.BoxList(boxes), image_shapes[i, 0], image_shapes[i, 1])
        ...
        for one_hot_encoding in self.groundtruth_lists(fields.BoxListFields.classes):
            groundtruth_classes_with_background_list.append(
                tf.cast(tf.pad(one_hot_encoding, [[0, 0], [1, 0]], mode="CONSTANT"), dtype=tf.float32)
            )
    ...
    def _sample_box_classifier_minibatch_single_image(
        ...
        (cls_targets, cls_weights, _, _, _) = self._detector_target_assigner.assign(
            ...
            groundtruth_boxlist,
            groundtruth_classes_with_background,
            unmatched_class_label=tf.constant([1] + self._num_classes * [0], dtype=tf.float32),
    ...
    def _postprocess_box_classifier(
        ...
        class_predictions_with_background_batch = tf.reshape(
            class_predictions_with_background, [-1, self.max_num_proposals, self.num_classes + 1]
        )
        ...
        if mask_predictions is not None:
            ...
            mask_predictions_batch = tf.reshape(
                mask_predictions, [-1, self.max_num_proposals, self.num_classes, mask_height, mask_width]
            )
        ...
        if refined_decoded_boxes_batch.shape[2] > 1:
            class_ids = tf.expand_dims(
                tf.argmax(class_predictions_with_background_batch[:, :, 1:], axis=2, output_type=tf.int32), axis=-1
            )
        ...
        raw_normalized_detection_boxes = shape_utils.static_or_dynamic_map_fn(
            self._normalize_and_clip_boxes, elems=[raw_detection_boxes, image_shapes], dtype=tf.float32
        )
    ...
    def _normalize_and_clip_boxes(self, boxes_and_image_shape):
        ...
        normalized_boxes_per_image = box_list_ops.to_normalized_coordinates(
            box_list.BoxList(boxes_per_image), image_shape[0], image_shape[1], check_range=False
        ).get()
    ...
    def loss(self, prediction_dict, true_image_shapes, scope=None):
        ...
        with tf.name_scope(scope, "Loss", prediction_dict.values()):
            (
                ...
                groundtruth_masks_list,
                groundtruth_weights_list,
            ) = self._format_groundtruth_data(self._image_batch_shape_2d(prediction_dict["image_shape"]))
        ...
        with tf.name_scope("RPNLoss"):
            ...
            def _minibatch_subsample_fn(inputs):
                ...
                return self._first_stage_sampler.subsample(
                    tf.cast(cls_weights, tf.bool), self._first_stage_minibatch_size, tf.cast(cls_targets, tf.bool)
                )
            ...
            localization_losses = self._first_stage_localization_loss(
                rpn_box_encodings, batch_reg_targets, weights=sampled_reg_indices, losses_mask=losses_mask
            )
            ...
            localization_loss = tf.multiply(
                self._first_stage_loc_loss_weight, localization_loss, name="localization_loss"
            )
    ...
    def _loss_box_classifier(
        ...
        with tf.name_scope("BoxClassifierLoss"):
            ...
            num_proposals_or_one = tf.cast(
                tf.expand_dims(tf.maximum(num_proposals, tf.ones_like(num_proposals)), 1), dtype=tf.float32
            )
            ...
            if self.groundtruth_has_field(fields.InputDataFields.groundtruth_labeled_classes):
                gt_labeled_classes = self.groundtruth_lists(fields.InputDataFields.groundtruth_labeled_classes)
                gt_labeled_classes = tf.pad(gt_labeled_classes, [[0, 0], [1, 0]], mode="CONSTANT", constant_values=1)
                batch_cls_weights *= tf.expand_dims(gt_labeled_classes, 1)
            class_predictions_with_background = tf.reshape(
                class_predictions_with_background, [batch_size, self.max_num_proposals, -1]
            )

            flat_cls_targets_with_background = tf.reshape(
                batch_cls_targets_with_background, [batch_size * self.max_num_proposals, -1]
            )
            ...
            one_hot_flat_cls_targets_with_background = tf.one_hot(
                one_hot_flat_cls_targets_with_background, flat_cls_targets_with_background.get_shape()[1]
            )

            # If using a shared box across classes use directly
            if refined_box_encodings.shape[1] == 1:
                reshaped_refined_box_encodings = tf.reshape(
                    refined_box_encodings, [batch_size, self.max_num_proposals, self._box_coder.code_size]
                )
            # For anchors with multiple labels, picks refined_location_encodings
            # for just one class to avoid over-counting for regression loss and
            # (optionally) mask loss.
            else:
                reshaped_refined_box_encodings = self._get_refined_encodings_for_postitive_class(
                    refined_box_encodings, one_hot_flat_cls_targets_with_background, batch_size
                )
            ...
            if self._hard_example_miner:
                (second_stage_loc_loss, second_stage_cls_loss) = self._unpad_proposals_and_apply_hard_mining(
                    proposal_boxlists, second_stage_loc_losses, second_stage_cls_losses, num_proposals
                )
            localization_loss = tf.multiply(
                self._second_stage_loc_loss_weight, second_stage_loc_loss, name="localization_loss"
            )

            classification_loss = tf.multiply(
                self._second_stage_cls_loss_weight, second_stage_cls_loss, name="classification_loss"
            )
            ...
            if prediction_masks is not None:
                ...
                (batch_mask_targets, _, _, batch_mask_target_weights, _) = target_assigner.batch_assign_targets(
                    target_assigner=self._detector_target_assigner,
                    anchors_batch=proposal_boxlists,
                    gt_box_batch=groundtruth_boxlists,
                ...
                if prediction_masks.get_shape().as_list()[1] == 1:
                    ...
                    prediction_masks_masked_by_class_targets = tf.boolean_mask(
                        prediction_masks_with_background, tf.greater(one_hot_flat_cls_targets_with_background, 0)
                    )
                ...
                reshaped_prediction_masks = tf.reshape(
                    prediction_masks_masked_by_class_targets, [batch_size, -1, mask_height * mask_width]
                )

                batch_mask_targets_shape = tf.shape(batch_mask_targets)
                flat_gt_masks = tf.reshape(
                    batch_mask_targets, [-1, batch_mask_targets_shape[2], batch_mask_targets_shape[3]]
                )
    ...
    def _get_mask_proposal_boxes_and_classes(
        ...
        proposal_boxes = tf.reshape(
            box_list_ops.to_absolute_coordinates(
                box_list.BoxList(tf.reshape(detection_boxes, [-1, 4])), image_shape[1], image_shape[2]
            ).get(),
        ...
        (batch_cls_targets_with_background, _, _, _, _) = target_assigner.batch_assign_targets(
            target_assigner=self._detector_target_assigner,
            anchors_batch=proposal_boxlists,
            gt_box_batch=groundtruth_boxlists,
            gt_class_targets_batch=groundtruth_classes_with_background_list,
    ...
    def _get_refined_encodings_for_postitive_class(
        ...
        refined_box_encodings_masked_by_class_targets = box_list_ops.boolean_mask(
            box_list.BoxList(tf.reshape(refined_box_encodings_with_background, [-1, self._box_coder.code_size])),
            tf.reshape(tf.greater(flat_cls_targets_with_background, 0), [-1]),
            use_static_shapes=self._use_static_shapes,
            indicator_sum=batch_size * self.max_num_proposals if self._use_static_shapes else None,
        ).get()
        return tf.reshape(
    ...
    def _unpad_proposals_and_apply_hard_mining(
        self, proposal_boxlists, second_stage_loc_losses, second_stage_cls_losses, num_proposals
    ):
        ...
        """
        for proposal_boxlist, single_image_loc_loss, single_image_cls_loss, single_image_num_proposals in zip(
            proposal_boxlists,
            ...
            proposal_boxlist = box_list.BoxList(
                tf.slice(proposal_boxlist.get(), [0, 0], [single_image_num_proposals, -1])
            )
    ...
    def restore_map(self, fine_tune_checkpoint_type="detection", load_all_detection_checkpoint_vars=False):
        ...
        if fine_tune_checkpoint_type == "classification":
            return self._feature_extractor.restore_from_classification_checkpoint_fn(
                self.first_stage_feature_extractor_scope, self.second_stage_feature_extractor_scope
            )
        ...
        feature_extractor_variables = slim.filter_variables(variables_to_restore, include_patterns=include_patterns)
    ...
    def restore_from_objects(self, fine_tune_checkpoint_type="detection"):
        ...
        """
        if fine_tune_checkpoint_type == "classification":
            return {"feature_extractor": self._feature_extractor.classification_backbone}
