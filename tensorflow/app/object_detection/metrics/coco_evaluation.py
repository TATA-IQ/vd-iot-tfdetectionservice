...
from object_detection.utils import (json_utils, np_mask_ops,
...
class CocoDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        groundtruth_area = groundtruth_dict.get(standard_fields.InputDataFields.groundtruth_area)
        groundtruth_keypoints = groundtruth_dict.get(standard_fields.InputDataFields.groundtruth_keypoints)
        ...
        if groundtruth_keypoint_visibilities is not None and not groundtruth_keypoint_visibilities.shape[0]:
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        # Drop optional fields if empty tensor.
        detection_keypoints = detections_dict.get(standard_fields.DetectionResultFields.detection_keypoints)
        ...
        if self._skip_predictions_for_unlabeled_class:
            det_classes = detections_dict[standard_fields.DetectionResultFields.detection_classes]
    ...
    def dump_detections_to_json_file(self, json_output_path):
        ...
        if json_output_path and json_output_path is not None:
            with tf.gfile.GFile(json_output_path, "w") as fid:
                ...
                json_utils.Dump(obj=self._detection_boxes_list, fid=fid, float_digits=4, indent=2)
    ...
    def add_eval_dict(self, eval_dict):
        ...
        groundtruth_labeled_classes = eval_dict.get(input_data_fields.groundtruth_labeled_classes, None)
        ...
        if not image_id.shape.as_list():
            ...
            if num_gt_boxes_per_image is None:
                num_gt_boxes_per_image = tf.tile(
                    tf.shape(groundtruth_boxes)[1:2], multiples=tf.shape(groundtruth_boxes)[0:1]
                )
            if num_det_boxes_per_image is None:
                num_det_boxes_per_image = tf.tile(
                    tf.shape(detection_boxes)[1:2], multiples=tf.shape(detection_boxes)[0:1]
                )
            if is_annotated is None:
...
class CocoKeypointEvaluator(CocoDetectionEvaluator):
    ...
    def __init__(self, category_id, category_keypoints, class_text, oks_sigmas=None):
        ...
        if oks_sigmas:
            self._oks_sigmas = np.array([oks_sigmas[kpt_id_to_name[idx]] for idx in self._keypoint_ids])
        else:
            # Default all per-keypoint sigmas to 0.
            self._oks_sigmas = np.full((len(self._keypoint_ids)), 0.05)
            tf.logging.warning("No default keypoint OKS sigmas provided. Will use 0.05")
        tf.logging.info("Using the following keypoint OKS sigmas: {}".format(self._oks_sigmas))
        self._metrics = None
        super(CocoKeypointEvaluator, self).__init__([{"id": self._category_id, "name": class_text}])
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        groundtruth_classes = groundtruth_dict[standard_fields.InputDataFields.groundtruth_classes]
        groundtruth_boxes = groundtruth_dict[standard_fields.InputDataFields.groundtruth_boxes]
        groundtruth_keypoints = groundtruth_dict[standard_fields.InputDataFields.groundtruth_keypoints]
        class_indices = [idx for idx, gt_class_id in enumerate(groundtruth_classes) if gt_class_id == self._category_id]
        filtered_groundtruth_classes = np.take(groundtruth_classes, class_indices, axis=0)
        filtered_groundtruth_boxes = np.take(groundtruth_boxes, class_indices, axis=0)
        filtered_groundtruth_keypoints = np.take(groundtruth_keypoints, class_indices, axis=0)
        filtered_groundtruth_keypoints = np.take(filtered_groundtruth_keypoints, self._keypoint_ids, axis=1)

        filtered_groundtruth_dict = {}
        filtered_groundtruth_dict[standard_fields.InputDataFields.groundtruth_classes] = filtered_groundtruth_classes
        filtered_groundtruth_dict[standard_fields.InputDataFields.groundtruth_boxes] = filtered_groundtruth_boxes
        filtered_groundtruth_dict[
            standard_fields.InputDataFields.groundtruth_keypoints
        ] = filtered_groundtruth_keypoints

        if standard_fields.InputDataFields.groundtruth_is_crowd in groundtruth_dict.keys():
            groundtruth_is_crowd = groundtruth_dict[standard_fields.InputDataFields.groundtruth_is_crowd]
            filtered_groundtruth_is_crowd = np.take(groundtruth_is_crowd, class_indices, 0)
            filtered_groundtruth_dict[
                standard_fields.InputDataFields.groundtruth_is_crowd
            ] = filtered_groundtruth_is_crowd
        if standard_fields.InputDataFields.groundtruth_area in groundtruth_dict.keys():
            groundtruth_area = groundtruth_dict[standard_fields.InputDataFields.groundtruth_area]
            filtered_groundtruth_area = np.take(groundtruth_area, class_indices, 0)
            filtered_groundtruth_dict[standard_fields.InputDataFields.groundtruth_area] = filtered_groundtruth_area
        if standard_fields.InputDataFields.groundtruth_keypoint_visibilities in groundtruth_dict.keys():
        ...
        super(CocoKeypointEvaluator, self).add_single_ground_truth_image_info(image_id, filtered_groundtruth_dict)
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        detection_boxes = detections_dict[standard_fields.DetectionResultFields.detection_boxes]
        detection_scores = detections_dict[standard_fields.DetectionResultFields.detection_scores]
        detection_keypoints = detections_dict[standard_fields.DetectionResultFields.detection_keypoints]
        class_indices = [idx for idx, class_id in enumerate(detection_classes) if class_id == self._category_id]
        ...
        filtered_detection_keypoints = np.take(detection_keypoints, class_indices, axis=0)
        filtered_detection_keypoints = np.take(filtered_detection_keypoints, self._keypoint_ids, axis=1)

        filtered_detections_dict = {}
        filtered_detections_dict[standard_fields.DetectionResultFields.detection_classes] = filtered_detection_classes
        filtered_detections_dict[standard_fields.DetectionResultFields.detection_boxes] = filtered_detection_boxes
        filtered_detections_dict[standard_fields.DetectionResultFields.detection_scores] = filtered_detection_scores
        filtered_detections_dict[
            standard_fields.DetectionResultFields.detection_keypoints
        ] = filtered_detection_keypoints

        super(CocoKeypointEvaluator, self).add_single_detected_image_info(image_id, filtered_detections_dict)
    ...
    def evaluate(self):
        ...
        coco_wrapped_groundtruth = coco_tools.COCOWrapper(groundtruth_dict, detection_type="bbox")
        coco_wrapped_detections = coco_wrapped_groundtruth.LoadAnnotations(self._detection_boxes_list)
        ...
        keypoint_metrics = {"Keypoints_" + key: value for key, value in iter(keypoint_metrics.items())}
    ...
    def add_eval_dict(self, eval_dict):
        ...
        groundtruth_keypoint_visibilities = eval_dict.get(input_data_fields.groundtruth_keypoint_visibilities, None)
        detection_boxes = eval_dict[detection_fields.detection_boxes]
        detection_scores = eval_dict[detection_fields.detection_scores]
        detection_classes = eval_dict[detection_fields.detection_classes]
        ...
        if not image_id.shape.as_list():
            ...
            if groundtruth_keypoint_visibilities is None:
                groundtruth_keypoint_visibilities = tf.fill(
                    [tf.shape(groundtruth_boxes)[1], tf.shape(groundtruth_keypoints)[2]], tf.constant(2, dtype=tf.int32)
                )
            groundtruth_keypoint_visibilities = tf.expand_dims(groundtruth_keypoint_visibilities, 0)
        else:
            if num_gt_boxes_per_image is None:
                num_gt_boxes_per_image = tf.tile(
                    tf.shape(groundtruth_boxes)[1:2], multiples=tf.shape(groundtruth_boxes)[0:1]
            ...
            if groundtruth_keypoint_visibilities is None:
                groundtruth_keypoint_visibilities = tf.fill(
                    [tf.shape(groundtruth_keypoints)[1], tf.shape(groundtruth_keypoints)[2]],
...
class CocoMaskEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def __init__(self, categories, include_metrics_per_category=False, super_categories=None):
        """Constructor.
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        if groundtruth_masks_shape[1:] != detection_masks.shape[1:]:
            raise ValueError(
                "Spatial shape of groundtruth masks and detection masks are incompatible: {} vs {}".format(
    ...
    def dump_detections_to_json_file(self, json_output_path):
        ...
        if json_output_path and json_output_path is not None:
            tf.logging.info("Dumping detections to output json file.")
            with tf.gfile.GFile(json_output_path, "w") as fid:
                json_utils.Dump(obj=self._detection_masks_list, fid=fid, float_digits=4, indent=2)
    ...
    def evaluate(self):
        ...
        coco_wrapped_groundtruth = coco_tools.COCOWrapper(groundtruth_dict, detection_type="segmentation")
        ...
        mask_evaluator = coco_tools.COCOEvalWrapper(
            coco_wrapped_groundtruth, coco_wrapped_detection_masks, agnostic_mode=False, iou_type="segm"
        )
    ...
    def add_eval_dict(self, eval_dict):
        ...
        if not image_id.shape.as_list():
            ...
            if num_det_boxes_per_image is None:
                num_det_boxes_per_image = tf.tile(
                    tf.shape(detection_scores)[1:2], multiples=tf.shape(detection_scores)[0:1]
                )
...
class CocoPanopticSegmentationEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def __init__(self, categories, include_metrics_per_category=False, iou_threshold=0.5, ioa_threshold=0.5):
        """Constructor.
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        if image_id in self._groundtruth_masks:
            tf.logging.warning(
                "Ignoring groundtruth with image %s, since it has already been added to the ground truth database.",
                image_id,
            )
            return

        self._groundtruth_masks[image_id] = groundtruth_dict[standard_fields.InputDataFields.groundtruth_instance_masks]
        self._groundtruth_class_labels[image_id] = groundtruth_dict[standard_fields.InputDataFields.groundtruth_classes]
        groundtruth_is_crowd = groundtruth_dict.get(standard_fields.InputDataFields.groundtruth_is_crowd)
    ...
    def evaluate(self):
        ...
        mask_metrics = self._compute_panoptic_metrics(sum_tp_iou, sum_num_tp, sum_num_fp, sum_num_fn)
        return mask_metrics
    ...
    def get_estimator_eval_metric_ops(self, eval_dict):
            ...
            for (
                ...
                self.add_single_ground_truth_image_info(
                    image_id,
                    {
                        ...
                        "groundtruth_instance_masks": groundtruth_instance_masks[:num_gt_mask],
        ...
        if self._include_metrics_per_category:
            for category_dict in self._categories:
                metric_names.append(
                    "PanopticQuality@%.2fIOU_ByCategory/%s" % (self._iou_threshold, category_dict["name"])
                )
    ...
    def _evaluate_all_masks(self):
        ...
        for image_id in self._groundtruth_class_labels:
            ...
            (
                ...
                crowd_gt_indices, self._groundtruth_masks[image_id], self._groundtruth_class_labels[image_id]
            )
            ...
            (overlaps, pred_matched, gt_matched) = self._match_predictions_to_groundtruths(
                predicted_masks,
            ...
            for class_id, is_matched, overlap in zip(predicted_class_labels, pred_matched, overlaps):
                if is_matched:
    ...
    def _compute_panoptic_metrics(self, sum_tp_iou, sum_num_tp, sum_num_fp, sum_num_fn):
        ...
        for category in self._categories:
            ...
            (panoptic_quality, segmentation_quality, recognition_quality) = self._compute_panoptic_metrics_single_class(
                sum_tp_iou[class_id], sum_num_tp[class_id], sum_num_fp[class_id], sum_num_fn[class_id]
            )
            if panoptic_quality is not None:
                ...
                if self._include_metrics_per_category:
                    mask_metrics[
                        "PanopticQuality@%.2fIOU_ByCategory/%s" % (self._iou_threshold, category["name"])
                    ] = panoptic_quality
        mask_metrics["PanopticQuality@%.2fIOU" % self._iou_threshold] = sum_pq / num_valid_classes
        mask_metrics["SegmentationQuality@%.2fIOU" % self._iou_threshold] = sum_sq / num_valid_classes
        mask_metrics["RecognitionQuality@%.2fIOU" % self._iou_threshold] = sum_rq / num_valid_classes
        mask_metrics["NumValidClasses"] = num_valid_classes
        mask_metrics["NumTotalClasses"] = len(self._categories)
        return mask_metrics

    def _compute_panoptic_metrics_single_class(self, sum_tp_iou, num_tp, num_fp, num_fn):
    ...
    def _separate_normal_and_crowd_labels(self, crowd_gt_indices, groundtruth_masks, groundtruth_classes):
        ...
        if crowd_gt_indices is None:
            ...
            if groundtruth_masks.shape[0] != crowd_gt_indices.shape[0]:
                raise ValueError("The number of masks doesn't match the number of is_crowd labels.")
    ...
    def _match_predictions_to_groundtruths(
        ...
        """
        if groundtruth_masks.shape[0] != groundtruth_classes.shape[0]:
            raise ValueError("The number of GT masks doesn't match the number of labels.")
        if predicted_masks.shape[0] != predicted_classes.shape[0]:
            raise ValueError("The number of predicted masks doesn't match the number of labels.")
            ...
            for gid in range(groundtruth_classes.shape[0]):
                if predicted_classes[pid] == groundtruth_classes[gid]:
                    if (not with_replacement) and gt_matched[gid]:
                        continue
                    if not is_crowd:
                        overlap = np_mask_ops.iou(predicted_masks[pid : pid + 1], groundtruth_masks[gid : gid + 1])[
                            0, 0
                        ]
                    else:
                        overlap = np_mask_ops.ioa(groundtruth_masks[gid : gid + 1], predicted_masks[pid : pid + 1])[
                            0, 0
                        ]
    ...
    def _unpack_evaluation_dictionary_items(self, eval_dict):
        ...
        groundtruth_instance_masks = eval_dict[input_data_fields.groundtruth_instance_masks]
        groundtruth_is_crowd = eval_dict.get(input_data_fields.groundtruth_is_crowd, None)
        num_gt_masks_per_image = eval_dict.get(input_data_fields.num_groundtruth_boxes, None)
        detection_classes = eval_dict[detection_fields.detection_classes]
        detection_masks = eval_dict[detection_fields.detection_masks]
        ...
        if not image_id.shape.as_list():
            ...
            if num_gt_masks_per_image is None:
                num_gt_masks_per_image = tf.tile(
                    tf.shape(groundtruth_classes)[1:2], multiples=tf.shape(groundtruth_classes)[0:1]
                )
            if num_det_masks_per_image is None:
                num_det_masks_per_image = tf.tile(
                    tf.shape(detection_classes)[1:2], multiples=tf.shape(detection_classes)[0:1]
                )
        return (
            image_id,
            groundtruth_classes,
