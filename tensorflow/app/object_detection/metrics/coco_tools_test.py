...
class CocoToolsTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._detections_list = [
            {"image_id": "first", "category_id": 1, "bbox": [100.0, 100.0, 100.0, 100.0], "score": 0.8},
            {"image_id": "second", "category_id": 1, "bbox": [50.0, 50.0, 50.0, 50.0], "score": 0.7},
        ]
    ...
    def testExportGroundtruthToCOCO(self):
        ...
        result = coco_tools.ExportGroundtruthToCOCO(
            image_ids, groundtruth_boxes, groundtruth_classes, categories, output_path=output_path
        )
    ...
    def testExportDetectionsToCOCO(self):
        ...
        result = coco_tools.ExportDetectionsToCOCO(
            image_ids, detections_boxes, detections_scores, detections_classes, categories, output_path=output_path
        )
    ...
    def testExportSegmentsToCOCO(self):
        image_ids = ["first", "second"]
        detection_masks = [
            np.array([[[0, 1, 0, 1], [0, 1, 1, 0], [0, 0, 0, 1], [0, 1, 0, 1]]], dtype=np.uint8),
            np.array([[[0, 1, 0, 1], [0, 1, 1, 0], [0, 0, 0, 1], [0, 1, 0, 1]]], dtype=np.uint8),
        ]
        ...
        result = coco_tools.ExportSegmentsToCOCO(
            image_ids, detection_masks, detection_scores, detection_classes, categories, output_path=output_path
        )
        with tf.gfile.GFile(output_path, "r") as f:
            written_result = f.read()
            written_result = json.loads(written_result)
    ...
    def testExportKeypointsToCOCO(self):
        image_ids = ["first", "second"]
        detection_keypoints = [
            np.array([[[100, 200], [300, 400], [500, 600]], [[50, 150], [250, 350], [450, 550]]], dtype=np.int32),
            np.array([[[110, 210], [310, 410], [510, 610]], [[60, 160], [260, 360], [460, 560]]], dtype=np.int32),
        ]
        ...
        result = coco_tools.ExportKeypointsToCOCO(
            image_ids, detection_keypoints, detection_scores, detection_classes, categories, output_path=output_path
        )
    ...
    def testSingleImageGroundtruthExportWithKeypoints(self):
        ...
        for i, annotation in enumerate(coco_annotations):
            ...
            self.assertTrue(np.all(np.isclose(annotation["keypoints"][0::3], keypoints[i, :, 1])))
            self.assertTrue(np.all(np.isclose(annotation["keypoints"][1::3], keypoints[i, :, 0])))
            self.assertTrue(np.all(np.equal(annotation["keypoints"][2::3], visibilities[i])))
