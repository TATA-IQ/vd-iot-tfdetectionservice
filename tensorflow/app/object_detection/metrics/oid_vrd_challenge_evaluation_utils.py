...
def build_groundtruth_vrd_dictionary(data, class_label_map, relationship_label_map):
    ...
    labels["subject"] = data_boxes["LabelName1"].map(lambda x: class_label_map[x]).to_numpy()
    labels["object"] = data_boxes["LabelName2"].map(lambda x: class_label_map[x]).to_numpy()
    labels["relation"] = data_boxes["RelationshipLabel"].map(lambda x: relationship_label_map[x]).to_numpy()
...
def build_predictions_vrd_dictionary(data, class_label_map, relationship_label_map):
    ...
    return {
        ...
        standard_fields.DetectionResultFields.detection_scores: data_boxes["Score"].to_numpy(),
    }
