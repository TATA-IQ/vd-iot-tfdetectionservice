...
from object_detection.utils import config_util, tf_version
...
def get_pipeline_config_path(model_name):
    ...
    if model_name == MODEL_NAME_FOR_KEYPOINTS_TEST:
        return os.path.join(tf.resource_loader.get_data_files_path(), "test_data", model_name + ".config")
...
def _get_keypoints_labelmap_path():
    """Returns an absolute path to label map file."""
    return os.path.join(tf.resource_loader.get_data_files_path(), "data", "face_person_with_keypoints_label_map.pbtxt")


def _get_sequence_example_labelmap_path():
    """Returns an absolute path to label map file."""
    return os.path.join(tf.resource_loader.get_data_files_path(), "data", "snapshot_serengeti_label_map.pbtxt")
...
class ModelLibTest(tf.test.TestCase):
            ...
            if mode == "train":
                ...
                features, labels = _make_initializable_iterator(
                    inputs.create_eval_input_fn(
                        configs["eval_config"], configs["eval_input_config"], configs["model"]
                    )()
                ).get_next()
                ...
                features, labels = _make_initializable_iterator(
                    inputs.create_eval_input_fn(
                        configs["eval_config"], configs["train_input_config"], configs["model"]
                    )()
                ).get_next()
                model_mode = tf.estimator.ModeKeys.EVAL
                batch_size = 1
    ...
    def _assert_model_fn_for_predict(self, configs):
        ...
        with tf.Graph().as_default():
            features, _ = _make_initializable_iterator(
                inputs.create_eval_input_fn(configs["eval_config"], configs["eval_input_config"], configs["model"])()
            ).get_next()
    ...
    def test_create_estimator_and_inputs_sequence_example(self):
        ...
        run_config = tf.estimator.RunConfig()
        hparams = model_hparams.create_hparams(hparams_overrides="load_pretrained=false")
        pipeline_config_path = get_pipeline_config_path(MODEL_NAME_FOR_SEQUENCE_EXAMPLE_TEST)
        train_steps = 20
        train_and_eval_dict = model_lib.create_estimator_and_inputs(
            run_config, hparams, pipeline_config_path, train_steps=train_steps
        )
    ...
    def test_create_tpu_estimator_and_inputs(self):
        ...
        train_steps = 20
        train_and_eval_dict = model_lib.create_estimator_and_inputs(
            run_config, hparams, pipeline_config_path, train_steps=train_steps, use_tpu_estimator=True
        )
        estimator = train_and_eval_dict["estimator"]
        train_steps = train_and_eval_dict["train_steps"]
        ...
        with self.test_session() as sess:
            unbatched_tensor_dict_out = sess.run(
                ...
                feed_dict={
                    ...
                    groundtruth_boxes_placeholder: np.random.rand(2, 5, 4).astype(np.float32),
                    groundtruth_classes_placeholder: np.random.rand(2, 5, 6).astype(np.float32),
                    groundtruth_weights_placeholder: np.random.rand(2, 5).astype(np.float32),
                },
            )
