...
class CenterNetMobileNetV2FeatureExtractor(center_net_meta_arch.CenterNetFeatureExtractor):
    ...
    def __init__(
        self, mobilenet_v2_net, channel_means=(0.0, 0.0, 0.0), channel_stds=(1.0, 1.0, 1.0), bgr_ordering=False
    ):
        ...
        for num_filters in [256, 128, 64]:
            ...
            conv = tf.keras.layers.Conv2D(filters=num_filters, kernel_size=1, strides=1, padding="same")
            output = conv(output)
        ...
        self._network = tf.keras.models.Model(inputs=self._network.input, outputs=output)

    def preprocess(self, resized_inputs):
        resized_inputs = super(CenterNetMobileNetV2FeatureExtractor, self).preprocess(resized_inputs)
...
def mobilenet_v2(channel_means, channel_stds, bgr_ordering):
    ...
    return CenterNetMobileNetV2FeatureExtractor(
        network, channel_means=channel_means, channel_stds=channel_stds, bgr_ordering=bgr_ordering
    )
