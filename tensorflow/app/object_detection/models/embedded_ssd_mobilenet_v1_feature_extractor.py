    ...
    def extract_features(self, preprocessed_inputs):
        ...
        if image_height is None or image_width is None:
            shape_assert = tf.Assert(
                tf.logical_and(
                    tf.equal(tf.shape(preprocessed_inputs)[1], 256), tf.equal(tf.shape(preprocessed_inputs)[2], 256)
                ),
            ...
            raise ValueError(
                "image size must be = 256 in both height and width; image dim = %d,%d" % (image_height, image_width)
            )
