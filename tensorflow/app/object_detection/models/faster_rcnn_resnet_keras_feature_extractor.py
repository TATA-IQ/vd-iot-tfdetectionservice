...
class FasterRCNNResnetKerasFeatureExtractor(faster_rcnn_meta_arch.FasterRCNNKerasFeatureExtractor):
    ...
    def get_proposal_feature_extractor_model(self, name=None):
        ...
        with tf.name_scope(name):
            with tf.name_scope("ResnetV1"):
                conv4_last_layer = _RESNET_MODEL_CONV4_LAST_LAYERS[self._resnet_v1_base_model_name]
                proposal_features = self.classification_backbone.get_layer(name=conv4_last_layer).output
                keras_model = tf.keras.Model(inputs=self.classification_backbone.inputs, outputs=proposal_features)
    ...
    def get_box_classifier_feature_extractor_model(self, name=None):
        ...
        with tf.name_scope(name):
            with tf.name_scope("ResnetV1"):
                conv4_last_layer = _RESNET_MODEL_CONV4_LAST_LAYERS[self._resnet_v1_base_model_name]
                proposal_feature_maps = self.classification_backbone.get_layer(name=conv4_last_layer).output
                proposal_classifier_features = self.classification_backbone.get_layer(name="conv5_block3_out").output
...
class FasterRCNNResnet50KerasFeatureExtractor(FasterRCNNResnetKerasFeatureExtractor):
    ...
    def __init__(self, is_training, first_stage_features_stride=16, batch_norm_trainable=False, weight_decay=0.0):
        """Constructor.
