...
class SsdFeatureExtractorTestBase(test_case.TestCase):
    ...
    def check_extract_features_returns_correct_shape(
        ...
        def graph_fn(image_tensor):
            return self._extract_features(image_tensor, feature_extractor, use_keras=use_keras)
    ...
    def check_extract_features_returns_correct_shapes_with_dynamic_inputs(
        ...
        feature_maps = self.execute_cpu(
            graph_fn, [np.array(image_height, dtype=np.int32), np.array(image_width, dtype=np.int32)], graph=g
        )
        for feature_map, expected_shape in zip(feature_maps, expected_feature_map_shapes):
            self.assertAllEqual(feature_map.shape, expected_shape)

    def check_extract_features_raises_error_with_invalid_image_size(
        self, image_height, image_width, depth_multiplier, pad_to_multiple, use_keras=False, use_depthwise=False
    ):
        with test_utils.GraphContextOrNone() as g:
            batch = 4
        ...
        def graph_fn():
            feature_maps = self._extract_features(preprocessed_inputs, feature_extractor, use_keras=use_keras)
            return feature_maps
    ...
    def check_feature_extractor_variables_under_scope(
        self, depth_multiplier, pad_to_multiple, scope_name, use_keras=False, use_depthwise=False
    ):
    ...
    def get_feature_extractor_variables(self, depth_multiplier, pad_to_multiple, use_keras=False, use_depthwise=False):
        ...
        with g.as_default():
            ...
            self._extract_features(preprocessed_inputs, feature_extractor, use_keras=use_keras)
