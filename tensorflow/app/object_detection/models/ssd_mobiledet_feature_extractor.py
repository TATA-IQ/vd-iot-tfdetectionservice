...
def _conv(h, filters, kernel_size, strides=1, normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu6):
...
def _inverted_bottleneck_no_expansion(h, filters, activation_fn=tf.nn.relu6, kernel_size=3, strides=1, use_se=False):
    ...
    with tf.variable_scope(None, default_name="IBNNoExpansion"):
        ...
        h = _separable_conv(h, None, kernel_size, strides=strides, activation_fn=activation_fn)
        if use_se:
...
def _inverted_bottleneck(
    h, filters, activation_fn=tf.nn.relu6, kernel_size=3, expansion=8, strides=1, use_se=False, residual=True
):
...
def _fused_conv(
    ...
    with tf.variable_scope(None, default_name="FusedConv"):
        ...
        if expansion <= 1:
            raise ValueError("Expansion factor must be greater than 1.")
        h = _conv(h, expanded_filters, kernel_size, strides=strides, activation_fn=activation_fn)
        if use_se:
            hidden_dim = _scale_filters(expanded_filters, 0.25)
            h = _squeeze_and_excite(h, hidden_dim, activation_fn=activation_fn)
        h = _conv(h, filters, 1, activation_fn=tf.identity)
...
def _tucker_conv(
    ...
    with tf.variable_scope(None, default_name="TuckerConv"):
        ...
        h = _conv(h, output_rank, kernel_size, strides=strides, activation_fn=activation_fn)
        h = _conv(h, filters, 1, activation_fn=tf.identity)
        if residual:
            h = h + shortcut
        return h
...
def mobiledet_cpu_backbone(h, multiplier=1.0):
    ...
    h = _inverted_bottleneck_no_expansion(h, _scale(8), use_se=True, activation_fn=_swish6)
    endpoints["C1"] = h
...
def mobiledet_edgetpu_backbone(h, multiplier=1.0):
    ...
    endpoints = {}
    h = _conv(h, _scale(32), 3, strides=2, activation_fn=tf.nn.relu6)
    h = tucker(h, _scale(16), input_rank_ratio=0.25, output_rank_ratio=0.75, residual=False)
    endpoints["C1"] = h
...
def mobiledet_gpu_backbone(h, multiplier=1.0):
    ...
    # block 1
    h = tucker(h, _scale(16), input_rank_ratio=0.25, output_rank_ratio=0.25, residual=False)
    endpoints["C1"] = h
...
class SSDMobileDetFeatureExtractorBase(ssd_meta_arch.SSDFeatureExtractor):
    ...
    def __init__(
        ...
        if use_explicit_padding:
            raise NotImplementedError("Explicit padding is not yet supported in MobileDet backbones.")
    ...
    def extract_features(self, preprocessed_inputs):
        ...
        with tf.variable_scope(self._scope_name, reuse=self._reuse_weights):
            with slim.arg_scope(
                [slim.batch_norm], is_training=self._is_training, epsilon=0.01, decay=0.99, center=True, scale=True
            ):
                endpoints = self._backbone_fn(padded_inputs, multiplier=self._depth_multiplier)
