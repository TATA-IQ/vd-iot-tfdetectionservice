...
Block = collections.namedtuple("Block", ["inputs", "output_level", "kernel_size", "expansion_size"])
...
def _apply_multiplier(d, multiplier, min_depth):
    p = {"num_outputs": d}
    mobilenet.depth_multiplier(p, multiplier=multiplier, divisible_by=8, min_depth=min_depth)
    return p["num_outputs"]


def _apply_size_dependent_ordering(
    input_feature, feature_level, block_level, expansion_size, use_explicit_padding, use_native_resize_op
):
    ...
    if feature_level >= block_level:  # Perform 1x1 then upsampling.
        ...
        if use_native_resize_op:
            ...
            node = tf.image.resize_nearest_neighbor(node, [input_shape[1] * scale, input_shape[2] * scale])
        else:
...
def training_scope(l2_weight_decay=1e-4, is_training=None):
    ...
    with slim.arg_scope(
        ...
        weights_initializer=tf.initializers.truncated_normal(stddev=0.536),  # He_normal for 3x3 depthwise kernel.
        weights_regularizer=slim.l2_regularizer(l2_weight_decay),
    ), slim.arg_scope(
        [slim.batch_norm], is_training=is_training, epsilon=0.01, decay=0.99, center=True, scale=True
    ) as s:
        return s
...
class SSDMobileNetV2MnasFPNFeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
    ...
    def _verify_config(self, inputs):
        ...
        base_width = inputs[0].shape.as_list()[1] * 2 ** self._head_def["feature_levels"][0]
        for i in range(1, num_inputs):
            width = inputs[i].shape.as_list()[1]
            level = self._head_def["feature_levels"][i]
            expected_width = base_width // 2**level
            if width != expected_width:
                raise ValueError("Resolution of input {} does not match its level {}.".format(i, level))
        ...
        for cell_spec in self._head_def["spec"]:
            ...
            for i in range(num_inputs):
                if cell_spec[-num_inputs + i].output_level != self._head_def["feature_levels"][i]:
                    raise ValueError(
                        "Mismatch between node level {} and desired output level {}.".format(
                            cell_spec[-num_inputs + i].output_level, self._head_def["feature_levels"][i]
                        )
                    )
            ...
            for bi, block_spec in enumerate(cell_spec):
                for inp in block_spec.inputs:
                    if inp >= bi + num_inputs:
                        raise ValueError("Block {} is trying to access uncreated block {}.".format(bi, inp))
    ...
    def extract_features(self, preprocessed_inputs):
        ...
        with tf.variable_scope("MobilenetV2", reuse=self._reuse_weights) as scope:
            with slim.arg_scope(mobilenet_v2.training_scope(is_training=None, bn_decay=0.99)), slim.arg_scope(
                [mobilenet.depth_multiplier], min_depth=self._min_depth
            ):
                with slim.arg_scope(training_scope(l2_weight_decay=4e-5, is_training=self._is_training)):
        ...
        multiplier_func = functools.partial(
            _apply_multiplier, multiplier=self._depth_multiplier, min_depth=self._min_depth
        )
        with tf.variable_scope("MnasFPN", reuse=self._reuse_weights):
            with slim.arg_scope(training_scope(l2_weight_decay=1e-4, is_training=self._is_training)):
