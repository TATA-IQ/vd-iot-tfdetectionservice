...
class ExportSavedModelTPUTest(tf.test.TestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        ...
        ("faster_rcnn", get_path("faster_rcnn/faster_rcnn_resnet101_atrous_coco.config"), "image_tensor", True, 20),
    )
    def testExportAndLoad(self, pipeline_config_file, input_type="image_tensor", use_bfloat16=False, repeat=1):
        ...
        export_saved_model_tpu_lib.export(
            pipeline_config_file, ckpt_path, export_dir, input_placeholder_name, input_type, use_bfloat16
        )
