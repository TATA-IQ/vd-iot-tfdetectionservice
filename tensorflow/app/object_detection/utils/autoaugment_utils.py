...
def _apply_bbox_augmentation(image, bbox, augmentation_func, *args):
    ...
    augmented_bbox_content = tf.pad(
        augmented_bbox_content, [[min_y, (image_height - 1) - max_y], [min_x, (image_width - 1) - max_x], [0, 0]]
    )
...
def _apply_bbox_augmentation_wrapper(image, bbox, new_bboxes, prob, augmentation_func, func_changes_bbox, *args):
    ...
    if func_changes_bbox:
        ...
        augmented_image = tf.cond(
            should_apply_op, lambda: _apply_bbox_augmentation(image, bbox, augmentation_func, *args), lambda: image
        )
...
def _apply_multi_bbox_augmentation_wrapper(image, bboxes, prob, aug_func, func_changes_bbox, *args):
    ...
    image, bboxes = tf.cond(
        ...
        lambda: (image, bboxes),
        # pylint:disable=g-long-lambda
        lambda: _apply_multi_bbox_augmentation(image, bboxes, prob, aug_func, func_changes_bbox, *args),
    )
    # pylint:enable=g-long-lambda
    return image, bboxes
...
def bbox_cutout(image, bboxes, pad_fraction, replace_with_mean):
    ...
    def apply_bbox_cutout(image, bboxes, pad_fraction):
        ...
        # `replace`.
        image = tf.where(
            tf.equal(mask, 0), tf.cast(tf.ones_like(image, dtype=image.dtype) * replace, dtype=image.dtype), image
        )
        return image
