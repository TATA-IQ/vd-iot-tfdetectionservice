...
class ConfigUtilTest(tf.test.TestCase):
    ...
    def testNewClassificationLocalizationWeightRatio(self):
        ...
        hparams = contrib_training.HParams(classification_localization_weight_ratio=new_weight_ratio)
        pipeline_config_path = os.path.join(self.get_temp_dir(), "pipeline.config")
    ...
    def testCheckAndParseInputConfigKey(self):
        ...
        is_valid_input_config_key, key_name, input_name, field_name = config_util.check_and_parse_input_config_key(
    ...
    def testUpdateInputReaderConfigSuccess(self):
        ...
        config_util.update_input_reader_config(
            configs, key_name="train_input_config", input_name=None, field_name="shuffle", value=new_shuffle
        )
    ...
    def testUpdateInputReaderConfigErrors(self):
        ...
        with self.assertRaisesRegexp(ValueError, "Duplicate input name found when overriding."):
            config_util.update_input_reader_config(
                configs, key_name="eval_input_configs", input_name="same_eval_name", field_name="shuffle", value=False
            )

        with self.assertRaisesRegexp(ValueError, "Input name name_not_exist not found when overriding."):
            config_util.update_input_reader_config(
                configs, key_name="eval_input_configs", input_name="name_not_exist", field_name="shuffle", value=False
            )

        with self.assertRaisesRegexp(ValueError, "Unknown input config overriding."):
            config_util.update_input_reader_config(
                configs, key_name="eval_input_configs", input_name=None, field_name="shuffle", value=False
            )
