...
def extract_submodel(model, inputs, outputs, name=None):
    ...
    def _recurse_in_model(tensor):
        ...
        if (tensor.experimental_ref() == inputs.experimental_ref()) or (isinstance(inputs, list) and tensor in inputs):
            if tensor.experimental_ref() not in model_inputs_dict:
                model_inputs_dict[tensor.experimental_ref()] = tf.keras.layers.Input(tensor=tensor)
    ...
    if isinstance(inputs, list):
        model_inputs = [model_inputs_dict[tensor.experimental_ref()] for tensor in inputs]
    else:
