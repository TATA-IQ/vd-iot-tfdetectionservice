...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def _build_metric_names(self):
        ...
        for idx in range(self._num_classes):
            if idx + self._label_id_offset in category_index:
                ...
                self._metric_names.append(
                    self._metric_prefix
                    + "PerformanceByCategory/AP@{}IOU/{}".format(self._matching_iou_threshold, category_name)
                )
                if self._evaluate_corlocs:
                    self._metric_names.append(
                        self._metric_prefix
                        + "PerformanceByCategory/CorLoc@{}IOU/{}".format(self._matching_iou_threshold, category_name)
                    )
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        if standard_fields.InputDataFields.groundtruth_difficult in six.viewkeys(groundtruth_dict) and (
...
class OpenImagesDetectionEvaluator(ObjectDetectionEvaluator):
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        # (unless there are no annotations for the groundtruth on this image)
        # use values from the dictionary or insert None otherwise.
        if standard_fields.InputDataFields.groundtruth_group_of in six.viewkeys(groundtruth_dict) and (
...
class OpenImagesChallengeEvaluator(OpenImagesDetectionEvaluator):
    ...
    def __init__(
        self, categories, evaluate_masks=False, matching_iou_threshold=0.5, evaluate_corlocs=False, group_of_weight=1.0
    ):
        """Constructor.
    ...
    def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
        ...
        groundtruth_classes = groundtruth_dict[input_fields.groundtruth_classes] - self._label_id_offset
        ...
        self._evaluatable_labels[image_id] = np.unique(np.concatenate((image_classes, groundtruth_classes)))
...
class ObjectDetectionEvaluation(object):
    ...
    def add_single_ground_truth_image_info(
        ...
        self._update_ground_truth_statistics(
            groundtruth_class_labels,
            groundtruth_is_difficult_list.astype(dtype=bool) | mask_presence_indicator,  # ignore boxes without masks
    ...
    def _update_ground_truth_statistics(
        ...
        for class_index in range(self.num_class):
            ...
            num_groupof_gt_instances = self.group_of_weight * np.sum(
                groundtruth_class_labels[groundtruth_is_group_of_list & ~groundtruth_is_difficult_list] == class_index
            )
