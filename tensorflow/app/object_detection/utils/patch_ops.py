...
def get_patch_mask(y, x, patch_size, image_shape):
    ...
    with tf.control_dependencies(
        [
            ...
            tf.debugging.assert_greater_equal(mask_center_yx, 0, message="Patch center (y, x) must be >= (0, 0)"),
            tf.debugging.assert_less(mask_center_yx, image_hw, message="Patch center (y, x) must be < image (h, w)"),
        ]
    ):
