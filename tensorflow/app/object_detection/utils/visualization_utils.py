...
def draw_heatmaps_on_image_tensors(images, heatmaps, apply_sigmoid=False):
    ...
    def draw_heatmaps(image_and_heatmaps):
        ...
        image_with_heatmaps = tf.py_function(draw_heatmaps_on_image_array, image_and_heatmaps, tf.uint8)
    ...
    for indx in range(eval_dict[input_data_fields.original_image].shape[0]):
        ...
        if detection_fields.detection_keypoints in eval_dict:
            ...
            if detection_fields.detection_keypoint_scores in eval_dict:
                keypoint_scores = tf.expand_dims(eval_dict[detection_fields.detection_keypoint_scores][indx], axis=0)
            else:
                keypoint_scores = tf.cast(keypoint_ops.set_keypoint_visibilities(keypoints), dtype=tf.float32)
        ...
        if input_data_fields.groundtruth_keypoints in eval_dict:
            groundtruth_keypoints = tf.expand_dims(eval_dict[input_data_fields.groundtruth_keypoints][indx], axis=0)
...
def draw_densepose_visualizations(
    eval_dict, max_boxes_to_draw=20, min_score_thresh=0.2, num_parts=24, dp_coord_to_visualize=0
):
    ...
    for indx in range(eval_dict[input_data_fields.original_image].shape[0]):
        ...
        def draw_densepose_py_func(image, detection_masks, surface_coords, scores):
            ...
            for i, (score, surface_coord, mask) in enumerate(zip(scores, surface_coords, detection_masks)):
                if i == max_boxes_to_draw:
                    break
                if score > min_score_thresh:
                    draw_part_mask_on_image_array(image, mask, num_parts=num_parts)
                    draw_float_channel_on_image_array(
                        surface_coord_image, surface_coord[:, :, dp_coord_to_visualize], mask
                    )
            return np.concatenate([image, surface_coord_image], axis=1)

        image_with_densepose = tf.py_func(
            draw_densepose_py_func, [image, detection_masks, surface_coords, scores], tf.uint8
        )
...
def draw_float_channel_on_image_array(image, channel, mask, alpha=0.9, cmap="YlGn"):
    ...
    if image.shape[:2] != channel.shape:
        raise ValueError(
            "The image has spatial dimensions %s but the channel has dimensions %s" % (image.shape[:2], channel.shape)
        )
    if image.shape[:2] != mask.shape:
        raise ValueError(
            "The image has spatial dimensions %s but the mask has dimensions %s" % (image.shape[:2], mask.shape)
    ...
    pil_colored_channel = Image.fromarray(np.uint8(colored_channel * 255)).convert("RGBA")
    pil_mask = Image.fromarray(np.uint8(255.0 * alpha * (mask > 0))).convert("L")
