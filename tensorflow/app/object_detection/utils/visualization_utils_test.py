...
class VisualizationUtilsTest(test_case.TestCase):
    ...
    def test_draw_bounding_boxes_on_image_tensors(self):
        ...
        def graph_fn():
            ...
            boxes = tf.constant(
                [[[0.4, 0.25, 0.75, 0.75], [0.5, 0.3, 0.6, 0.9]], [[0.25, 0.25, 0.75, 0.75], [0.1, 0.3, 0.6, 1.0]]]
            )
    ...
    def test_draw_keypoints_on_image_with_default_keypoint_scores(self):
        ...
        visualization_utils.draw_keypoints_on_image(
            test_image, keypoints, keypoint_edges=keypoint_edges, keypoint_edge_width=1, keypoint_edge_color="green"
        )
        width_final, height_final = test_image.size
    ...
    def test_draw_float_channel_on_image_array(self):
        ...
        # 0.5 -> White
        # 1.0 -> Red
        visualization_utils.draw_float_channel_on_image_array(test_image, channel, mask, alpha=1.0, cmap="bwr")
        expected_result = np.asarray([[[0, 0, 0], [255, 254, 254]], [[0, 0, 255], [255, 0, 0]]], dtype=np.uint8)
        self.assertAllEqual(test_image, expected_result)
    ...
    def test_draw_heatmaps_on_image_array(self):
        ...
        self.assertAllEqual(
            output_image, np.array([[[240, 248, 255], [127, 255, 0]], [[127, 255, 0], [240, 248, 255]]])
        )
    ...
    def test_draw_heatmaps_on_image_tensors(self):
        ...
        self.assertAllEqual(
            output_image_np,
            np.expand_dims(np.array([[[240, 248, 255], [127, 255, 0]], [[127, 255, 0], [240, 248, 255]]]), axis=0),
        )
    ...
    def test_eval_metric_ops(self):
        ...
        eval_metric_ops = visualization_utils.VisualizeSingleFrameDetections(
            category_index, max_examples_to_draw=max_examples_to_draw, summary_name_prefix=metric_op_base
        )
        ...
        with self.test_session() as sess:
            ...
            for i in range(max_examples_to_draw):
                ...
                sess.run(
                    update_op,
                    feed_dict={
                        original_image: np.random.randint(low=0, high=256, size=(4, 6 + i, 7 + i, 3), dtype=np.uint8),
                        original_image_spatial_shape: [[6 + i, 7 + i], [6 + i, 7 + i], [6 + i, 7 + i], [6 + i, 7 + i]],
                        true_image_shape: [[6 + i, 7 + i, 3], [6 + i, 7 + i, 3], [6 + i, 7 + i, 3], [6 + i, 7 + i, 3]],
                    },
                )
    ...
    def test_visualize_boxes_and_labels_on_image_array(self):
        ...
        detections = np.array([[0.8, 0.1, 0.9, 0.1, 1.0, 0.1], [0.1, 0.3, 0.8, 0.7, 1.0, 0.6]])
