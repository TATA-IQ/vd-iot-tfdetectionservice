...
class VRDDetectionEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def _process_groundtruth_boxes(self, groundtruth_box_tuples):
        ...
        raise NotImplementedError(
            "_process_groundtruth_boxes method should be implemented in subclassesof VRDDetectionEvaluator."
        )
    ...
    def _process_detection_boxes(self, detections_box_tuples):
        ...
        """
        raise NotImplementedError(
            "_process_detection_boxes method should be implemented in subclassesof VRDDetectionEvaluator."
        )
    ...
    def evaluate(self, relationships=None):
        ...
        vrd_metrics = {
            (self._metric_prefix + "weightedAP@{}IOU".format(self._matching_iou_threshold)): weighted_average_precision,
        ...
        if relationships:
            for key, average_precision in six.iteritems(average_precisions):
                vrd_metrics[
                    self._metric_prefix + "AP@{}IOU/{}".format(self._matching_iou_threshold, relationships[key])
                ] = average_precision
        else:
            for key, average_precision in six.iteritems(average_precisions):
                vrd_metrics[
                    self._metric_prefix + "AP@{}IOU/{}".format(self._matching_iou_threshold, key)
                ] = average_precision
