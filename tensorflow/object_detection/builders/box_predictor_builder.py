...
def build_mask_rcnn_keras_box_predictor(
    ...
    if predict_instance_masks:
        third_stage_heads[mask_rcnn_box_predictor.MASK_PREDICTIONS] = keras_mask_head.MaskRCNNMaskHead(
            is_training=is_training,
            num_classes=num_classes,
            conv_hyperparams=conv_hyperparams,
            freeze_batchnorm=freeze_batchnorm,
...
def build_mask_rcnn_box_predictor(
    ...
    third_stage_heads = {}
    if predict_instance_masks:
        third_stage_heads[mask_rcnn_box_predictor.MASK_PREDICTIONS] = mask_head.MaskRCNNMaskHead(
            num_classes=num_classes,
            conv_hyperparams_fn=conv_hyperparams_fn,
            mask_height=mask_height,
            mask_width=mask_width,
...
def build(argscope_fn, box_predictor_config, is_training, num_classes, add_background_class=True):
    ...
    if box_predictor_oneof == "rfcn_box_predictor":
        ...
        box_predictor_object = rfcn_box_predictor.RfcnBoxPredictor(
            ...
            num_classes=num_classes,
            conv_hyperparams_fn=conv_hyperparams_fn,
            crop_size=[config_box_predictor.crop_height, config_box_predictor.crop_width],
            num_spatial_bins=[
