...
def read_dataset(file_read_func, input_files, config, filename_shard_fn=None):
    ...
    if config.shuffle:
        ...
        tf.logging.warning(
            "`shuffle` is false, but the input data stream is still slightly shuffled since `num_readers` > 1."
        )
    ...
    records_dataset = filename_dataset.apply(
        tf.data.experimental.parallel_interleave(
            file_read_func, cycle_length=num_readers, block_length=config.read_block_length, sloppy=config.shuffle
        )
    )
    if config.shuffle:
...
def build(
    input_reader_config, batch_size=None, transform_input_data_fn=None, input_context=None, reduce_to_frame_fn=None
):
    ...
    if input_reader_config.WhichOneof("input_reader") == "tf_record_input_reader":
        ...
        # are deleted.
        dataset = dataset_map_fn(dataset, decoder.decode, batch_size, input_reader_config)
        if reduce_to_frame_fn:
            dataset = reduce_to_frame_fn(dataset, dataset_map_fn, batch_size, input_reader_config)
        if transform_input_data_fn is not None:
            dataset = dataset_map_fn(dataset, transform_input_data_fn, batch_size, input_reader_config)
        if batch_size:
