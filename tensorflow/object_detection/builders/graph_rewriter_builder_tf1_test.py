...
class QuantizationBuilderTest(tf.test.TestCase):
    def testQuantizationBuilderSetsUpCorrectTrainArguments(self):
        with mock.patch.object(contrib_quantize, "experimental_create_training_graph") as mock_quant_fn:
    ...
    def testQuantizationBuilderSetsUpCorrectEvalArguments(self):
        with mock.patch.object(contrib_quantize, "experimental_create_eval_graph") as mock_quant_fn:
            with mock.patch.object(slim, "summarize_collection") as mock_summarize_col:
                graph_rewriter_proto = graph_rewriter_pb2.GraphRewriter()
                graph_rewriter_proto.quantization.delay = 10
