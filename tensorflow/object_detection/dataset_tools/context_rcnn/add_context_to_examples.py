...
class ReKeyDataFn(beam.DoFn):
    ...
    def __init__(self, sequence_key, time_horizon, reduce_image_size, max_image_dimension):
        ...
        if time_horizon is None or time_horizon in {"year", "month", "week", "day", "hour", "minute"}:
        ...
        self._session = None
        self._num_examples_processed = beam.metrics.Metrics.counter("data_rekey", "num_tf_examples_processed")
        self._num_images_resized = beam.metrics.Metrics.counter("data_rekey", "num_images_resized")
        self._num_images_read = beam.metrics.Metrics.counter("data_rekey", "num_images_read")
        self._num_images_found = beam.metrics.Metrics.counter("data_rekey", "num_images_read")
        self._num_got_shape = beam.metrics.Metrics.counter("data_rekey", "num_images_got_shape")
        self._num_images_found_size = beam.metrics.Metrics.counter("data_rekey", "num_images_found_size")
        self._num_examples_cleared = beam.metrics.Metrics.counter("data_rekey", "num_examples_cleared")
        self._num_examples_updated = beam.metrics.Metrics.counter("data_rekey", "num_examples_updated")
    ...
    def _resize_image(self, input_example):
        ...
        original_image = copy.deepcopy(input_example.features.feature["image/encoded"].bytes_list.value[0])
        self._num_images_read.inc(1)

        height = copy.deepcopy(input_example.features.feature["image/height"].int64_list.value[0])

        width = copy.deepcopy(input_example.features.feature["image/width"].int64_list.value[0])

        self._num_got_shape.inc(1)

        new_height, new_width = self._largest_size_at_most(height, width, self._max_image_dimension)
        ...
        input_example.features.feature["image/encoded"].bytes_list.value.extend([encoded_resized_image])
        input_example.features.feature["image/height"].int64_list.value.extend([new_height])
        input_example.features.feature["image/width"].int64_list.value.extend([new_width])
    ...
    def _rekey_examples(self, tfrecord_entry):
        ...
        if self._time_horizon:
            date_captured = datetime.datetime.strptime(
                six.ensure_str(input_example.features.feature["image/date_captured"].bytes_list.value[0]),
                "%Y-%m-%d %H:%M:%S",
            )
            ...
            if self._time_horizon == "year":
                ...
                new_key = new_key + six.ensure_binary("/" + str(year) + "/" + str(month))
            elif self._time_horizon == "week":
                new_key = new_key + six.ensure_binary("/" + str(year) + "/" + str(month) + "/" + str(week))
            elif self._time_horizon == "day":
                new_key = new_key + six.ensure_binary("/" + str(year) + "/" + str(month) + "/" + str(day))
...
class SortGroupedDataFn(beam.DoFn):
    ...
    def __init__(self, sequence_key, sorted_image_ids, max_num_elements_in_context_features):
        """Initialization function.
        ...
        """
        self._session = None
        self._num_examples_processed = beam.metrics.Metrics.counter("sort_group", "num_groups_sorted")
        self._too_many_elements = beam.metrics.Metrics.counter("sort_group", "too_many_elements")
        self._split_elements = beam.metrics.Metrics.counter("sort_group", "split_elements")
        self._sequence_key = six.ensure_binary(sequence_key)
        self._sorted_image_ids = sorted_image_ids
        self._max_num_elements_in_context_features = max_num_elements_in_context_features
    ...
    def _sort_image_examples(self, grouped_entry):
        ...
        def get_date_captured(example):
            return datetime.datetime.strptime(
                six.ensure_str(example.features.feature["image/date_captured"].bytes_list.value[0]), "%Y-%m-%d %H:%M:%S"
            )
...
def get_sliding_window(example_list, max_clip_length, stride_length):
    ...
    if len(example_list) < max_clip_length:
        ...
        starting_values = [i * stride_length for i in range(len(example_list)) if len(example_list) > i * stride_length]
        for start in starting_values:
            result = tuple(itertools.islice(example_list, start, min(start + max_clip_length, len(example_list))))
            yield result
...
class GenerateContextFn(beam.DoFn):
    ...
    def __init__(
        ...
        self._num_keys_processed = beam.metrics.Metrics.counter("sequence_data_generation", "num_keys_processed")
        self._sequence_key = sequence_key
        ...
        self._images_kept = beam.metrics.Metrics.counter("sequence_data_generation", "images_kept")
        self._images_loaded = beam.metrics.Metrics.counter("sequence_data_generation", "images_loaded")
    ...
    def _build_context_features(self, example_list):
        ...
        for idx, example in enumerate(example_list):
            if self._subsample_context_features_rate > 0:
                if (idx % self._subsample_context_features_rate) != 0:
                    example.features.feature["context_features_idx"].int64_list.value.append(
                        self._max_num_elements_in_context_features + 1
                    )
                    continue
            if self._keep_only_positives:
                if (
                    example.features.feature["image/embedding_score"].float_list.value[0]
            ...
            if self._keep_only_positives_gt:
                if len(example.features.feature["image/object/bbox/xmin"].float_list.value) < 1:
                    example.features.feature["context_features_idx"].int64_list.value.append(
                        self._max_num_elements_in_context_features + 1
                    )
                    continue

            example_embedding = list(example.features.feature["image/embedding"].float_list.value)
            context_features.extend(example_embedding)
            example.features.feature["context_features_idx"].int64_list.value.append(count)
            count += 1
            example_image_id = example.features.feature["image/source_id"].bytes_list.value[0]
        ...
        # be kept as default.
        if example_list and ("image/embedding_length" in example_list[0].features.feature):
            feature_length = example_list[0].features.feature["image/embedding_length"].int64_list.value[0]

        if self._pad_context_features:
            while len(context_features_image_id_list) < (self._max_num_elements_in_context_features):
    ...
    def _add_context_to_example(self, grouped_entry):
        ...
        if self._add_context_features:
            context_features, feature_length, context_features_image_id_list = self._build_context_features(
                example_list
            )
        ...
        if self._output_type == "tf_sequence_example":
            if self._max_clip_length is not None:
                # For now, no overlap
                clips = get_sliding_window(example_list, self._max_clip_length, self._max_clip_length)
            else:
                clips = [example_list]
            ...
            for clip_num, clip_list in enumerate(clips):
                ...
                seq_example.context.feature["clip/media_id"].bytes_list.value.append(video_id.encode("utf8"))
                seq_example.context.feature["clip/frames"].int64_list.value.append(len(clip_list))

                seq_example.context.feature["clip/start/timestamp"].int64_list.value.append(0)
                seq_example.context.feature["clip/end/timestamp"].int64_list.value.append(len(clip_list))
                seq_example.context.feature["image/format"].bytes_list.value.append(six.ensure_binary("JPG"))
                ...
                seq_example.context.feature["image/context_feature_length"].int64_list.value.append(feature_length)
                seq_example.context.feature["image/context_features"].float_list.value.extend(context_features)
                if self._keep_context_features_image_id_list:
                    seq_example.context.feature["image/context_features_image_id_list"].bytes_list.value.extend(
                        context_features_image_id_list
                    )

                encoded_image_list = seq_example.feature_lists.feature_list["image/encoded"]
                timestamps_list = seq_example.feature_lists.feature_list["image/timestamp"]
                context_features_idx_list = seq_example.feature_lists.feature_list["image/context_features_idx"]
                date_captured_list = seq_example.feature_lists.feature_list["image/date_captured"]
                unix_time_list = seq_example.feature_lists.feature_list["image/unix_time"]
                location_list = seq_example.feature_lists.feature_list["image/location"]
                image_ids_list = seq_example.feature_lists.feature_list["image/source_id"]
                gt_xmin_list = seq_example.feature_lists.feature_list["region/bbox/xmin"]
                gt_xmax_list = seq_example.feature_lists.feature_list["region/bbox/xmax"]
                gt_ymin_list = seq_example.feature_lists.feature_list["region/bbox/ymin"]
                gt_ymax_list = seq_example.feature_lists.feature_list["region/bbox/ymax"]
                gt_type_list = seq_example.feature_lists.feature_list["region/label/index"]
                gt_type_string_list = seq_example.feature_lists.feature_list["region/label/string"]
                gt_is_annotated_list = seq_example.feature_lists.feature_list["region/is_annotated"]

                for idx, example in enumerate(clip_list):
                    encoded_image = encoded_image_list.feature.add()
                    encoded_image.bytes_list.value.extend(example.features.feature["image/encoded"].bytes_list.value)

                    image_id = image_ids_list.feature.add()
                    image_id.bytes_list.value.append(example.features.feature["image/source_id"].bytes_list.value[0])
                    ...
                    unix_time.float_list.value.extend(example.features.feature["image/unix_time"].float_list.value)
                    location = location_list.feature.add()
                    location.bytes_list.value.extend(example.features.feature["image/location"].bytes_list.value)
                    ...
                    gt_xmin.float_list.value.extend(example.features.feature["image/object/bbox/xmin"].float_list.value)
                    gt_xmax.float_list.value.extend(example.features.feature["image/object/bbox/xmax"].float_list.value)
                    gt_ymin.float_list.value.extend(example.features.feature["image/object/bbox/ymin"].float_list.value)
                    gt_ymax.float_list.value.extend(example.features.feature["image/object/bbox/ymax"].float_list.value)
            ...
            for example in example_list:
                ...
                if self._add_context_features:
                    example.features.feature["image/context_features"].float_list.value.extend(context_features)
                    example.features.feature["image/context_feature_length"].int64_list.value.append(feature_length)

                if self._keep_context_features_image_id_list:
                    example.features.feature["image/context_features_image_id_list"].bytes_list.value.extend(
                        context_features_image_id_list
                    )
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        required=True,
        help="TFRecord containing images in tf.Example format, with added contextual memory banks.",
    )
    parser.add_argument(
        "--sequence_key",
        dest="sequence_key",
        default="image/location",
        help="Key to use when grouping sequences: so far supports `image/seq_id` and `image/location`.",
    )
    parser.add_argument(
        "--context_feature_length",
        dest="context_feature_length",
        default=2057,
        help="The length of the context feature embeddings stored in the input data.",
    )
    parser.add_argument(
        "--time_horizon",
        dest="time_horizon",
    ...
    parser.add_argument(
        ...
        default=True,
        help="downsamples images to have longest side max_image_dimension, maintaining aspect ratio",
    )
    parser.add_argument(
        "--max_image_dimension", dest="max_image_dimension", default=1024, help="Sets max image dimension for resizing."
    )
    parser.add_argument(
        "--add_context_features",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Whether the image source_ids are sortable to deal with date_captured tie-breaks.",
    )
    parser.add_argument(
        "--image_ids_to_keep",
        dest="image_ids_to_keep",
        default="All",
        help="Path to .json list of image ids to keep, used for ground truth eval creation.",
    )
    parser.add_argument(
        "--keep_context_features_image_id_list",
        dest="keep_context_features_image_id_list",
        default=False,
        help="Whether or not to keep a list of the image_ids corresponding to the memory bank.",
    )
    parser.add_argument(
        "--keep_only_positives",
    ...
    parser.add_argument(
        ...
        default=0.7,
        help="What score threshold to use for boxes in context_features, when `keep_only_positives` is set to `True`.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_clip_length", dest="max_clip_length", default=None, help="Max length for sequence example outputs."
    )
    parser.add_argument("--num_shards", dest="num_shards", default=0, help="Number of output shards.")
