...
class ParseImage(beam.DoFn):
    ...
    def process(self, image_id):
        ...
        feature_dict = {
            ...
            "image/location": dataset_util.bytes_feature(str(image_location_id).encode("utf8")),
            "image/seq_num_frames": dataset_util.int64_feature(image_sequence_num_frames),
            "image/seq_frame_num": dataset_util.int64_feature(image_sequence_frame_num),
            "image/seq_id": dataset_util.bytes_feature(image_sequence_id.encode("utf8")),
            "image/date_captured": dataset_util.bytes_feature(image_datetime.encode("utf8")),
        }
        ...
        if annotations:
            ...
            for object_annotations in annotations:
                ...
                category_names.append(self._category_dict[category_id]["name"].encode("utf8"))
                ...
                feature_dict.update(
                    {
                        ...
                        "image/class/text": dataset_util.bytes_list_feature(["empty".encode("utf8")]),
                    }
                )
            else:
                feature_dict.update(
                    {
                        "image/class/label": dataset_util.int64_list_feature([category_ids[0]]),
                        "image/class/text": dataset_util.bytes_list_feature([category_names[0]]),
                    }
                )
...
def create_pipeline(
    ...
    image_examples = (
        ...
        >> beam.ParDo(
            ParseImage(
                image_directory, data["images"], data["annotations"], data["categories"], keep_bboxes=keep_bboxes
            )
        )
    )
    _ = (
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        required=True,
        help="Path and prefix to store TFRecords containing images in tf.Exampleformat.",
    )
    parser.add_argument(
        "--input_annotations_file",
