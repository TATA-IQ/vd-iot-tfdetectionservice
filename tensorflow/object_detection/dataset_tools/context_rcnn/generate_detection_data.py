...
class GenerateDetectionDataFn(beam.DoFn):
    ...
    def _run_inference_and_generate_detections(self, tfrecord_entry):
        ...
        image_class_labels = input_example.features.feature["image/object/class/label"].int64_list.value

        image_class_texts = input_example.features.feature["image/object/class/text"].bytes_list.value
        ...
        if len(image_class_labels) == 1:
            ...
            for idx, score in enumerate(detection_scores[0]):
                if score >= self._confidence_threshold and idx < num_detections:
                    ...
                    example.features.feature["image/object/class/score"].float_list.value.extend([score])

                    example.features.feature["image/object/class/label"].int64_list.value.extend(
                        [image_class_labels[0]]
                    )

                    example.features.feature["image/object/class/text"].bytes_list.value.extend([image_class_texts[0]])
...
def construct_pipeline(pipeline, input_tfrecord, output_tfrecord, model_dir, confidence_threshold, num_shards):
