...
if tf_version.is_tf2():
    from object_detection.dataset_tools.context_rcnn import \
        generate_embedding_data  # pylint:disable=g-import-not-at-top
...
class FakeModel(model.DetectionModel):
    ...
    def postprocess(self, prediction_dict, true_image_shapes):
        with tf.control_dependencies(prediction_dict.values()):
            num_features = 100
            feature_dims = 10
            classifier_feature = np.ones((2, feature_dims, feature_dims, num_features), dtype=np.float32).tolist()
            postprocessed_tensors = {
                "detection_boxes": tf.constant([[[0.0, 0.1, 0.5, 0.6], [0.5, 0.5, 0.8, 0.8]]], tf.float32),
                "detection_scores": tf.constant([[0.95, 0.6]], tf.float32),
                "detection_multiclass_scores": tf.constant([[[0.1, 0.7, 0.2], [0.3, 0.1, 0.6]]], tf.float32),
...
class GenerateEmbeddingData(tf.test.TestCase):
    def _save_checkpoint_from_mock_model(self, checkpoint_path):
        ...
        exported_checkpoint_manager = tf.train.CheckpointManager(ckpt, checkpoint_path, max_to_keep=1)
    ...
    def assert_expected_example(self, example, topk=False, botk=False):
        # Check embeddings
        if topk or botk:
            self.assertEqual(len(example.features.feature["image/embedding"].float_list.value), 218)
            self.assertAllEqual(example.features.feature["image/embedding_count"].int64_list.value, [2])
        else:
            self.assertEqual(len(example.features.feature["image/embedding"].float_list.value), 109)
            self.assertAllEqual(example.features.feature["image/embedding_count"].int64_list.value, [1])

        self.assertAllEqual(example.features.feature["image/embedding_length"].int64_list.value, [109])

        # Check annotations
        self.assertAllClose(example.features.feature["image/object/bbox/ymin"].float_list.value, [0.0])
        self.assertAllClose(example.features.feature["image/object/bbox/xmin"].float_list.value, [0.1])
        self.assertAllClose(example.features.feature["image/object/bbox/ymax"].float_list.value, [0.5])
        self.assertAllClose(example.features.feature["image/object/bbox/xmax"].float_list.value, [0.6])
        self.assertAllClose(example.features.feature["image/object/class/score"].float_list.value, [0.95])
        self.assertAllClose(example.features.feature["image/object/class/label"].int64_list.value, [5])
        self.assertAllEqual(example.features.feature["image/object/class/text"].bytes_list.value, [b"hyena"])
        self.assertAllClose(example.features.feature["image/class/label"].int64_list.value, [5])
        self.assertAllEqual(example.features.feature["image/class/text"].bytes_list.value, [b"hyena"])

        # Check other essential attributes.
        self.assertAllEqual(example.features.feature["image/height"].int64_list.value, [400])
        self.assertAllEqual(example.features.feature["image/width"].int64_list.value, [600])
        self.assertAllEqual(example.features.feature["image/source_id"].bytes_list.value, [b"image_id"])
    ...
    def test_generate_embedding_data_fn(self):
        ...
        self.assertAllEqual(
            tf.train.Example.FromString(generated_example).features.feature["image/object/class/text"].bytes_list.value,
            [b"hyena"],
        )
