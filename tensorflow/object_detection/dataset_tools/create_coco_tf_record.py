...
tf.flags.DEFINE_string("train_keypoint_annotations_file", "", "Training annotations JSON file.")
tf.flags.DEFINE_string("val_keypoint_annotations_file", "", "Validation annotations JSON file.")
# DensePose is only available for coco 2014.
tf.flags.DEFINE_string("train_densepose_annotations_file", "", "Training annotations JSON file for DensePose.")
tf.flags.DEFINE_string("val_densepose_annotations_file", "", "Validation annotations JSON file for DensePose.")
tf.flags.DEFINE_string("output_dir", "/tmp/", "Output data directory.")
# Whether to only produce images/annotations on person class (for keypoint /
# densepose task).
tf.flags.DEFINE_boolean(
    "remove_non_person_annotations", False, "Whether to remove all annotations for non-person objects."
)
tf.flags.DEFINE_boolean(
    "remove_non_person_images", False, "Whether to remove all examples that do not contain a person."
)
...
def create_tf_example(
    ...
    for object_annotations in annotations_list:
        ...
        if include_keypoint:
            ...
            if annotation_id in keypoint_annotations_dict:
                ...
                keypoints_x.extend([float(x_abs) / image_width for x_abs in keypoints_x_abs])
                keypoints_y_abs = keypoints[1::3]
                keypoints_y.extend([float(y_abs) / image_height for y_abs in keypoints_y_abs])
        ...
        if include_densepose:
            ...
            if annotation_id in densepose_annotations_dict and all(
                key in densepose_annotations_dict[annotation_id] for key in densepose_keys
            ):
                ...
                dp_part_index.extend([int(i - _DP_PART_ID_OFFSET) for i in dp_annotations["dp_I"]])
                ...
                dp_x_box_rel = [clip_to_unit(val / 256.0) for val in dp_annotations["dp_x"]]
                dp_x_norm = [(float(x) + x_box_rel * width) / image_width for x_box_rel in dp_x_box_rel]
                dp_y_box_rel = [clip_to_unit(val / 256.0) for val in dp_annotations["dp_y"]]
                dp_y_norm = [(float(y) + y_box_rel * height) / image_height for y_box_rel in dp_y_box_rel]
    ...
    if remove_non_person_images and not any(name == b"person" for name in category_names):
        return (key, None, num_annotations_skipped, num_keypoint_annotation_skipped, num_densepose_annotation_skipped)
    feature_dict = {
    ...
    if include_keypoint:
        ...
        feature_dict["image/object/keypoint/num"] = dataset_util.int64_list_feature(num_keypoints)
        feature_dict["image/object/keypoint/visibility"] = dataset_util.int64_list_feature(keypoints_visibility)
        feature_dict["image/object/keypoint/text"] = dataset_util.bytes_list_feature(keypoints_name)
        num_keypoint_annotation_skipped = len(keypoint_annotations_dict) - num_keypoint_annotation_used
    if include_densepose:
        feature_dict["image/object/densepose/num"] = dataset_util.int64_list_feature(dp_num_points)
        feature_dict["image/object/densepose/part_index"] = dataset_util.int64_list_feature(dp_part_index)
        ...
        num_densepose_annotation_skipped = len(densepose_annotations_dict) - num_densepose_annotation_used

    example = tf.train.Example(features=tf.train.Features(feature=feature_dict))
    return (key, example, num_annotations_skipped, num_keypoint_annotation_skipped, num_densepose_annotation_skipped)
...
def _create_tf_record_from_coco_annotations(
    ...
    with contextlib2.ExitStack() as tf_record_close_stack, tf.gfile.GFile(annotations_file, "r") as fid:
        ...
        if densepose_annotations_file:
            logging.info("Finished writing, skipped %d DensePose annotations.", total_num_densepose_annotations_skipped)
...
def main(_):
    ...
    _create_tf_record_from_coco_annotations(
        FLAGS.testdev_annotations_file, FLAGS.test_image_dir, testdev_output_path, FLAGS.include_masks, num_shards=50
    )
