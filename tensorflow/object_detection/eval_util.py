...
def visualize_detection_results(
    ...
    if export_dir:
        if keep_image_id_for_visualization_export and result_dict[fields.InputDataFields().key]:
            export_path = os.path.join(
                export_dir, "export-{}-{}.png".format(tag, result_dict[fields.InputDataFields().key])
            )
        else:
    ...
    summary = tf.Summary(
        value=[
            tf.Summary.Value(
                tag=tag, image=tf.Summary.Image(encoded_image_string=vis_utils.encode_image_array_as_png_str(image))
            )
        ]
    )
    summary_writer = tf.summary.FileWriterCache.get(summary_dir)
    ...
    with slim.queues.QueueRunners(sess):
        try:
            ...
            if eval_export_path and eval_export_path is not None:
                for evaluator in evaluators:
                    if isinstance(evaluator, coco_evaluation.CocoDetectionEvaluator) or isinstance(
...
def repeated_checkpoint_run(
    ...
    while True:
        ...
        if not model_path:
            tf.logging.info(
                "No model found in %s. Will try again in %d seconds", checkpoint_dirs[0], eval_interval_secs
            )
...
def resize_detection_masks(detection_boxes, detection_masks, original_image_spatial_shapes):
    ...
    return shape_utils.static_or_dynamic_map_fn(
        _resize_detection_masks, elems=[detection_boxes, detection_masks, original_image_spatial_shapes], dtype=tf.uint8
    )
...
def _resize_groundtruth_masks(args):
    ...
    mask = tf.image.resize_images(
        mask, original_image_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR, align_corners=True
    )
...
def result_dict_for_single_example(
    image, key, detections, groundtruth=None, class_agnostic=False, scale_to_absolute=False
):
...
def result_dict_for_batched_example(
    ...
    if original_image_spatial_shapes is None:
        original_image_spatial_shapes = tf.tile(
            tf.expand_dims(tf.shape(images)[1:3], axis=0), multiples=[tf.shape(images)[0], 1]
        )
    else:
        if len(original_image_spatial_shapes.shape) != 2 and original_image_spatial_shapes.shape[1] != 2:
    ...
    if scale_to_absolute:
        output_dict[detection_fields.detection_boxes] = shape_utils.static_or_dynamic_map_fn(
            _scale_box_to_absolute, elems=[detection_boxes, original_image_spatial_shapes], dtype=tf.float32
        )
    else:
    ...
    if detection_fields.detection_masks in detections:
        ...
        if detection_fields.detection_surface_coords in detections:
            detection_surface_coords = detections[detection_fields.detection_surface_coords]
            output_dict[detection_fields.detection_surface_coords] = shape_utils.static_or_dynamic_map_fn(
                _resize_surface_coordinate_masks,
                elems=[detection_boxes, detection_surface_coords, original_image_spatial_shapes],
                dtype=tf.float32,
            )
    ...
    if detection_fields.detection_keypoints in detections:
        ...
        if scale_to_absolute:
            output_dict[detection_fields.detection_keypoints] = shape_utils.static_or_dynamic_map_fn(
    ...
    if groundtruth:
        ...
        if input_data_fields.groundtruth_instance_masks in groundtruth:
            ...
            groundtruth[input_data_fields.groundtruth_instance_masks] = shape_utils.static_or_dynamic_map_fn(
        ...
        groundtruth_boxes = shape_utils.static_or_dynamic_map_fn(
            _scale_box_to_normalized_true_image, elems=[groundtruth_boxes, true_image_shapes], dtype=tf.float32
        )
        ...
        if input_data_fields.groundtruth_keypoints in groundtruth:
            ...
            def _scale_keypoints_to_normalized_true_image(args):
                ...
                true_height, true_width = true_image_shape[0], true_image_shape[1]
                normalized_window = tf.stack([0.0, 0.0, true_height / image_height, true_width / image_width])
                return keypoint_ops.change_coordinate_frame(keypoints, normalized_window)
        ...
        if scale_to_absolute:
            ...
            output_dict[input_data_fields.groundtruth_boxes] = shape_utils.static_or_dynamic_map_fn(
                _scale_box_to_absolute, elems=[groundtruth_boxes, original_image_spatial_shapes], dtype=tf.float32
            )
            if input_data_fields.groundtruth_keypoints in groundtruth:
                groundtruth_keypoints = output_dict[input_data_fields.groundtruth_keypoints]
                output_dict[input_data_fields.groundtruth_keypoints] = shape_utils.static_or_dynamic_map_fn(
                    _scale_keypoint_to_absolute,
            ...
            if parameterized_metric.WhichOneof("parameterized_metric") == EVAL_KEYPOINT_METRIC:
                ...
                evaluators_list.append(
                    EVAL_METRICS_CLASS_DICT[EVAL_KEYPOINT_METRIC](
                        category["id"], category_keypoints, class_label, keypoint_metrics.keypoint_label_to_sigmas
                    )
                )
