...
class ContextRCNNMetaArch(faster_rcnn_meta_arch.FasterRCNNMetaArch):
    ...
    def get_side_inputs(features):
        ...
        if (
            ...
            raise ValueError("Please make sure context_features and valid_context_size are in the features")

        return {
            fields.InputDataFields.context_features: features[fields.InputDataFields.context_features],
            fields.InputDataFields.valid_context_size: features[fields.InputDataFields.valid_context_size],
        }

    def _compute_second_stage_input_feature_maps(
        self, features_to_crop, proposal_boxes_normalized, image_shape, context_features, valid_context_size
    ):
        ...
        box_features = self._crop_and_resize_fn(
            features_to_crop, proposal_boxes_normalized, None, [self._initial_crop_size, self._initial_crop_size]
        )

        attention_features = self._context_feature_extract_fn(
            box_features=box_features, context_features=context_features, valid_context_size=valid_context_size
        )
