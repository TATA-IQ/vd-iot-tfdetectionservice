...
from object_detection.core import losses, post_processing
...
class ContextRCNNMetaArchTest(test_case.TestCase, parameterized.TestCase):
    ...
    def test_prediction_mock_tf1(self, mock_context_rcnn_lib_v1):
        ...
        context_features = tf.random_uniform((2, 20, 10), minval=0, maxval=255, dtype=tf.float32)
        valid_context_size = tf.random_uniform((2,), minval=0, maxval=10, dtype=tf.int32)
        features = {
    ...
    def test_prediction_end_to_end(self, static_shapes):
        ...
        def graph_fn():
            ...
            side_inputs = model.get_side_inputs(features)
            prediction_dict = model.predict(preprocessed_inputs, true_image_shapes, **side_inputs)
            return (
