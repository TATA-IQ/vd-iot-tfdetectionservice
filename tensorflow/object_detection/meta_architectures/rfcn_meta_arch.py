...
class RFCNMetaArch(faster_rcnn_meta_arch.FasterRCNNMetaArch):
    ...
    def _predict_second_stage(
        ...
        class_predictions_with_background = tf.squeeze(
            tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1), axis=1
        )

        absolute_proposal_boxes = ops.normalized_to_image_coordinates(
            proposal_boxes_normalized, image_shape, parallel_iterations=self._parallel_iterations
        )
        ...
        if self._return_raw_detections_during_predict:
            prediction_dict.update(
                self._raw_detections_and_feature_map_inds(refined_box_encodings, absolute_proposal_boxes)
            )
