...
from object_detection.core import box_list, box_list_ops, matcher, model
...
class SSDMetaArch(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        if self._return_raw_detections_during_predict:
            predictions_dict.update(
                self._raw_detections_and_feature_map_inds(predictions_dict["box_encodings"], boxlist_list)
            )
    ...
    def _get_feature_map_spatial_dims(self, feature_maps):
        ...
        """
        feature_map_shapes = [
            shape_utils.combined_static_and_dynamic_shape(feature_map) for feature_map in feature_maps
        ]
    ...
    def postprocess(self, prediction_dict, true_image_shapes):
        ...
        with tf.name_scope("Postprocessor"):
            ...
            if (
                ...
                detection_dict[fields.DetectionResultFields.detection_multiclass_scores] = nmsed_additional_fields[
            ...
            if nmsed_additional_fields is not None and fields.BoxListFields.keypoints in nmsed_additional_fields:
                detection_dict[fields.DetectionResultFields.detection_keypoints] = nmsed_additional_fields[
    ...
    def loss(self, prediction_dict, true_image_shapes, scope=None):
        ...
        with tf.name_scope(scope, "Loss", prediction_dict.values()):
            ...
            classification_loss = tf.multiply(
                (self._classification_loss_weight / normalizer), classification_loss, name="classification_loss"
            )
    ...
    def _minibatch_subsample_fn(self, inputs):
        ...
        return self._random_example_sampler.subsample(
            tf.cast(cls_weights, tf.bool), batch_size=None, labels=tf.cast(positives_indicator, tf.bool)
        )
    ...
    def _summarize_target_assignment(self, groundtruth_boxes_list, match_list):
        ...
        avg_num_gt_boxes = tf.reduce_mean(
            tf.cast(tf.stack([tf.shape(x)[0] for x in groundtruth_boxes_list]), dtype=tf.float32)
        )
        avg_num_matched_gt_boxes = tf.reduce_mean(
            tf.cast(tf.stack([match.num_matched_rows() for match in match_list]), dtype=tf.float32)
        )
        avg_pos_anchors = tf.reduce_mean(
            tf.cast(tf.stack([match.num_matched_columns() for match in match_list]), dtype=tf.float32)
        )
        avg_neg_anchors = tf.reduce_mean(
            tf.cast(tf.stack([match.num_unmatched_columns() for match in match_list]), dtype=tf.float32)
        )
        avg_ignored_anchors = tf.reduce_mean(
            tf.cast(tf.stack([match.num_ignored_columns() for match in match_list]), dtype=tf.float32)
        )
    ...
    def _batch_decode(self, box_encodings, anchors):
        ...
        if decoded_boxes.has_field(fields.BoxListFields.keypoints):
            ...
            decoded_keypoints = tf.reshape(
                decoded_keypoints, tf.stack([combined_shape[0], combined_shape[1], num_keypoints, 2])
            )
