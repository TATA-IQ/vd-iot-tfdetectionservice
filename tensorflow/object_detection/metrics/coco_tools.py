...
class COCOWrapper(coco.COCO):
    ...
    def __init__(self, dataset, detection_type="bbox"):
        ...
        if detection_type not in supported_detection_types:
            raise ValueError(
                "Unsupported detection type: {}. Supported values are: {}".format(
...
class COCOEvalWrapper(cocoeval.COCOeval):
    ...
    def ComputeMetrics(self, include_metrics_per_category=False, all_metrics_per_category=False, super_categories=None):
            ...
            if super_categories:
                for key in super_categories:
                    if category in super_categories[key]:
                        ...
                        super_category_ap[metric_name] += self.category_stats[0][category_index]
...
def ExportGroundtruthToCOCO(image_ids, groundtruth_boxes, groundtruth_classes, categories, output_path=None):
    ...
    for image_id, boxes, classes in zip(image_ids, groundtruth_boxes, groundtruth_classes):
        ...
        groundtruth_export_list.extend(
            ExportSingleImageGroundtruthToCoco(image_id, annotation_id, category_id_set, boxes, classes)
        )
...
def ExportDetectionsToCOCO(
    image_ids, detection_boxes, detection_scores, detection_classes, categories, output_path=None
):
    ...
    for image_id, boxes, scores, classes in zip(image_ids, detection_boxes, detection_scores, detection_classes):
        detections_export_list.extend(
            ExportSingleImageDetectionBoxesToCoco(image_id, category_id_set, boxes, scores, classes)
        )
    if output_path:
        with tf.gfile.GFile(output_path, "w") as fid:
...
def ExportSegmentsToCOCO(image_ids, detection_masks, detection_scores, detection_classes, categories, output_path=None):
    ...
    for image_id, masks, scores, classes in zip(image_ids, detection_masks, detection_scores, detection_classes):
        ...
        segment_export_list.extend(
            ExportSingleImageDetectionMasksToCoco(image_id, category_id_set, np.squeeze(masks, axis=3), scores, classes)
        )
...
def ExportKeypointsToCOCO(
    image_ids, detection_keypoints, detection_scores, detection_classes, categories, output_path=None
):
    ...
    for image_id, keypoints, scores, classes in zip(
        ...
        category_id_to_num_keypoints_map = {
            cat["id"]: cat["num_keypoints"] for cat in categories if "num_keypoints" in cat
        }
        ...
        for i in range(num_boxes):
            ...
            if classes[i] in category_id_to_num_keypoints_map:
                ...
                instance_keypoints = np.concatenate(
                    [keypoints[i, 0:num_keypoints, :], np.expand_dims(np.ones(num_keypoints), axis=1)], axis=1
                ).astype(int)
