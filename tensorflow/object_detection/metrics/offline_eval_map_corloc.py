...
def _generate_sharded_filenames(filename):
    ...
    if m:
        ...
        return [re.sub(r"@(\d{1,})", "-%.5d-of-%.5d" % (i, num_shards), filename) for i in range(num_shards)]
    else:
        return [filename]
...
def main(argv):
    ...
    configs = config_util.get_configs_from_multiple_files(
        eval_input_config_path=FLAGS.input_config_path, eval_config_path=FLAGS.eval_config_path
    )
