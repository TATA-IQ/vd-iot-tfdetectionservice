...
def merge_boxes_and_masks(box_data, mask_data):
    return pd.merge(
        box_data, mask_data, how="outer", on=["LabelName", "ImageID", "XMin", "XMax", "YMin", "YMax", "IsGroupOf"]
    )
...
def build_groundtruth_dictionary(data, class_label_map):
    ...
    dictionary = {
        standard_fields.InputDataFields.groundtruth_boxes: data_location[["YMin", "XMin", "YMax", "XMax"]].to_numpy(),
        standard_fields.InputDataFields.groundtruth_classes: data_location["LabelName"]
        .map(lambda x: class_label_map[x])
        .to_numpy(),
        standard_fields.InputDataFields.groundtruth_group_of: data_location["IsGroupOf"].to_numpy().astype(int),
        standard_fields.InputDataFields.groundtruth_image_classes: data_labels["LabelName"]
        .map(lambda x: class_label_map[x])
        .to_numpy(),
    }
    ...
    if "Mask" in data_location:
        segments, _ = _decode_raw_data_into_masks_and_boxes(
            data_location["Mask"], data_location["ImageWidth"], data_location["ImageHeight"]
        )
...
def build_predictions_dictionary(data, class_label_map):
    ...
    dictionary = {
        ...
        .map(lambda x: class_label_map[x])
        .to_numpy(),
        standard_fields.DetectionResultFields.detection_scores: data["Score"].to_numpy(),
    }
