...
class FloatParser(data_parser.DataToNumpyParser):
    ...
    def parse(self, tf_example):
        return (
            np.array(tf_example.features.feature[self.field_name].float_list.value, dtype=np.float).transpose()
...
class Int64Parser(data_parser.DataToNumpyParser):
    ...
    def parse(self, tf_example):
        return (
            np.array(tf_example.features.feature[self.field_name].int64_list.value, dtype=np.int64).transpose()
...
class TfExampleDetectionAndGTParser(data_parser.DataToNumpyParser):
    ...
    def __init__(self):
        ...
        self.optional_items_to_handlers = {
            fields.InputDataFields.groundtruth_difficult: Int64Parser(fields.TfExampleFields.object_difficult),
