...
def _prepare_groundtruth_for_eval(detection_model, class_agnostic, max_number_of_boxes):
    ...
    if detection_model.groundtruth_has_field(fields.InputDataFields.groundtruth_labeled_classes):
        labeled_classes_list = detection_model.groundtruth_lists(fields.InputDataFields.groundtruth_labeled_classes)
        ...
        if len(labeled_classes) > 1:
            ...
            groundtruth[input_data_fields.groundtruth_labeled_classes] = tf.stack(padded_labeled_classes)
        else:
    ...
    if detection_model.groundtruth_has_field(fields.BoxListFields.densepose_surface_coords):
        groundtruth[input_data_fields.groundtruth_dp_surface_coords] = tf.stack(
            detection_model.groundtruth_lists(fields.BoxListFields.densepose_surface_coords)
        )
...
def unstack_batch(tensor_dict, unpad_groundtruth_tensors=True):
    ...
    if unpad_groundtruth_tensors:
        if fields.InputDataFields.num_groundtruth_boxes not in unbatched_tensor_dict:
            raise ValueError(
                "`num_groundtruth_boxes` not found in tensor_dict. Keys available: {}".format(
        ...
        for key in unpad_keys:
            ...
            for num_gt, padded_tensor in zip(
                unbatched_tensor_dict[fields.InputDataFields.num_groundtruth_boxes], unbatched_tensor_dict[key]
            ):
...
def create_model_fn(detection_model_fn, configs, hparams=None, use_tpu=False, postprocess_on_cpu=False):
    ...
    def model_fn(features, labels, mode, params=None):
        ...
        if use_tpu and train_config.use_bfloat16:
            with tf.tpu.bfloat16_scope():
                prediction_dict = detection_model.predict(
                    preprocessed_images, features[fields.InputDataFields.true_image_shape], **side_inputs
                )
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if train_config.fine_tune_checkpoint and load_pretrained:
                ...
                available_var_map = variables_helper.get_variables_available_in_checkpoint(
                    asg_map, train_config.fine_tune_checkpoint, include_global_step=False
                )
                if use_tpu:
        ...
        if mode in (tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL):
            if mode == tf.estimator.ModeKeys.EVAL and eval_config.use_dummy_loss_in_eval:
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            trainable_variables = slim.filter_variables(
                tf.trainable_variables(), include_patterns=include_variables, exclude_patterns=exclude_variables
            )
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            if eval_config.use_moving_averages:
                ...
                saver = tf.train.Saver(
                    variables_to_restore, keep_checkpoint_every_n_hours=keep_checkpoint_every_n_hours
                )
        ...
        if use_tpu and mode != tf.estimator.ModeKeys.EVAL:
            ...
            if scaffold is None:
                keep_checkpoint_every_n_hours = train_config.keep_checkpoint_every_n_hours
                saver = tf.train.Saver(
                    sharded=True, keep_checkpoint_every_n_hours=keep_checkpoint_every_n_hours, save_relative_paths=True
                )
...
def create_estimator_and_inputs(
    ...
    **kwargs,
    ...
    train_input_fn = create_train_input_fn(
        train_config=train_config, train_input_config=train_input_config, model_config=model_config
    )
    eval_input_fns = [
        create_eval_input_fn(eval_config=eval_config, eval_input_config=eval_input_config, model_config=model_config)
    ...
    eval_on_train_input_fn = create_eval_input_fn(
        eval_config=eval_config, eval_input_config=eval_on_train_input_config, model_config=model_config
    )
...
def create_train_and_eval_specs(
    ...
    for index, (eval_spec_name, eval_input_fn) in enumerate(zip(eval_spec_names, eval_input_fns)):
        ...
        eval_specs.append(
            tf.estimator.EvalSpec(name=eval_spec_name, input_fn=eval_input_fn, steps=None, exporters=exporter)
        )
...
def _evaluate_checkpoint(estimator, input_fn, checkpoint_path, name, max_retries=0):
    ...
    while always_retry or retries <= max_retries:
        try:
            return estimator.evaluate(input_fn=input_fn, steps=None, checkpoint_path=checkpoint_path, name=name)
    ...
    for ckpt in tf.train.checkpoints_iterator(
        ...
        try:
            eval_results = _evaluate_checkpoint(
                estimator=estimator, input_fn=input_fn, checkpoint_path=ckpt, name=name, max_retries=max_retries
            )
...
def populate_experiment(
    ...
    **kwargs,
    ...
    export_strategies = [
        contrib_learn.utils.saved_model_export_utils.make_export_strategy(serving_input_fn=predict_input_fn)
    ]
