...
flags.DEFINE_integer(
    "train_batch_size",
    None,
    "Batch size for training. If this is not provided, batch size is read from training config.",
)
flags.DEFINE_integer("num_train_steps", None, "Number of train steps.")
flags.DEFINE_boolean("eval_training_data", False, "If training data should be evaluated for this job.")
flags.DEFINE_integer(
    "sample_1_of_n_eval_examples", 1, "Will sample one of every n eval input examples, where n is provided."
)
flags.DEFINE_integer(
...
def main(unused_argv):
    ...
    if FLAGS.mode == "eval":
        ...
        model_lib.continuous_eval(estimator, FLAGS.model_dir, input_fn, train_steps, name, FLAGS.max_eval_retries)
