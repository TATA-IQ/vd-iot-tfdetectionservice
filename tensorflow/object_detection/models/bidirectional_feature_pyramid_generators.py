...
def _create_bifpn_input_config(fpn_min_level, fpn_max_level, input_max_level, level_scales=None):
    ...
    for i in range(fpn_min_level, min(fpn_max_level, input_max_level) + 1):
        bifpn_input_params.append({"name": "0_up_lvl_{}".format(i), "scale": level_scales[i - fpn_min_level]})
...
def _create_bifpn_node_config(
    ...
    default_node_params = {
        ...
        "input_op": functools.partial(_create_bifpn_resample_block, downsample_method="max_pooling"),
...
def _create_bifpn_combine_op(num_inputs, name, combine_method):
    ...
    if num_inputs < 1:
        ...
        combine_op = bifpn_utils.BiFPNCombineLayer(combine_method=combine_method, name=name)
    return combine_op
...
class KerasBiFpnFeatureMaps(tf.keras.Model):
    ...
    def __init__(
        ...
        bifpn_node_config = _create_bifpn_node_config(
            bifpn_num_iterations, bifpn_num_filters, fpn_min_level, fpn_max_level, input_max_level, bifpn_node_params
        )
        bifpn_input_config = _create_bifpn_input_config(fpn_min_level, fpn_max_level, input_max_level)
        bifpn_output_node_names = _get_bifpn_output_node_names(fpn_min_level, fpn_max_level, bifpn_node_config)
        ...
        for node_config in bifpn_node_config:
            ...
            for input_name in node_config["inputs"]:
                if input_name not in all_node_names:
                    raise ValueError("Input feature map ({}) does not exist:".format(input_name))
