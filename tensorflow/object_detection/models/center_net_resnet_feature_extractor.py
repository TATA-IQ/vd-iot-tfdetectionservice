...
class CenterNetResnetFeatureExtractor(CenterNetFeatureExtractor):
    ...
    def __init__(self, resnet_type, channel_means=(0.0, 0.0, 0.0), channel_stds=(1.0, 1.0, 1.0), bgr_ordering=False):
        ...
        if resnet_type == "resnet_v2_101":
            self._base_model = tf.keras.applications.ResNet101V2(weights=None, include_top=False)
            output_layer = "conv5_block3_out"
        elif resnet_type == "resnet_v2_50":
            self._base_model = tf.keras.applications.ResNet50V2(weights=None, include_top=False)
            output_layer = "conv5_block3_out"
        else:
            raise ValueError("Unknown Resnet Model {}".format(resnet_type))
        output_layer = self._base_model.get_layer(output_layer)

        self._resnet_model = tf.keras.models.Model(inputs=self._base_model.input, outputs=output_layer.output)
        ...
        for num_filters in [256, 128, 64]:
            ...
            conv = tf.keras.layers.Conv2D(filters=num_filters, kernel_size=3, strides=1, padding="same")
        ...
        self._feature_extractor_model = tf.keras.models.Model(inputs=self._base_model.input, outputs=resnet_output)
    ...
    def preprocess(self, resized_inputs):
        ...
        """
        resized_inputs = super(CenterNetResnetFeatureExtractor, self).preprocess(resized_inputs)
...
def resnet_v2_101(channel_means, channel_stds, bgr_ordering):
    ...
    return CenterNetResnetFeatureExtractor(
        resnet_type="resnet_v2_101", channel_means=channel_means, channel_stds=channel_stds, bgr_ordering=bgr_ordering
    )
...
def resnet_v2_50(channel_means, channel_stds, bgr_ordering):
    ...
    return CenterNetResnetFeatureExtractor(
        resnet_type="resnet_v2_50", channel_means=channel_means, channel_stds=channel_stds, bgr_ordering=bgr_ordering
    )
