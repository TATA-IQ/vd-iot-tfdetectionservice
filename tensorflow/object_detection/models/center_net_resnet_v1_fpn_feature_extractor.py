...
_RESNET_MODEL_OUTPUT_LAYERS = {
    "resnet_v1_18": ["conv2_block2_out", "conv3_block2_out", "conv4_block2_out", "conv5_block2_out"],
    "resnet_v1_34": ["conv2_block3_out", "conv3_block4_out", "conv4_block6_out", "conv5_block3_out"],
    "resnet_v1_50": ["conv2_block3_out", "conv3_block4_out", "conv4_block6_out", "conv5_block3_out"],
    "resnet_v1_101": ["conv2_block3_out", "conv3_block4_out", "conv4_block23_out", "conv5_block3_out"],
}
...
class CenterNetResnetV1FpnFeatureExtractor(CenterNetFeatureExtractor):
    ...
    def __init__(self, resnet_type, channel_means=(0.0, 0.0, 0.0), channel_stds=(1.0, 1.0, 1.0), bgr_ordering=False):
        ...
        if resnet_type == "resnet_v1_50":
            self._base_model = tf.keras.applications.ResNet50(weights=None, include_top=False)
        elif resnet_type == "resnet_v1_101":
            self._base_model = tf.keras.applications.ResNet101(weights=None, include_top=False)
        ...
        outputs = [self._base_model.get_layer(output_layer_name).output for output_layer_name in output_layers]

        self._resnet_model = tf.keras.models.Model(inputs=self._base_model.input, outputs=outputs)
        resnet_outputs = self._resnet_model(self._base_model.input)

        # Construct the top-down feature maps.
        top_layer = resnet_outputs[-1]
        residual_op = tf.keras.layers.Conv2D(filters=256, kernel_size=1, strides=1, padding="same")
        ...
        for i, num_filters in enumerate(num_filters_list):
            ...
            residual_op = tf.keras.layers.Conv2D(filters=num_filters, kernel_size=1, strides=1, padding="same")
            ...
            conv = tf.keras.layers.Conv2D(filters=next_num_filters, kernel_size=3, strides=1, padding="same")
            top_down = conv(top_down)
            top_down = tf.keras.layers.BatchNormalization()(top_down)
            top_down = tf.keras.layers.ReLU()(top_down)

        self._feature_extractor_model = tf.keras.models.Model(inputs=self._base_model.input, outputs=top_down)
    ...
    def preprocess(self, resized_inputs):
        ...
        """
        resized_inputs = super(CenterNetResnetV1FpnFeatureExtractor, self).preprocess(resized_inputs)
...
def resnet_v1_101_fpn(channel_means, channel_stds, bgr_ordering):
    ...
    return CenterNetResnetV1FpnFeatureExtractor(
        resnet_type="resnet_v1_101", channel_means=channel_means, channel_stds=channel_stds, bgr_ordering=bgr_ordering
    )
...
def resnet_v1_50_fpn(channel_means, channel_stds, bgr_ordering):
    ...
    return CenterNetResnetV1FpnFeatureExtractor(
        resnet_type="resnet_v1_50", channel_means=channel_means, channel_stds=channel_stds, bgr_ordering=bgr_ordering
    )
...
def resnet_v1_34_fpn(channel_means, channel_stds, bgr_ordering):
    ...
    return CenterNetResnetV1FpnFeatureExtractor(
        resnet_type="resnet_v1_34", channel_means=channel_means, channel_stds=channel_stds, bgr_ordering=bgr_ordering
    )
...
def resnet_v1_18_fpn(channel_means, channel_stds, bgr_ordering):
    ...
    return CenterNetResnetV1FpnFeatureExtractor(
        resnet_type="resnet_v1_18", channel_means=channel_means, channel_stds=channel_stds, bgr_ordering=bgr_ordering
    )
