...
class FasterRCNNInceptionV2FeatureExtractor(faster_rcnn_meta_arch.FasterRCNNFeatureExtractor):
        ...
        with tf.control_dependencies([shape_assert]):
            with tf.variable_scope("InceptionV2", reuse=self._reuse_weights) as scope:
                with _batch_norm_arg_scope(
                    [slim.conv2d, slim.separable_conv2d], batch_norm_scale=True, train_batch_norm=self._train_batch_norm
                ):
    ...
    def _extract_box_classifier_features(self, proposal_feature_maps, scope):
        ...
        with tf.variable_scope("InceptionV2", reuse=self._reuse_weights):
            with slim.arg_scope(
                [slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding="SAME", data_format=data_format
            ):
                with _batch_norm_arg_scope(
                    [slim.conv2d, slim.separable_conv2d], batch_norm_scale=True, train_batch_norm=self._train_batch_norm
                ):
                    with tf.variable_scope("Mixed_5a"):
                        with tf.variable_scope("Branch_0"):
                            branch_0 = slim.conv2d(
                                net, depth(128), [1, 1], weights_initializer=trunc_normal(0.09), scope="Conv2d_0a_1x1"
                            )
                            branch_0 = slim.conv2d(branch_0, depth(192), [3, 3], stride=2, scope="Conv2d_1a_3x3")
                        with tf.variable_scope("Branch_1"):
                            branch_1 = slim.conv2d(
                                net, depth(192), [1, 1], weights_initializer=trunc_normal(0.09), scope="Conv2d_0a_1x1"
                            )
                    ...
                    with tf.variable_scope("Mixed_5b"):
                        ...
                        with tf.variable_scope("Branch_2"):
                            branch_2 = slim.conv2d(
                                net, depth(160), [1, 1], weights_initializer=trunc_normal(0.09), scope="Conv2d_0a_1x1"
                            )
