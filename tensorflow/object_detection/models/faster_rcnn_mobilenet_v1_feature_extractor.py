...
class FasterRCNNMobilenetV1FeatureExtractor(faster_rcnn_meta_arch.FasterRCNNFeatureExtractor):
    ...
    def _extract_proposal_features(self, preprocessed_inputs, scope):
        ...
        with slim.arg_scope(
            mobilenet_v1.mobilenet_v1_arg_scope(is_training=self._train_batch_norm, weight_decay=self._weight_decay)
        ):
            ...
            with slim.arg_scope(
                ...
                with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding="SAME"):
                    net = slim.separable_conv2d(
                        net, depth(conv_depth), [3, 3], depth_multiplier=1, stride=2, scope="Conv2d_12_pointwise"
                    )
                    return slim.separable_conv2d(
                        net, depth(conv_depth), [3, 3], depth_multiplier=1, stride=1, scope="Conv2d_13_pointwise"
                    )
