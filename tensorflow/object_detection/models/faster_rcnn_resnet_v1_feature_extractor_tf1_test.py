...
class FasterRcnnResnetV1FeatureExtractorTest(tf.test.TestCase):
    def _build_feature_extractor(
        self, first_stage_features_stride, activation_fn=tf.nn.relu, architecture="resnet_v1_101"
    ):
    ...
    def test_extract_box_classifier_features_returns_expected_size(self):
        ...
        proposal_classifier_features = feature_extractor.extract_box_classifier_features(
    ...
    def test_overwriting_activation_fn(self):
        for architecture in ["resnet_v1_50", "resnet_v1_101", "resnet_v1_152"]:
            feature_extractor = self._build_feature_extractor(
                first_stage_features_stride=16, architecture=architecture, activation_fn=tf.nn.relu6
            )
            preprocessed_inputs = tf.random_uniform([4, 224, 224, 3], maxval=255, dtype=tf.float32)
