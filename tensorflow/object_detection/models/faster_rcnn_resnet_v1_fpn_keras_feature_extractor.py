...
_RESNET_MODEL_OUTPUT_LAYERS = {
    ...
    "resnet_v1_152": ["conv2_block3_out", "conv3_block8_out", "conv4_block36_out", "conv5_block3_out"],
}
...
class _ResnetFPN(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        fpn_input_image_features = [
            (feature_block, feature_block_map[feature_block]) for feature_block in feature_block_list
        ]
...
class FasterRCNNResnetV1FpnKerasFeatureExtractor(faster_rcnn_meta_arch.FasterRCNNKerasFeatureExtractor):
        ...
        with tf.name_scope(name):
            with tf.name_scope("ResnetV1FPN"):
                full_resnet_v1_model = self._resnet_v1_base_model(
                    ...
                    conv_hyperparams=(
                        self._conv_hyperparams if self._override_base_feature_extractor_hyperparams else None
                    ),
                    classes=None,
                    weights=None,
                    include_top=False,
                ...
                outputs = [
                    full_resnet_v1_model.get_layer(output_layer_name).output for output_layer_name in output_layers
                ]
                self.classification_backbone = tf.keras.Model(inputs=full_resnet_v1_model.inputs, outputs=outputs)

                self._base_fpn_max_level = min(self._fpn_max_level, 5)
                self._num_levels = self._base_fpn_max_level + 1 - self._fpn_min_level
                self._fpn_features_generator = feature_map_generators.KerasFpnTopDownFeatureMaps(
                ...
                for i in range(self._base_fpn_max_level, self._fpn_max_level):
                    ...
                    layers.append(
                        self._conv_hyperparams.build_batch_norm(
                            training=(self._is_training and not self._freeze_batchnorm), name=layer_name + "_batchnorm"
                        )
                    )
...
class FasterRCNNResnet50FpnKerasFeatureExtractor(FasterRCNNResnetV1FpnKerasFeatureExtractor):
