...
def batchnorm():
    try:
        return tf.keras.layers.experimental.SyncBatchNormalization(name="batchnorm", epsilon=1e-5, momentum=0.1)
    except AttributeError:
        return tf.keras.layers.BatchNormalization(name="batchnorm", epsilon=1e-5, momentum=0.1, fused=BATCH_NORM_FUSED)
...
class ConvolutionalBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, kernel_size, out_channels, stride=1, relu=True, padding="same"):
        ...
        self.conv = tf.keras.layers.Conv2D(
            filters=out_channels, kernel_size=kernel_size, use_bias=False, strides=stride, padding=padding
        )
...
class SkipConvolution(ConvolutionalBlock):
    ...
    def __init__(self, out_channels, stride):
        ...
        """
        super(SkipConvolution, self).__init__(out_channels=out_channels, kernel_size=1, stride=stride, relu=False)


class ResidualBlock(tf.keras.layers.Layer):
    """A Residual block."""

    def __init__(self, out_channels, skip_conv=False, kernel_size=3, stride=1, padding="same"):
        ...
        self.conv_block = ConvolutionalBlock(kernel_size=kernel_size, out_channels=out_channels, stride=stride)

        self.conv = tf.keras.layers.Conv2D(
            filters=out_channels, kernel_size=kernel_size, use_bias=False, strides=1, padding=padding
        )
        self.norm = batchnorm()
...
class InputDownsampleBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, out_channels_initial_conv, out_channels_residual_block):
        ...
        self.conv_block = ConvolutionalBlock(
            kernel_size=7, out_channels=out_channels_initial_conv, stride=2, padding="valid"
        )
        self.residual_block = ResidualBlock(out_channels=out_channels_residual_block, stride=2, skip_conv=True)
...
class InputConvBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, out_channels_initial_conv, out_channels_residual_block):
        ...
        self.conv_block = ConvolutionalBlock(
            kernel_size=7, out_channels=out_channels_initial_conv, stride=1, padding="valid"
        )
        self.residual_block = ResidualBlock(out_channels=out_channels_residual_block, stride=1, skip_conv=True)

    def call(self, inputs):
        return self.residual_block(self.conv_block(inputs))


def _make_repeated_residual_blocks(out_channels, num_blocks, initial_stride=1, residual_channels=None):
    ...
    for i in range(num_blocks - 1):
        ...
        blocks.append(ResidualBlock(out_channels=residual_channels, stride=stride, skip_conv=skip_conv))
    ...
    blocks.append(ResidualBlock(out_channels=out_channels, skip_conv=skip_conv, stride=stride))
...
class EncoderDecoderBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, num_stages, channel_dims, blocks_per_stage):
        ...
        self.encoder_block1 = _make_repeated_residual_blocks(
            out_channels=out_channels, num_blocks=blocks_per_stage[0], initial_stride=1
        )
        self.encoder_block2 = _make_repeated_residual_blocks(
            out_channels=out_channels_downsampled, num_blocks=blocks_per_stage[0], initial_stride=2
        )

        if num_stages > 1:
            self.inner_block = [EncoderDecoderBlock(num_stages - 1, channel_dims[1:], blocks_per_stage[1:])]
        else:
        ...
        self.decoder_block = _make_repeated_residual_blocks(
            residual_channels=out_channels_downsampled, out_channels=out_channels, num_blocks=blocks_per_stage[0]
        )
    ...
    def call(self, inputs):
        ...
        inner_block_outputs = _apply_blocks(encoded_downsampled_outputs, self.inner_block)
...
class HourglassNetwork(tf.keras.Model):
    ...
    def __init__(self, num_stages, channel_dims, blocks_per_stage, num_hourglasses, downsample=True):
        ...
        if downsample:
            self.downsample_input = InputDownsampleBlock(
                out_channels_initial_conv=channel_dims[0], out_channels_residual_block=channel_dims[1]
            )
        else:
        ...
        for _ in range(self.num_hourglasses):
            self.hourglass_network.append(
                EncoderDecoderBlock(
                    num_stages=num_stages, channel_dims=channel_dims[1:], blocks_per_stage=blocks_per_stage
                )
            )
            self.output_conv.append(ConvolutionalBlock(kernel_size=3, out_channels=channel_dims[1]))
        ...
        for _ in range(self.num_hourglasses - 1):
            self.intermediate_conv1.append(ConvolutionalBlock(kernel_size=1, out_channels=channel_dims[1], relu=False))
            self.intermediate_conv2.append(ConvolutionalBlock(kernel_size=1, out_channels=channel_dims[1], relu=False))
            self.intermediate_residual.append(ResidualBlock(out_channels=channel_dims[1]))
    ...
    def call(self, inputs):
        ...
        for i in range(self.num_hourglasses):
            ...
            if i < self.num_hourglasses - 1:
                secondary_output = self.intermediate_conv1[i](inputs) + self.intermediate_conv2[i](output)
...
def _encoder_decoder_depth(network):
    ...
    if isinstance(network.inner_block[0], EncoderDecoderBlock):
        ...
        return encoder_block2_layers + decoder_block_layers + _layer_depth(network.inner_block)
...
def hourglass_depth(network):
    ...
    intermediate_layers = _layer_depth(network.intermediate_conv2) + _layer_depth(network.intermediate_residual)

    # network.output_conv is applied before sending input to the later stages
    output_layers = _layer_depth(network.output_conv)

    encoder_decoder_layers = sum(_encoder_decoder_depth(net) for net in network.hourglass_network)

    return input_conv_layers + encoder_decoder_layers + intermediate_layers + output_layers
