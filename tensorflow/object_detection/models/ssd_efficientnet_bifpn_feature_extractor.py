...
from object_detection.utils import ops, shape_utils, tf_version
...
class SSDEfficientNetBiFPNKerasFeatureExtractor(ssd_meta_arch.SSDKerasFeatureExtractor):
    ...
    def __init__(
        ...
        if depth_multiplier != 1.0:
            raise ValueError("EfficientNetBiFPN does not support a non-default depth_multiplier.")
        ...
        if override_base_feature_extractor_hyperparams:
            raise ValueError("EfficientNetBiFPN does not support override_base_feature_extractor_hyperparams.")
        ...
        logging.info("EfficientDet EfficientNet backbone version: %s", self._efficientnet_version)
        logging.info("EfficientDet BiFPN num filters: %d", self._bifpn_num_filters)
        logging.info("EfficientDet BiFPN num iterations: %d", self._bifpn_num_iterations)

        self._backbone_max_level = min(max(_EFFICIENTNET_LEVEL_ENDPOINTS.keys()), bifpn_max_level)
        self._output_layer_names = [
            _EFFICIENTNET_LEVEL_ENDPOINTS[i] for i in range(bifpn_min_level, self._backbone_max_level + 1)
        ]
        self._output_layer_alias = ["level_{}".format(i) for i in range(bifpn_min_level, self._backbone_max_level + 1)]
        ...
        efficientnet_base = efficientnet_model.EfficientNet.from_name(
            model_name=self._efficientnet_version, overrides={"rescale_input": False}
        )
        outputs = [
            efficientnet_base.get_layer(output_layer_name).output for output_layer_name in self._output_layer_names
        ]
        self._efficientnet = tf.keras.Model(inputs=efficientnet_base.inputs, outputs=outputs)
    ...
    def _extract_features(self, preprocessed_inputs):
        ...
        base_feature_maps = self._efficientnet(ops.pad_to_multiple(preprocessed_inputs, self._pad_to_multiple))

        output_feature_map_dict = self._bifpn_stage(list(zip(self._output_layer_alias, base_feature_maps)))

        return list(output_feature_map_dict.values())


class SSDEfficientNetB0BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
...
class SSDEfficientNetB1BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
...
class SSDEfficientNetB2BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
...
class SSDEfficientNetB3BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
...
class SSDEfficientNetB4BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
...
class SSDEfficientNetB5BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
...
class SSDEfficientNetB6BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
...
class SSDEfficientNetB7BiFPNKerasFeatureExtractor(SSDEfficientNetBiFPNKerasFeatureExtractor):
