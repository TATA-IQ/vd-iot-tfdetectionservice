                    ...
                    with slim.arg_scope([slim.dropout], is_training=self._is_training):
                        ...
                        if depth > 0 and self._num_layers_before_predictor > 0:
                            for i in range(self._num_layers_before_predictor):
                                net = slim.conv2d(
                                    net, depth, [1, 1], reuse=tf.AUTO_REUSE, scope="Conv2d_%d_1x1_%d" % (i, depth)
                                )
                        ...
                        for head_name in sorted_keys:
                            ...
                            prediction = head_obj.predict(
                                features=net, num_predictions_per_location=num_predictions_per_location
                            )
...
class WeightSharedConvolutionalBoxPredictor(box_predictor.BoxPredictor):
    ...
    def _insert_additional_projection_layer(self, image_feature, inserted_layer_counter, target_channel):
        ...
        if self._apply_batch_norm:
            image_feature = slim.batch_norm(
                image_feature, scope="ProjectionLayer/conv2d_{}/BatchNorm".format(inserted_layer_counter)
            )
    ...
    def _compute_base_tower(self, tower_name_scope, image_feature, feature_index):
        ...
        for i in range(self._num_layers_before_predictor):
            ...
            if self._apply_batch_norm:
                net = slim.batch_norm(
                    net, scope="{}/conv2d_{}/BatchNorm/feature_{}".format(tower_name_scope, i, feature_index)
                )
            net = tf.nn.relu6(net)
        return net

    def _predict_head(
        self, head_name, head_obj, image_feature, box_tower_feature, feature_index, num_predictions_per_location
    ):
        ...
        if self._share_prediction_tower:
            ...
            head_tower_feature = self._compute_base_tower(
                tower_name_scope=tower_name_scope, image_feature=image_feature, feature_index=feature_index
            )
    ...
    def _predict(self, image_features, num_predictions_per_location_list):
        ...
        if len(set(num_predictions_per_location_list)) > 1:
            raise ValueError(
                "num predictions per location must be same for allfeature maps, found: {}".format(
        ...
        for feature_index, (image_feature, num_predictions_per_location) in enumerate(
            ...
            with tf.variable_scope("WeightSharedConvolutionalBoxPredictor", reuse=tf.AUTO_REUSE):
                with slim.arg_scope(self._conv_hyperparams_fn()):
                    ...
                    with slim.arg_scope([slim.dropout], is_training=self._is_training):
                        (image_feature, inserted_layer_counter) = self._insert_additional_projection_layer(
                        ...
                        box_tower_feature = self._compute_base_tower(
                            tower_name_scope=box_tower_scope, image_feature=image_feature, feature_index=feature_index
                        )
                        box_encodings = self._box_prediction_head.predict(
                            features=box_tower_feature, num_predictions_per_location=num_predictions_per_location
                        )
