...
class ConvolutionalBoxPredictor(box_predictor.KerasBoxPredictor):
    ...
    def build(self, input_shapes):
        ...
        if len(input_shapes) != len(self._prediction_heads[BOX_ENCODINGS]):
            raise ValueError(
                "This box predictor was constructed with %d heads,but there are %d inputs."
                ...
                for i in range(self._num_layers_before_predictor):
                    net.append(
                        keras.Conv2D(
                            depth,
                            [1, 1],
                            name="SharedConvolutions_%d/Conv2d_%d_1x1_%d" % (stack_index, i, depth),
                            padding="SAME",
                    ...
                    net.append(
                        self._conv_hyperparams.build_batch_norm(
                            ...
                            name="SharedConvolutions_%d/Conv2d_%d_1x1_%d_norm" % (stack_index, i, depth),
                        )
                    )
                    net.append(
                        self._conv_hyperparams.build_activation_layer(
                            name="SharedConvolutions_%d/Conv2d_%d_1x1_%d_activation" % (stack_index, i, depth),
                        )
                    )
...
class WeightSharedConvolutionalBoxPredictor(box_predictor.KerasBoxPredictor):
    ...
    def _insert_additional_projection_layer(self, inserted_layer_counter, target_channel):
        ...
        if inserted_layer_counter >= 0:
            use_bias = False if (self._apply_batch_norm and not self._conv_hyperparams.force_use_bias()) else True
    ...
    def _compute_base_tower(self, tower_name_scope, feature_index):
        ...
        for additional_conv_layer_idx in range(self._num_layers_before_predictor):
            ...
            activation_layers.append(
                self._conv_hyperparams.build_activation_layer(
                    name="{}/conv2d_{}/activation_{}".format(tower_name_scope, additional_conv_layer_idx, feature_index)
                )
            )
    ...
    def build(self, input_shapes):
        ...
        for feature_index in range(len(input_shapes)):
            ...
            inserted_layer_counter, projection_layers = self._insert_additional_projection_layer(
            ...
            for head_name in self._sorted_head_names:
                ...
                if self._share_prediction_tower:
                    ...
                    head_tower_feature = _apply_layers(
                        self._base_tower_layers_for_heads[head_name][index], image_feature
                    )
