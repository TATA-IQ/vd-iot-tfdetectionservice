        ...
        if self._use_depthwise:
            ...
            self._class_predictor_layers.append(
                conv_hyperparams.build_batch_norm(
                    training=(is_training and not freeze_batchnorm), name="ClassPredictor_depthwise_batchnorm"
                )
            )
            self._class_predictor_layers.append(
                conv_hyperparams.build_activation_layer(name="ClassPredictor_depthwise_activation")
            )
            self._class_predictor_layers.append(
            ...
            self._class_predictor_layers.append(
                tf.keras.layers.Conv2D(
                    ...
                    padding="SAME",
                    name="ClassPredictor",
                    bias_initializer=tf.constant_initializer(self._class_prediction_bias_init),
                    **conv_hyperparams.params(use_bias=True),
                )
            )
...
class MaskRCNNClassHead(head.KerasHead):
    ...
    def __init__(
        self, is_training, num_class_slots, fc_hyperparams, freeze_batchnorm, use_dropout, dropout_keep_prob, name=None
    ):
        """Constructor.
        ...
        self._class_predictor_layers.append(
            fc_hyperparams.build_batch_norm(
                training=(is_training and not freeze_batchnorm), name="ClassPredictor_batchnorm"
            )
        )
