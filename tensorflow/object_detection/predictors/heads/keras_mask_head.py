...
class ConvolutionalMaskHead(head.KerasHead):
        ...
        if self._use_depthwise:
            ...
            self._mask_predictor_layers.append(
                conv_hyperparams.build_batch_norm(
                    training=(is_training and not freeze_batchnorm), name="MaskPredictor_depthwise_batchnorm"
                )
            )
            self._mask_predictor_layers.append(
                conv_hyperparams.build_activation_layer(name="MaskPredictor_depthwise_activation")
            )
            self._mask_predictor_layers.append(
    ...
    def _predict(self, features):
        ...
        mask_predictions = tf.reshape(
            mask_predictions, [batch_size, -1, self._num_masks, self._mask_height, self._mask_width]
        )
...
class MaskRCNNMaskHead(head.KerasHead):
    ...
    def build(self, input_shapes):
        ...
        for i in range(self._mask_prediction_num_conv_layers - 1):
            ...
            self._mask_predictor_layers.append(
                self._conv_hyperparams.build_activation_layer(name="MaskPredictor_activation_{}".format(i))
            )
        ...
        if self._convolve_then_upsample:
            ...
            self._mask_predictor_layers.append(
                self._conv_hyperparams.build_batch_norm(
                    training=(self._is_training and not self._freeze_batchnorm), name="MaskPredictor_upsample_batchnorm"
                )
            )
            self._mask_predictor_layers.append(
                self._conv_hyperparams.build_activation_layer(name="MaskPredictor_upsample_activation")
            )
