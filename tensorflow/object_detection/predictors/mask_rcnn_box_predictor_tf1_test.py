    ...
    def test_get_boxes_with_five_classes(self):
        def graph_fn(image_features):
            ...
            box_predictions = mask_box_predictor.predict(
                [image_features], num_predictions_per_location=[1], scope="BoxPredictor", prediction_stage=2
            )
            return (
    ...
    def test_get_instance_masks(self):
        def graph_fn(image_features):
            mask_box_predictor = box_predictor_builder.build_mask_rcnn_box_predictor(
                ...
                dropout_keep_prob=0.5,
                box_code_size=4,
                conv_hyperparams_fn=self._build_arg_scope_with_hyperparams(op_type=hyperparams_pb2.Hyperparams.CONV),
                predict_instance_masks=True,
            )
            box_predictions = mask_box_predictor.predict(
                [image_features], num_predictions_per_location=[1], scope="BoxPredictor", prediction_stage=3
            )
