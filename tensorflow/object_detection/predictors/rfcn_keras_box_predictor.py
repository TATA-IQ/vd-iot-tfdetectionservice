...
class RfcnKerasBoxPredictor(box_predictor.KerasBoxPredictor):
    ...
    def __init__(
        ...
        super(RfcnKerasBoxPredictor, self).__init__(
            is_training, num_classes, freeze_batchnorm=freeze_batchnorm, inplace_batchnorm_update=False, name=name
        )
        ...
        self._shared_conv_layers.append(
            tf.keras.layers.Conv2D(
                self._depth, [1, 1], padding="SAME", name="reduce_depth_conv", **self._conv_hyperparams.params()
            )
        )
        self._shared_conv_layers.append(
            self._conv_hyperparams.build_batch_norm(
                training=(self._is_training and not self._freeze_batchnorm), name="reduce_depth_batchnorm"
            )
        )
        ...
        location_feature_map_depth = (
            self._num_spatial_bins[0] * self._num_spatial_bins[1] * self.num_classes * self._box_code_size
        )
        ...
        self._box_encoder_layers.append(
            self._conv_hyperparams.build_batch_norm(
                training=(self._is_training and not self._freeze_batchnorm), name="refined_locations_batchnorm"
            )
        )
        ...
        self._class_predictor_layers.append(
            self._conv_hyperparams.build_batch_norm(
                training=(self._is_training and not self._freeze_batchnorm), name="class_predictions_batchnorm"
            )
        )
    ...
    def _predict(self, image_features, proposal_boxes, **kwargs):
        ...
        class_predictions_with_background = tf.reshape(
            class_predictions_with_background, [batch_size * num_boxes, 1, self._total_classes]
        )
