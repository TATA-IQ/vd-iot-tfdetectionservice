...
def build_graph(pipeline_config, shapes_info, input_type="encoded_image_string_tensor", use_bfloat16=True):
    ...
    def tpu_subgraph_predict_fn(preprocessed_inputs, true_image_shapes):
        ...
        return (
            ...
            # [batch, anchor, depth] -> [depth, batch, anchor]
            tf.transpose(prediction_dict[RPN_OBJECTNESS_PREDICTIONS_WITH_BACKGROUND], perm=[2, 0, 1]),
            # [anchors, depth]
    ...
    def tpu_subgraph_predict():
        if use_bfloat16:
            with bfloat16_scope():
                return tpu.rewrite(tpu_subgraph_predict_fn, [preprocessed_inputs, true_image_shapes])
        else:
            return tpu.rewrite(tpu_subgraph_predict_fn, [preprocessed_inputs, true_image_shapes])
    ...
    for k in prediction_dict:
        if isinstance(prediction_dict[k], list):
            prediction_dict[k] = [
                prediction_dict[k][idx].set_shape(shapes_info[k][idx]) for idx in len(prediction_dict[k])
            ]
        else:
