...
def build_graph(pipeline_config, shapes_info, input_type="encoded_image_string_tensor", use_bfloat16=False):
    ...
    def predict_tpu_subgraph(preprocessed_inputs, true_image_shapes):
        ...
        return [
            ...
            tf.transpose(prediction_dict[CLASS_PREDICTIONS_WITH_BACKGROUND], perm=[2, 0, 1]),
            tf.transpose(prediction_dict[ANCHORS], perm=[1, 0]),
        ]

    @function.Defun(capture_resource_var_by_value=False)
    def predict_tpu():
        return tpu.rewrite(predict_tpu_subgraph, [preprocessed_inputs, true_image_shapes])
    ...
    (preprocessed_inputs, box_encodings, class_predictions_with_background, anchors) = recover_shape(
        preprocessed_inputs, prediction_outputs, shapes_info
    )
