...
def create_conv_block(
    ...
    if apply_batchnorm:
        layers.append(
            conv_hyperparams.build_batch_norm(training=(is_training and not freeze_batchnorm), name=name + "batchnorm")
        )

    if apply_activation:
        activation_layer = conv_hyperparams.build_activation_layer(name=name + "activation")
    ...
    if downsample_method == "max_pooling":
        layers.append(
            tf.keras.layers.MaxPooling2D(
                pool_size=kernel_size, strides=stride, padding=padding, name=name + "downsample_max_x{}".format(stride)
            )
        )
    elif downsample_method == "avg_pooling":
        layers.append(
            tf.keras.layers.AveragePooling2D(
                pool_size=kernel_size, strides=stride, padding=padding, name=name + "downsample_avg_x{}".format(stride)
            )
        )
        ...
        layers.append(
            conv_hyperparams.build_batch_norm(
                training=(is_training and not freeze_batchnorm), name=name + "downsample_batchnorm"
            )
        )
        layers.append(conv_hyperparams.build_activation_layer(name=name + "downsample_activation"))
    else:
...
def create_upsample_feature_map_ops(scale, use_native_resize_op, name):
    ...
    if use_native_resize_op:
        ...
        def resize_nearest_neighbor(image):
            ...
            return tf.image.resize_nearest_neighbor(image, [image_shape[1] * scale, image_shape[2] * scale])

        layers.append(
            tf.keras.layers.Lambda(resize_nearest_neighbor, name=name + "nearest_neighbor_upsampling_x{}".format(scale))
        )
    else:
...
def create_resample_feature_map_ops(
    ...
    if input_scale_factor < output_scale_factor:
        if output_scale_factor % input_scale_factor != 0:
            raise ValueError(
                "Invalid scale factor: input scale 1/{} not divisible byoutput scale 1/{}".format(
                    input_scale_factor, output_scale_factor
                )
            )
        ...
        if input_scale_factor % output_scale_factor != 0:
            raise ValueError(
                "Invalid scale factor: input scale 1/{} not a divisor ofoutput scale 1/{}".format(
                    input_scale_factor, output_scale_factor
                )
            )
...
class BiFPNCombineLayer(tf.keras.layers.Layer):
    ...
    def _combine_weighted_sum(self, inputs):
        return tf.squeeze(tf.linalg.matmul(tf.stack(inputs, axis=-1), self.per_input_weights), axis=[-1])

    def _combine_attention(self, inputs):
        normalized_weights = tf.nn.softmax(self.per_input_weights)
        return tf.squeeze(tf.linalg.matmul(tf.stack(inputs, axis=-1), normalized_weights), axis=[-1])
    ...
    def build(self, input_shape):
        ...
        if len(input_shape) < 2:
            raise ValueError(
                "A BiFPN combine layer should be called on a list of at least 2 inputs. Got "
                + str(len(input_shape))
                + " inputs."
            )
        ...
        if self.combine_method in {"weighted_sum", "attention", "fast_attention"}:
            self.per_input_weights = self.add_weight(
                name="bifpn_combine_weights", shape=(len(input_shape), 1), initializer="ones", trainable=True
            )
        ...
        for i in range(1, len(input_shape)):
            if input_shape[i] != output_shape:
                raise ValueError(
                    ...
                    "but input_shape[0] is {} while input_shape[{}] is {}".format(output_shape, i, input_shape[i])
                )
