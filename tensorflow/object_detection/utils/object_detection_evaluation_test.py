...
from object_detection.utils import object_detection_evaluation, tf_version
...
class OpenImagesChallengeEvaluatorTest(tf.test.TestCase):
    def test_returns_correct_detection_metric_values(self):
        ...
        detected_boxes = np.array(
            [[10, 10, 11, 11], [100, 100, 120, 120], [100, 100, 220, 220], [10, 10, 11, 11]], dtype=float
        )
        ...
        self.assertAlmostEqual(
            metrics[expected_metric_name + "_PerformanceByCategory/AP@0.5IOU/elephant"], 0.333333333333
        )
...
class PrecisionAtRecallEvaluationTest(tf.test.TestCase):
    ...
    def test_returns_correct_metric_values(self):
        ...
        self.assertAlmostEqual(metrics[self.wp_eval._metric_prefix + "PerformanceByCategory/AP@0.5IOU/cat"], 0.5 / 4)
        self.assertAlmostEqual(
            metrics[self.wp_eval._metric_prefix + "Precision/mAP@0.5IOU@[0.0,0.5]Recall"], 1.0 / (3 + 1 + 2) / 4
        )
        self.wp_eval.clear()
        self.assertFalse(self.wp_eval._image_ids)
    ...
    def test_returns_correct_metric_values_with_difficult_list(self):
        ...
        self.assertAlmostEqual(metrics[self.wp_eval._metric_prefix + "PerformanceByCategory/AP@0.5IOU/cat"], 0.5 / 3)
        self.assertAlmostEqual(
            metrics[self.wp_eval._metric_prefix + "Precision/mAP@0.5IOU@[0.0,0.5]Recall"], 1.0 / (3 + 1 + 2) / 3
        )
        self.wp_eval.clear()
        self.assertFalse(self.wp_eval._image_ids)
    ...
    def test_add_single_detected_image_info(self):
        ...
        for i in range(self.od_eval.num_class):
            for j in range(len(expected_scores_per_class[i])):
                ...
                self.assertTrue(
                    np.array_equal(expected_tp_fp_labels_per_class[i][j], self.od_eval.tp_fp_labels_per_class[i][j])
                )
        self.assertTrue(
            np.array_equal(
                expected_num_images_correctly_detected_per_class, self.od_eval.num_images_correctly_detected_per_class
            )
        )
...
@unittest.skipIf(tf_version.is_tf2(), "Eval Metrics ops are supported in TF1.X only.")
class ObjectDetectionEvaluatorTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def _make_evaluation_dict(
        ...
        detection_boxes = tf.concat(
            [
                tf.tile(tf.constant([[[0.0, 0.0, 1.0, 1.0]]]), multiples=[batch_size - 1, 1, 1]),
                tf.constant([[[0.0, 0.0, 0.5, 0.5]]]),
            ],
            axis=0,
        )
        detection_scores = tf.concat(
            [tf.tile(tf.constant([[0.5]]), multiples=[batch_size - 1, 1]), tf.constant([[0.8]])], axis=0
        )
