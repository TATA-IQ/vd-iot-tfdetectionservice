...
class FeatureGridCoordinateTest(test_case.TestCase):
    def test_snap_box_points_to_nearest_4_pixels(self):
        ...
        (feature_grid_y0, feature_grid_x0, feature_grid_y1, feature_grid_x1) = self.execute(
    ...
    def test_feature_point_indices(self):
        ...
        def graph_fn(feature_grid_y, feature_grid_x, box_levels):
            return spatial_ops.ravel_indices(
                feature_grid_y, feature_grid_x, num_feature_levels, feature_height, feature_width, box_levels
            )
...
class MultiLevelRoIAlignTest(test_case.TestCase):
    ...
    def test_interpolation_with_4_points_per_bin(self):
        def graph_fn(image, boxes, levels):
            return spatial_ops.multilevel_roi_align(
                [image], boxes, levels, output_size=[1, 1], num_samples_per_cell_y=2, num_samples_per_cell_x=2
            )
    ...
    def test_large_input(self):
        if self.has_tpu():
            ...
            self.assertEqual(roi_features.shape, (batch_size, num_boxes, output_size[0], output_size[1], num_filters))
...
class MatMulCropAndResizeTest(test_case.TestCase):
    ...
    def testMatMulCropAndResize3x3To2x2_2Channels(self):
        ...
        image = np.array(
            [[[[1, 0], [2, 1], [3, 2]], [[4, 3], [5, 4], [6, 5]], [[7, 6], [8, 7], [9, 8]]]], dtype=np.float32
        )
        boxes = np.array([[[0, 0, 1, 1], [0, 0, 0.5, 0.5]]], dtype=np.float32)
    ...
    def testMultilevelMatMulCropAndResize(self):
        def graph_fn(image1, image2, boxes, box_levels):
            return spatial_ops.multilevel_matmul_crop_and_resize([image1, image2], boxes, box_levels, crop_size=[2, 2])
        ...
        boxes = np.array([[[1, 1, 0, 0], [0.5, 0.5, 0, 0]], [[0, 0, 1, 1], [0, 0, 0.5, 0.5]]], dtype=np.float32)
        box_levels = np.array([[0, 1], [1, 1]], dtype=np.int32)
        expected_output = [
            [[[[9, 0], [7, 0]], [[3, 0], [1, 0]]], [[[5, 4], [4, 3]], [[2, 1], [1, 0]]]],
            [[[[1, 0], [3, 2]], [[7, 6], [9, 8]]], [[[1, 0], [2, 1]], [[4, 3], [5, 4]]]],
        ]
        crop_output = self.execute(graph_fn, image + [boxes, box_levels])
        self.assertAllClose(crop_output, expected_output)
...
class NativeCropAndResizeTest(test_case.TestCase):
    ...
    def testMultilevelBatchCropAndResize3x3To2x2_2Channels(self):
        def graph_fn(image1, image2, boxes, box_levels):
            return spatial_ops.multilevel_native_crop_and_resize([image1, image2], boxes, box_levels, crop_size=[2, 2])
        ...
        image = [
            ...
            np.array([[[[1, 0], [2, 1]], [[4, 3], [5, 4]]], [[[1, 0], [2, 1]], [[4, 3], [5, 4]]]], dtype=np.float32),
        ]
        boxes = np.array([[[0, 0, 1, 1], [0, 0, 0.5, 0.5]], [[1, 1, 0, 0], [0.5, 0.5, 0, 0]]], dtype=np.float32)
        box_levels = np.array([[0, 1], [0, 0]], dtype=np.float32)
        expected_output = [
            [[[[1, 0], [3, 2]], [[7, 6], [9, 8]]], [[[1, 0], [1.5, 0.5]], [[2.5, 1.5], [3, 2]]]],
            [[[[9, 8], [7, 6]], [[3, 2], [1, 0]]], [[[5, 4], [4, 3]], [[2, 1], [1, 0]]]],
        ]
