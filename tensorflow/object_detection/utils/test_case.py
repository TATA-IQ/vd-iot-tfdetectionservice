...
if not tf2.enabled():
    from tensorflow.contrib import \
        tpu as \
        contrib_tpu  # pylint: disable=g-import-not-at-top, line-too-long
    ...
    def execute_tpu_tf1(self, compute_fn, inputs, graph=None):
        ...
        with self.session(graph=(graph or tf.Graph())) as sess:
            ...
            def wrap_graph_fn(*args, **kwargs):
                ...
                if not (isinstance(results, dict) or isinstance(results, tf.Tensor)) and hasattr(results, "__iter__"):
